...
# copy_safe (lib.sh:852) OVERWRITES the delivered .getff/ artefacts under --force. The lock — whose whole
...
if [ -z "$TSX_BIN" ]; then
  ...
  if [ -n "$PRACTICE_FIX" ]; then
    ...
    # there; setup.d/lib.sh:1598-1599, the lock writer both lanes share). The prior arm pointed at
    ...
    # by construction (setup.d/lib.sh:1627,1663 — the shared lock writer). REVERSE direction: cargo/go producers do
    ...
    # (setup.d/lib.sh:1598-1599, the lock writer both lanes share) — NOT .getff/ (only the PYTHON
