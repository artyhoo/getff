...
# at install.sh:547, and do_refresh lives at :747. Inside _py_deliver_agent_surface every delivery
...
# do_refresh's own contract (install.sh:739) names RULES.md / DESCRIPTION.md / ARCHITECTURE.md
...
# ── (5) A2-4: the framework-owned AI-USAGE-GUIDE.md IS refreshed (do_refresh install.sh:1373) ─────
...
if grep -qF 'STALE-GUIDE' "$P/.ai-factory/AI-USAGE-GUIDE.md"; then
  bad "(5) AI-USAGE-GUIDE.md not refreshed — diverges from do_refresh (install.sh:1373)"
else
...
EOF
  ...
  if grep -qF 'MY-OWN-RULE' "$RULES"; then
    ...
    bad "(9) --refresh overwrote RULES.md — it is consumer-authored (install.sh:739)"
  fi
fi
rm -rf "$PR"
