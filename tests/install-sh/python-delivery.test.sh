...
# ── Cell (ci): consumer CI workflow (python-delivery-v0 S2 Task 2) ────────────
# getff-NAMESPACED .github/workflows/getff-python.yml; never touches the consumer's own ci.yml.
echo ""; echo "  ── cell (ci): consumer CI workflow (pinned gates; no ci.yml clobber) ──"

# (ci-a) fresh dir → the pinned CI workflow is delivered byte-identical + carries the exact pins.
P=$(mktemp -d); printf '{"name":"cci","version":"0.0.0"}\n' > "$P/package.json"
run_delivery "$P" >/dev/null 2>&1
cmp -s "$TPL/github-actions-ci.yml" "$P/.github/workflows/getff-python.yml" \
  && ok "(ci-a) getff-python.yml copied byte-identical to the pinned CI template (fresh dir)" \
  || bad "(ci-a) getff-python.yml missing or differs from template"
grep -qF '@ast-grep/cli@0.44.1' "$P/.github/workflows/getff-python.yml" \
  && grep -qF 'ruff==0.15.21' "$P/.github/workflows/getff-python.yml" \
  && ok "(ci-a) delivered workflow carries the exact @ast-grep/cli@0.44.1 + ruff==0.15.21 pins (Rule A)" \
  || bad "(ci-a) delivered workflow missing an exact tool pin (ci-tool-pinning Rule A)"

# (ci-b) a consumer's own ci.yml is NEVER touched — getff writes only its namespaced file.
P=$(mktemp -d); printf '{"name":"cci2","version":"0.0.0"}\n' > "$P/package.json"
mkdir -p "$P/.github/workflows"; printf 'name: consumer ci\n' > "$P/.github/workflows/ci.yml"
run_delivery "$P" >/dev/null 2>&1
grep -qxF 'name: consumer ci' "$P/.github/workflows/ci.yml" \
  && ok "(ci-b) consumer's own ci.yml left byte-untouched (getff never clobbers a consumer workflow)" \
  || bad "(ci-b) consumer ci.yml was modified — STOP-line breach"
[ -f "$P/.github/workflows/getff-python.yml" ] \
  && ok "(ci-b) getff-python.yml delivered alongside the consumer's ci.yml" \
  || bad "(ci-b) getff-python.yml not delivered when a consumer ci.yml is present"

# (ci-c) a NON-getff file at OUR namespaced path → REFUSE loudly, never overwrite.
P=$(mktemp -d); printf '{"name":"cci3","version":"0.0.0"}\n' > "$P/package.json"
mkdir -p "$P/.github/workflows"; printf 'name: consumer-authored getff-python\n' > "$P/.github/workflows/getff-python.yml"
...
# (ci-d) --refresh (GETFF_TOOLCHAIN_REFRESH=1) overwrites our OWN stale getff-owned workflow.
P=$(mktemp -d); printf '{"name":"cci4","version":"0.0.0"}\n' > "$P/package.json"
run_delivery "$P" >/dev/null 2>&1                                          # install → delivers ours
printf '# generated by getff — stale\n' > "$P/.github/workflows/getff-python.yml"   # simulate stale getff copy
GETFF_TOOLCHAIN_REFRESH=1 run_delivery "$P" >/dev/null 2>&1
cmp -s "$TPL/github-actions-ci.yml" "$P/.github/workflows/getff-python.yml" \
  && ok "(ci-d) --refresh overwrote the stale getff-owned workflow with the current pinned template" \
  || bad "(ci-d) --refresh did not refresh the getff-owned workflow"
...
# OWN config is what ruff discovers, so the getff TID bans were SILENTLY unenforced (probe-proven).
# Fix: the lane ALWAYS delivers the getff bans to a stable .getff/ruff-bans.toml (fresh + every
# collision cell), and the CI points an isolated `ruff check . --config .getff/ruff-bans.toml` gate at
# it (--config REPLACES discovery — probe-proven). These arms assert the stable file lands in EVERY
# cell and the template carries the isolated gate.
echo ""; echo "  ── cell (bans): stable .getff/ruff-bans.toml delivered in every cell (S2-T2) ──"
for _cell in fresh collision-ruff collision-pyproject; do
  P=$(mktemp -d); printf '{"name":"cb","version":"0.0.0"}\n' > "$P/package.json"
  case "$_cell" in
    collision-ruff)      printf '[lint]\nselect = ["E"]\n' > "$P/ruff.toml" ;;         # cell (iii)
    collision-pyproject) printf '[project]\nname="cb"\n\n[tool.ruff.lint]\nselect=["E"]\n' > "$P/pyproject.toml" ;;  # cell (iv)
  esac
  run_delivery "$P" >/dev/null 2>&1
  cmp -s "$TPL/ruff.toml" "$P/.getff/ruff-bans.toml" \
    && ok "(bans/$_cell) .getff/ruff-bans.toml delivered byte-identical to the getff ruff bans (stable CI-gate target)" \
    || bad "(bans/$_cell) .getff/ruff-bans.toml missing or differs from the template bans — CI gate would have no cell-independent config"
done

# The delivered .getff/ruff-bans.toml carries the getff header (framework-owned, never a consumer file).
P=$(mktemp -d); printf '{"name":"cbh","version":"0.0.0"}\n' > "$P/package.json"
run_delivery "$P" >/dev/null 2>&1
grep -q 'generated by getff' "$P/.getff/ruff-bans.toml" \
  && ok "(bans) .getff/ruff-bans.toml is getff-header-marked (framework-owned, header-stamped)" \
  || bad "(bans) .getff/ruff-bans.toml lacks the getff header"

# The shipped CI template must carry the ISOLATED getff-bans gate (--config .getff/ruff-bans.toml),
# in ADDITION to the bare `ruff check .` step — else the collision-cell theatre returns.
grep -qF 'ruff check . --config .getff/ruff-bans.toml' "$TPL/github-actions-ci.yml" \
  && ok "(bans) CI template carries the isolated getff-bans gate: 'ruff check . --config .getff/ruff-bans.toml'" \
  || bad "(bans) CI template has NO isolated getff-bans gate — bare 'ruff check .' silently misses our bans in ruff-collision cells"
grep -qE '^[[:space:]]*run:[[:space:]]*ruff check \.[[:space:]]*$' "$TPL/github-actions-ci.yml" \
  && ok "(bans) CI template keeps the discovered-config 'ruff check .' step alongside the getff-bans gate" \
  || bad "(bans) CI template lost the discovered-config 'ruff check .' step"
...
# ── LIVE-FIRE: getff-bans gate fires in a ruff-COLLISION cell where bare `ruff check .` is BLIND ──
# The S2-T2 theatre: a consumer with their OWN ruff config (no getff bans). Bare `ruff check .`
# discovers THEIR config → a planted TID violation passes GREEN. The isolated getff gate
# (`ruff check . --config .getff/ruff-bans.toml`) fires RED on the SAME tree. GATED on RUFF_RUN.
echo ""; echo "  ── live-fire: getff-bans gate vs bare check in a ruff-collision cell (S2-T2; SKIP if no ruff) ──"
if [ -n "$RUFF_RUN" ]; then
  P=$(mktemp -d); printf '{"name":"lfb","version":"0.0.0"}\n' > "$P/package.json"
  # consumer's OWN ruff.toml — selects E only, NOT our TID bans (collision cell iii).
  printf '[lint]\nselect = ["E"]\n' > "$P/ruff.toml"
  run_delivery "$P" >/dev/null 2>&1
  # a file that violates OUR getff TID ban but is clean under the consumer's select=["E"].
  ...
  skip "live-fire getff-bans-gate SKIP — ruff==0.15.21 not obtainable; deterministic (bans) arms above carry CI"
fi
