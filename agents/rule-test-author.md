...
> **NOT authoritative for:** project goal — see the consumer's README.md; rule CREATION (detect stack → research → author a rule) — that is `agents/rule-researcher.md` / `/rule-research`; the enrichment-sidecar schema — defined in this umbrella's sidecar stage (S2, landed), this doc POINTS at it and never duplicates it; the emitted rule artifact itself — drift/hash-gated, this protocol never edits it.
...
> **Committed-fixture honesty (S5):** **astgrep is the only lane with a checked-in framework fixture** (`packages/core/synthesizer/fixtures/live-generation/rule-tests/astgrep.json`), so it is the only lane whose firing is proven by a committed artefact. The **ruff and cargo** firing paths are proven by **synthetic smokes** (the S5 pre-push tests seed a delivered-shape config + sidecar into a sandbox) — real, but not a checked-in fixture. Do not report ruff/cargo firing as fixture-backed; a per-lane committed fixture is the pending residual (#1093 follow-up).
...
- **astgrep — sidecar format landed + committed fixture (S2).** The rule configs ship (`.getff/astgrep-rules/*.yml`) and the enrichment-sidecar format is landed (`.ai-factory/rule-tests/astgrep.json`, `packages/core/synthesizer/rule-tests-sidecar.ts`); the framework carries a committed astgrep fixture (`packages/core/synthesizer/fixtures/live-generation/rule-tests/astgrep.json`) that the S5 firing runner + tests fire against. Full write→verify loop works today.
- **ruff / cargo — sidecar format landed, committed fixtures pending.** The configs ship (`.getff/ruff-bans.toml`; cargo `clippy.toml` via W4) and the sidecar format accepts these lanes, but no committed framework fixture ships for them yet: the ruff/cargo firing paths are proven by synthetic smokes (S5 tests seed a delivered-shape config + sidecar in a sandbox), not by a checked-in fixture. Consumer repair works against the delivered configs; a committed framework fixture per lane is the pending residual.
...
State plainly which lane you worked, what you edited (the material, never the rule), the exact isolation command, and the tool's own verdict verbatim. Only **astgrep** carries a committed framework fixture today; **ruff/cargo** firing is proven by synthetic smokes, not a checked-in fixture (see the per-lane honest-limits above) — if you worked a lane without a committed fixture, say so rather than implying checked-in coverage. Run the verify-half honestly rather than faking a repair. A smaller honest result beats a padded one.
