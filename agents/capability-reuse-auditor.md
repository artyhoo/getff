...
1. **Name the capability's problem-class in one sentence** — what does it *do*, functionally (not by its name)? (Counters T16 `#pattern-matching-on-name`: a name-similar existing tool may solve a different problem, and vice-versa.)
2. **Enumerate the candidate overlap set** (state counts before judging — T10):
   - `Grep`/`Glob` the SSOT ([docs/meta-factory/prior-art-evaluations.md](../docs/meta-factory/prior-art-evaluations.md)) for the capability-area (≥3 phrasings of the *function*, per [phase-research-coverage.md §1](../.claude/rules/phase-research-coverage.md) semantic-distance check). *(Consumer repo without that SSOT: substitute your own prior-art register, or use the existing `.claude/skills/` + `agents/` set itself as the overlap corpus — ask the operator to confirm the register path.)*
...
4. **Trailer↔body consistency** (if a trailer/verdict is supplied): does the body *do* what the verdict claims?
...
|---|---|---|
| **REUSE-EXISTING** | An own-stack/upstream capability already covers this problem-class; the new one adds no distinct residue. | Do not create it. Invoke/extend the existing one. (Merge > Improve > Create — dedup-first, [SSOT #189](../docs/meta-factory/prior-art-evaluations.md).) |
