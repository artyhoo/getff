...
function isServerOnlyImport(spec: string): boolean {
  ...
  if (/(^|[/@])infrastructure(\/|$)/.test(spec)) return true;
  if (spec.includes('config/env')) return true;
  return false;
}
  ...
  meta: {
    ...
    messages: {
      noServerImportInClient:
        "'use client' file cannot import server-only module `{{module}}` (R12). Move the dependency to a server boundary or wrap in an infrastructure adapter.",
    },
    schema: [],
  },
  defaultOptions: [],
  create(context) {
    ...
    return {
      ImportDeclaration(node: TSESTree.ImportDeclaration) {
        ...
        if (!isServerOnlyImport(node.source.value)) return;
        if (isExempt(lines[node.loc.start.line - 1] ?? '')) return;
        context.report({
          node,
