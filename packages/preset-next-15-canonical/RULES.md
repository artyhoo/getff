...
edit-time, getff's own rule-globs / lint-staged / generated-rule / link checks at pre-push,
and AI Factory's `rules-sidecar` (which reads this file) at `/aif-verify`. Your `tsc`,
`depcruise` and test suite are yours to wire — the pre-push hook does not run them. Each rule has a corresponding
