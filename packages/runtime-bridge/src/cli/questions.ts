...
export function parseQuestionsArgs(argv: string[], env: NodeJS.ProcessEnv): QuestionsArgs {
  ...
  return {
    projectId: (values.project as string | undefined) ?? env.RUNTIME_BRIDGE_AIF_PROJECT_ID ?? undefined,
...
async function main(): Promise<void> {
  ...
  let json = false;
  try {
    ({ projectId, json } = parseQuestionsArgs(process.argv.slice(2), process.env));
  } catch (err) {
