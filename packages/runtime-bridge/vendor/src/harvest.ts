...
export function scanParkSignals(task: ParkScanInput): string[] {
  const haystack = [
    ...
    .join('\n');
  if (!haystack) return [];
  return PARK_MARKERS.filter(([, re]) => re.test(haystack)).map(
    ([name]) => name,
  );
}
...
export function extractAffectedFiles(
  ...
  let found = false;
  for (
    let m = re.exec(reviewComments);
    m !== null;
    m = re.exec(reviewComments)
  ) {
    try {
...
export function resolveWorkDir(opts: {
  ...
  if (opts.taskWorktreePath)
    return { path: opts.taskWorktreePath, source: 'task-record' };
...
export async function harvestTask(
  ...
  if (!branch) {
    throw new Error(
      `harvest: task ${task.id} has no branchName — aif did not create/persist a feature branch`,
    );
  }
  ...
  if (await deps.hasUncommittedChanges(branch)) {
    if ((await deps.commitsAhead(branch, opts.baseBranch)) === 0) {
      ...
      await deps.commitAll(
        branch,
        `chore(harvest): commit reworked aif task ${task.id} — ${task.title}`,
      );
      committed = true;
    } else {
