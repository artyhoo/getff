...
# `{a,b}` braces, `!` exclusions; a leading `./` and a trailing `/` are normalised away —
# npm documents the `./packages/a` form — and a trailing ` # comment` on a
# pnpm-workspace.yaml line is dropped) over candidate dirs ≤8 levels deep, node_modules/.git/
...
function memberDirs(globs){const inc=[],exc=[];for(const g of globs){const neg=g.charAt(0)==="!";const body=nz(neg?g.slice(1):g);for(const p of braces(body))(neg?exc:inc).push(g2r(p));}if(!inc.length)return[];const cand=[];dirsUnder(".",1,cand);const seen={},out=[];for(const d of cand){let ok=false;for(const r of inc)if(r.test(d)){ok=true;break;}if(!ok)continue;let bad=false;for(const r of exc)if(r.test(d)){bad=true;break;}if(bad)continue;if(seen[d])continue;seen[d]=1;if(fs.existsSync(d+"/package.json"))out.push(d);}return out.sort();}
