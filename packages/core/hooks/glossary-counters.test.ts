...
function readCounts(counts: string): {
  terms: Record<string, { usages?: number; explanations?: number }>;
} {
...
function runInject(
  ...
  const r = spawnSync('bash', [INJECT_HOOK], {
    input: JSON.stringify({
      prompt,
      session_id: opts.sessionId ?? 'sess-inject',
    }),
    encoding: 'utf8',
    env: {
      ...process.env,
...
function writeTranscript(dir: string, answer: string): string {
  ...
  const lines = [
    JSON.stringify({
      type: 'user',
      message: { content: 'объясни термин приземлить' },
    }),
    JSON.stringify({
      type: 'assistant',
...
function runStop(opts: StopOpts): RunResult & { sb: string; residue: string } {
  ...
  const residue =
    opts.residueDir ?? mkdtempSync(join(tmpdir(), 'glossary-stop-res-'));
  if (!opts.residueDir) tmpDirs.push(residue);
  const sessionId = opts.sessionId ?? 'sess-stop';
  if (opts.pending?.length) {
    writeFileSync(
      pendingPath(sb, sessionId),
      opts.pending.map(([t, w]) => `${t}\t${w}`).join('\n') + '\n',
      'utf8',
    );
  }
  if (opts.counts)
    writeCounts(join(residue, '_glossary-counts.json'), opts.counts);
...
describe('glossary inject hook — usage counting', () => {
  it.skipIf(!JQ)(
    'counts a raw _Operator says_ word and injects the term line',
    () => {
      const r = runInjectTracked('объясни термин приземлить пожалуйста', {
        sessionId: 'sess-raw',
      });
      ...
      expect(readFileSync(pendingPath(r.sb.dir, 'sess-raw'), 'utf8')).toContain(
        'Land\tприземлить',
      );
    },
    SLOW_SHELL_MS,
  ...
  it.skipIf(!JQ)(
    'counts the term itself — a Latin term matches its own name',
    () => {
      const r = runInjectTracked('what does harvest mean here', {
        sessionId: 'sess-term',
      });
      expect(r.status).toBe(0);
  ...
  it.skipIf(!JQ)(
    'counts the transliteration — «харвест» is a usage of Harvest',
    () => {
      ...
      const r = runInjectTracked('что значит харвест тут', {
        sessionId: 'sess-translit',
      });
      expect(r.status).toBe(0);
  ...
  it.skipIf(!JQ)(
    'term + transliteration in one prompt is still ONE usage',
    () => {
      const r = runInjectTracked('harvest или харвест — одно и то же', {
        sessionId: 'sess-translit2',
      });
      expect(r.status).toBe(0);
      expect(readCounts(r.sb.counts).terms.Harvest.usages).toBe(1);
    },
  ...
  it.skipIf(!JQ)(
    'every Latin-named term carries a Cyrillic transliteration in CONTEXT.md',
    () => {
      ...
      for (const term of ['Harvest', 'Egress', 'Handoff']) {
        const block =
          ctx.split(/^## /m).find((b) => b.startsWith(`${term}\n`)) ?? '';
        const says =
  ...
  it.skipIf(!JQ)(
    'a synonym is NOT a usage — «слияние» means merge but matches nothing',
    () => {
      const r = runInjectTracked('объясни слияние ветки', {
        sessionId: 'sess-syn',
      });
      expect(r.status).toBe(0);
  ...
  it.skipIf(!JQ)(
    'an inflected form is NOT a usage — «приземлироваться» ≠ «приземли»',
    () => {
      ...
      const r = runInjectTracked('надо приземлироваться аккуратно', {
        sessionId: 'sess-infl',
      });
      expect(r.status).toBe(0);
      expect(r.stdout).toBe('');
  ...
  it.skipIf(!JQ)(
    'counts ONE usage per term per prompt even when the word appears twice',
    () => {
      const r = runInjectTracked('приземлить это приземлить', {
  ...
  it.skipIf(!JQ)(
    'matches the word inside a sentence (boundary classes, not substring)',
    () => {
      const r = runInjectTracked('давай приземлим? нет — приземлить!', {
...
describe('glossary thresholds — pack default, env override wins (fork ruling 3)', () => {
  it.skipIf(!JQ)(
    'pack default 3: at usages=2 the term still fires and counts the third',
    () => {
      ...
      const r2 = runInject('объясни термин приземлить', {
        sessionId: 'sess-default',
        env: { AIF_RESIDUE_DIR: r.sb.dir, TMPDIR: r.sb.dir } as Record<
          string,
          string
        >,
      });
  ...
  it.skipIf(!JQ)(
    'env AIF_GLOSSARY_USES=1 beats the pack: at usages=1 the term is learned',
    () => {
  ...
  it.skipIf(!JQ)(
    'env override cuts the OTHER way too: USES=5 keeps a 3-use term teaching',
    () => {
  ...
  it.skipIf(!JQ)(
    'learned-OR: explanations ≥ pack default silences the inject side too',
    () => {
      const sb = sandbox();
      writeCounts(sb.counts, { Land: { explanations: 5 } }); // explanations alone crossed
      const after = runInject('объясни термин приземлить', {
        sessionId: 'sess-or',
        env: { AIF_RESIDUE_DIR: sb.dir, TMPDIR: sb.dir } as Record<
          string,
          string
        >,
      });
...
describe('glossary Stop arm — explanation counting + the once-per-term demand', () => {
  it.skipIf(!JQ)(
    'counts the fixed form once per message and demands nothing that turn',
    () => {
      const r = runStop({
        sessionId: 'sess-exp',
  ...
  it.skipIf(!JQ)(
    'the form twice in one message is still ONE explanation',
    () => {
      const r = runStop({
        sessionId: 'sess-once',
        answer:
          'Land (merging the branch) and again Land (merging the branch).',
        pending: [['Land', 'приземлить']],
        counts: { Land: { usages: 2 } },
      });
      expect(
        readCounts(join(r.residue, '_glossary-counts.json')).terms.Land
          .explanations,
      ).toBe(1);
    },
    SLOW_SHELL_MS,
  ...
  it.skipIf(!JQ)(
    'below threshold without the form → ONE block demanding the inline form',
    () => {
      const r = runStop({
  ...
  it.skipIf(!JQ)(
    'the demand is one-shot per term: the next unexplained stop stays silent',
    () => {
  ...
  it.skipIf(!JQ)(
    'learned between prompt and Stop → nothing fires and the pending is dropped',
    () => {
      const r = runStop({
...
describe('same-path falsifier + inertness', () => {
  it.skipIf(!JQ)(
    'BOTH hooks resolve the SAME counters path — the kickoff item-4 falsifier',
    () => {
  ...
  it.skipIf(!JQ)(
    'absent CONTEXT.md → inject side fully inert (unarmed tree)',
    () => {
  ...
  it.skipIf(!JQ)(
    'absent CONTEXT.md → Stop arm inert even with a pending file (goldens stable)',
    () => {
      ...
      tmpDirs.push(residue);
      writeCounts(join(residue, '_glossary-counts.json'), {
        Land: { usages: 1 },
      });
...
describe('lang-pack parity for the AIF_GLOSSARY_ key class', () => {
  ...
  it('the word class enumerates its letters — no range, which glibc C.UTF-8 grep rejects', () => {
    // A range like the original `А-Я` passes on macOS grep but makes GNU grep under the
    // ubuntu CI default locale (C.UTF-8) fail with "Invalid collation character"; the hook's
    // `grep ... || continue` then skips every term and the inject side goes silent (PR 1824
    // CI, 2026-09-21). This static arm catches the range form on any host, before CI.
    for (const pack of [EN_PACK, RU_PACK]) {
      const m = readFileSync(pack, 'utf8').match(
        /^AIF_GLOSSARY_WORD_CLASS='([^']*)'$/m,
      );
      expect(m, `${pack} must define AIF_GLOSSARY_WORD_CLASS`).not.toBeNull();
      expect(m![1], `${pack} word class must not use a range`).not.toContain(
        '-',
      );
      expect(
        [...m![1]].length,
        `${pack} word class must carry all 66 Cyrillic letters`,
      ).toBe(66);
    }
  });

  it.skipIf(!JQ)(
    'check-parity.sh passes on the live packs (OK summary)',
    () => {
  ...
  it.skipIf(!JQ)(
    'PAIRED-NEGATIVE: an AIF_GLOSSARY_ key in ONE pack only → DRIFT naming it',
    () => {
      ...
      const r = spawnSync('bash', [join(box, 'check-parity.sh')], {
        encoding: 'utf8',
      });
...
describe('pack-lag (consumer delivery lag) — the pre-feature pack must not abort either hook', () => {
  ...
  function runStopFrom(
    script: string,
    opts: StopOpts,
  ): RunResult & { sb: string; residue: string } {
    const sb = opts.tmpDir ?? mkdtempSync(join(tmpdir(), 'glossary-stop-tmp-'));
    if (!opts.tmpDir) tmpDirs.push(sb);
    const residue =
      opts.residueDir ?? mkdtempSync(join(tmpdir(), 'glossary-stop-res-'));
  ...
  it.skipIf(!JQ)(
    'PAIRED-NEGATIVE: no demand function + no AIF_GLOSSARY_* keys → exit 0, no demand, no one-shot flag',
    () => {
      ...
      expect(existsSync(join(r.sb, 'aif-glossary-dem-sess-lag1-Land'))).toBe(
        false,
      );
      const counts = readCounts(join(r.residue, '_glossary-counts.json'));
      expect(counts.terms.Land.explanations).toBeUndefined();
  ...
  it.skipIf(!JQ)(
    'PAIRED-NEGATIVE: the explanation still COUNTS against a pre-feature pack',
    () => {
      const box = packLagBox();
      const r = runStopFrom(join(box, 'end-of-turn-reminder.sh'), {
        sessionId: 'sess-lag2',
  ...
  it.skipIf(!JQ)(
    'PAIRED-NEGATIVE: the inject hook survives a pre-feature pack and still injects',
    () => {
      const box = packLagBox();
      const sb = sandbox();
      const r = spawnSync('bash', [join(box, 'glossary-inject.sh')], {
        input: JSON.stringify({
          prompt: 'объясни термин приземлить',
          session_id: 'sess-lag3',
        }),
        encoding: 'utf8',
        env: {
          ...process.env,
...
describe('concurrent sessions — the counters file is machine-shared, so the increment must not lose updates', () => {
  ...
  it.skipIf(!JQ)(
    `${N} CONCURRENT prompts each carrying one term count ${N} usages, not fewer`,
    () => {
      const sb = sandbox();
  ...
  it.skipIf(!JQ)(
    `control: the same ${N} prompts run SEQUENTIALLY also count ${N}`,
    () => {
      const sb = sandbox();
      writeCounts(sb.counts, {});
  ...
  it.skipIf(!JQ)(
    'the lock is released — no lock directory survives a run',
    () => {
      const sb = sandbox();
      writeCounts(sb.counts, {});
      spawnInjects(sb.dir, true);
