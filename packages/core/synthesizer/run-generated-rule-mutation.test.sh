...
TEST_ROOT="$(git -C "$(dirname "$0")" rev-parse --show-toplevel)"
...
HOOK
  ...
  if [ -x /app/node_modules/.bin/tsx ] && [ -x /app/node_modules/.bin/eslint ]; then
    ok "a3p pre-fix mask acknowledged — /app fallback resolves .bin; wrong-root witness carried by a1p"
  else
...
arm_b() {
  ...
  git clone -q "$SCRATCH/remote.git" "$PLAIN" 2>/dev/null || { bad "arm (b): clone failed"; return; }
...
arm_c() {
  ...
  local PLAIN="$SCRATCH/plain2"
  git clone -q "$SCRATCH/remote.git" "$PLAIN" 2>/dev/null || { bad "arm (c): clone failed"; return; }
  mkdir -p "$PLAIN/scripts" "$PLAIN/.ai-factory/synthesizer-output" "$PLAIN/node_modules/.bin"
  cp "$RUNNER" "$PLAIN/scripts/run-generated-rule-mutation.sh"
  printf '#!/bin/sh\nexit 0\n' > "$PLAIN/node_modules/.bin/tsx"
  printf '#!/bin/sh\nexit 0\n' > "$PLAIN/node_modules/.bin/eslint"
