...
const REGISTRY_KEY_DECL_RE = /^\s{2}(FF[1-7]\d{3}):\s*\{/gm;
...
describe('diagnostics registry — structural invariants', () => {
  ...
  it('(a) every code matches ^FF[1-7]\\d{3}$', () => {
    for (const code of codes) {
      expect(code, `code "${code}" does not match ^FF[1-7]\\d{3}$`).toMatch(/^FF[1-7]\d{3}$/);
    }
  });
