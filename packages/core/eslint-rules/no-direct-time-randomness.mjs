...
const createRule = ESLintUtils.RuleCreator(() => `https://github.com/artyhoo/getff/blob/main/packages/preset-next-15-canonical/RULES.md#r7--time-randomness-io`);
    ...
    create(context) {
        ...
        return {
            "CallExpression[callee.type='MemberExpression'][callee.object.name='Date'][callee.property.name='now']"(node) {
                if (lineExempt(node.loc.start.line))
                    return;
            ...
            ImportDeclaration(node) {
                if (typeof node.source.value !== 'string')
                    return;
                if (!FORBIDDEN_MODULES.has(node.source.value))
                    return;
                if (lineExempt(node.loc.start.line))
                    return;
                context.report({
