...
var KNOWN_ECOSYSTEM_PREFIXES = /* @__PURE__ */ new Set(["npm", "cargo", "pip", "go"]);
...
function candidatePaths(framework, version, patternId) {
  const paths = [];
  if (framework) {
    const coerced = version ? import_semver.default.coerce(version) : null;
    const major = coerced?.major ?? null;
    if (major !== null) {
      paths.push(resolve2(STORE_ROOT, framework, `${major}.x`, `${patternId}.json`));
      if (major > 0) {
        paths.push(
          resolve2(STORE_ROOT, framework, `${major - 1}.x`, `${patternId}.json`)
        );
      }
    }
    paths.push(resolve2(STORE_ROOT, framework, "any", `${patternId}.json`));
  }
  paths.push(resolve2(STORE_ROOT, "shared", `${patternId}.json`));
  return paths;
}
...
var RESTRICTED_SYNTAX_SHAPED = /* @__PURE__ */ new Set([
  "no-restricted-syntax",
  ESLINT_RESTRICTED_RULE_NAME
]);
var RuleCollisionError = class extends Error {
...
function synthesize(plan) {
  ...
  for (const entry of plan.patterns) {
    ...
    if (rule.check.type === "declarative" && (!rule.check.engine || rule.check.engine === "eslint-restricted")) {
      mergeEslintRuleConfig(
        mergedEslintConfig,
        {
...
function generateDegradedSnippet(configPath) {
  return [
    `\xB7 R2 not auto-wired: AST editor unavailable (Node or ts-morph not present).`,
...
function wrapperSelectorsPresent(source, arrValue) {
  ...
  return entries.every((e) => {
    const sel = typeof e === "object" && e !== null ? e.selector : void 0;
...
function buildRuleValueExpr(value) {
  ...
  if (Array.isArray(value)) {
    ...
    const entries = value.slice(1).filter((e) => typeof e === "object" && e !== null && e.selector).map((e) => {
...
async function main() {
  ...
  switch (result.status) {
    case "already-wired":
      console.log(`\xB7 R2 already enforced in ${configPath} (no change)`);
      process2.exit(0);
      break;
    case "degrade":
      console.log(generateDegradedSnippet(configPath));
...
if (process2.argv[1] && process2.argv[1].endsWith("wire-eslint-r2.ts")) {
  main().catch((err) => {
...
async function main2() {
  ...
  if (stackIdx < 0 || !argv[stackIdx + 1]) {
    console.error("  \xB7 synth-and-wire: --stack <name> is required");
    process3.exit(0);
  }
  ...
  if (dryRun) {
    if (!existsSync4(configPath)) {
      ...
      if (result2.status === "already-wired") {
        ...
        console.log(`  [dry-run] [synth-wire] would wire synthesized rules into ${configPath} (status: ${result2.status})`);
      }
    }
    process3.exit(0);
  }
