...
const FAMILY_IDS = ['A', 'B', 'C', 'D', 'E', 'F1', 'F2', 'F3', 'G', 'H', 'I'] as const;
...
function absentEnums(node: unknown, parentKey = '', out: string[] = []): string[] {
  if (Array.isArray(node)) {
    node.forEach((x) => absentEnums(x, parentKey, out));
  } else if (node && typeof node === 'object') {
    for (const [k, v] of Object.entries(node as Record<string, unknown>)) {
      if (k === 'enum' && parentKey === 'absent' && Array.isArray(v)) out.push(...(v as string[]));
...
function scaffold(root: string) {
  for (const d of [
    'setup.d', 'skills', 'agents', '.claude/hooks', 'packages/core/templates/shared',
    '.claude/rules', 'scripts', 'packages/runtime-bridge/src/cli', 'plugin/hooks',
    'plugin/skills', 'plugin/commands', 'plugin/agents', 'plugin/.claude-plugin', '.claude/skills',
    'packages/core/manifest', 'tests/install-sh/baselines/ts-server',
  ]) mkdirSync(join(root, d), { recursive: true });
  writeFileSync(join(root, 'package.json'), JSON.stringify({ name: 'r', private: true }));
  writeFileSync(
    join(root, 'packages/core/manifest/rules-manifest.json'),
    JSON.stringify({ R1: { title: 'T', stack: ['ts-server'], check: { type: 'command', command: 'true' } } }),
  );
  writeFileSync(join(root, 'plugin/hooks/hooks.json'), JSON.stringify({ hooks: {} }));
  writeFileSync(join(root, 'plugin/.claude-plugin/plugin.json'), JSON.stringify({ version: '0.0.0' }));
...
describe('Principle 46 — D29 reference generator arms (spec §8)', () => {
  it('the generator exists and exposes the arm-testable surface', async () => {
    ...
    for (const fn of ['buildFamily', 'buildAllFamilies', 'familyDoc', 'run']) {
      expect(g[fn], `scripts/render-reference.mjs must export ${fn}`).toBeTypeOf('function');
    }
    expect(g.FAMILIES.map((f: { id: string }) => f.id).sort()).toEqual([...FAMILY_IDS].sort());
    ...
    expect(g.ABSENCE_REASONS.sort()).toEqual(['no-lane', 'no-operator-twin', 'not-byte-copied', 'unregistered']);
    ...
    for (const id of FAMILY_IDS) {
      const schema = JSON.parse(readFileSync(join(SCHEMA_DIR, `${id}.schema.json`), 'utf8'));
      for (const reason of absentEnums(schema)) {
        expect(g.ABSENCE_REASONS, `${id}.schema.json carries absence reason "${reason}" outside the closed enum`).toContain(reason);
      }
    }
  });
  ...
  it('arm A: reproduces the S0b gold cards byte-for-byte (vacuous until the fixtures land)', async () => {
    const g = await gen();
    const hasGold = existsSync(GOLD_FIXTURES) && readdirSync(GOLD_FIXTURES).length > 0;
    if (!hasGold) {
      ...
      expect.soft(hasGold, 'arm A dormant — fixtures/reference-gold/ empty until S0b').toBe(false);
      return;
    }
    for (const entry of readdirSync(GOLD_FIXTURES)) {
      ...
      const gold = readFileSync(join(GOLD_FIXTURES, entry, 'gold.fence'), 'utf8');
  ...
  it('arm B: every family doc validates against its draft-07 schema; no empty strings', async () => {
    ...
    for (const id of FAMILY_IDS) {
      ...
      expect(existsSync(schemaPath), `docs/site/reference/schema/${id}.schema.json must exist`).toBe(true);
      ...
      walk(doc, id);
      expect(empties, `family ${id} carries empty-string cells: ${empties.join(', ')}`).toEqual([]);
    }
  });
  ...
  it('arm C: per family, the §4 population predicate re-derived independently equals the member set (both directions)', async () => {
    const g = await gen();
    const families = g.buildAllFamilies(REPO_ROOT);
    const emitted = (id: string) => families[id].map((m: { name: string }) => m.name).sort();

    // A — setup.d/[0-9]*.sh
    const popA = readdirSync(join(REPO_ROOT, 'setup.d')).filter((f) => /^[0-9]+-.*\.sh$/.test(f)).sort();
    ...
    const opSkillDirs = (): string[] => {
      ...
      if (existsSync(join(REPO_ROOT, '.git'))) {
        const out = execFileSync('git', ['ls-files', '.claude/skills'], { cwd: REPO_ROOT, encoding: 'utf8' });
    ...
    expect(emitted('B'), 'B: population ↔ members (shipped ∪ operator, one row per name)').toEqual(popB);

    // C — agents minus the authoring-only skip-list, via the roster import (G12)
    const roster = await import(join(REPO_ROOT, 'scripts/render-install-roster.mjs'));
    const popC = roster.shippedAgents(REPO_ROOT).map((f: string) => f.replace(/\.md$/, '')).sort();
    expect(emitted('C'), 'C: population ↔ members (roster-imported shipped set)').toEqual(popC);
    ...
    const hooksJson = JSON.parse(readFileSync(join(REPO_ROOT, 'plugin/hooks/hooks.json'), 'utf8'));
    ...
    expect(emitted('D'), 'D: population ↔ members (.claude/hooks ∪ plugin-only registered)').toEqual(popD);
    ...
    const walkTpl = (d: string, pre: string) => {
      for (const e of readdirSync(d, { withFileTypes: true })) {
        ...
        else if (`${pre}${e.name}` !== 'templates.manifest.json') tpl.push(`${pre}${e.name}`);
      }
    };
    walkTpl(join(REPO_ROOT, 'packages/core/templates'), '');
    expect(emitted('E'), 'E: population ↔ members (all 38 template files)').toEqual(tpl.sort());
    ...
    const ruleFiles = readdirSync(join(REPO_ROOT, '.claude/rules'))
      .filter((f) => f.endsWith('.md') && f !== '00-rule-index.md').map((f) => f.replace(/\.md$/, ''));
    expect(emitted('F1'), 'F.1: population ↔ members (rules minus generated index)').toEqual(ruleFiles.sort());

    // F.2 — rules-manifest.json keys
    const manifest = JSON.parse(readFileSync(join(REPO_ROOT, 'packages/core/manifest/rules-manifest.json'), 'utf8'));
    expect(emitted('F2'), 'F.2: population ↔ members (manifest keys)').toEqual(Object.keys(manifest).sort());
    ...
    const shippedByPath = new Set(
      fingerprintRows().map((r) => r.path.split('/').pop()!).filter((b) => /\.(sh|mjs)$/.test(b)),
    );
    ...
    const popF3cards = allScripts
      .filter((s: { name: string }) => wired.has(s.name) && !s.testMaterial)
      .map((s: { name: string }) => s.name).sort();
    expect(emitted('F3'), 'F.3: cards == wired non-test scripts (cards, not the unwired list)').toEqual(popF3cards);
    const f3doc = g.familyDoc('F3', families.F3);
    const unwiredExpected = allScripts
      .filter((s: { name: string }) => !wired.has(s.name) && !s.testMaterial)
      .map((s: { name: string }) => s.name).sort();
    expect((f3doc.unwired ?? []).sort(), 'F.3: unwired[] == non-wired non-test scripts (§11 cleanup candidates)').toEqual(unwiredExpected);

    // G — packages/*/package.json
    const popG = readdirSync(join(REPO_ROOT, 'packages')).filter((d) =>
      existsSync(join(REPO_ROOT, 'packages', d, 'package.json'))).sort();
    expect(emitted('G'), 'G: population ↔ members (workspace packages)').toEqual(popG);
    ...
    const popH = readdirSync(join(REPO_ROOT, 'packages/runtime-bridge/src/cli'))
      .filter((f) => f.endsWith('.ts') && !H_ALLOW.includes(f)).map((f) => f.replace(/\.ts$/, '')).sort();
    expect(emitted('H'), 'H: population ↔ members (CLI commands minus plumbing allowlist)').toEqual(popH);

    // I — plugin skills ∪ commands ∪ agents (hooks belong to D, never double-rowed)
    const popI = [
      ...readdirSync(join(REPO_ROOT, 'plugin/skills')).filter((d) => existsSync(join(REPO_ROOT, 'plugin/skills', d, 'SKILL.md'))),
      ...readdirSync(join(REPO_ROOT, 'plugin/commands')).filter((f) => f.endsWith('.md')).map((f) => f.replace(/\.md$/, '')),
      ...readdirSync(join(REPO_ROOT, 'plugin/agents')).filter((f) => f.endsWith('.md')).map((f) => f.replace(/\.md$/, '')),
    ].sort();
    expect(emitted('I'), 'I: population ↔ members (plugin tree components)').toEqual(popI);
    ...
    for (const rel of tpl) {
      ...
      if (seen.has(h)) throw new Error(`template sha256 collision: ${seen.get(h)} and ${rel}`);
      seen.set(h, rel);
    }

    // Allowlisted plumbing paths exist (stale-pin visibility, §7 row 6)
    for (const p of [
      'plugin/hooks/_zcode-emit', 'plugin/hooks/run-hook.cmd', 'plugin/install/fetch-and-wire.sh',
      ...
      expect(existsSync(join(REPO_ROOT, p)), `allowlisted plumbing path must exist: ${p}`).toBe(true);
    }
  });
  ...
  it('arm C: absence tokens count per reason, printed and asserted; every token is enum-closed', async () => {
    ...
    const walk = (id: string, v: unknown) => {
      ...
      if (v && typeof v === 'object') {
        ...
        if (typeof o.absent === 'string') {
          expect(g.ABSENCE_REASONS, `family ${id}: token reason "${o.absent}" must be in the closed enum`)
            .toContain(o.absent);
    ...
    expect(counts['not-byte-copied'], 'E: rendered/merged templates carry not-byte-copied').toBeGreaterThan(0);
    expect(counts['no-operator-twin'], 'B: shipped skills without an operator twin carry no-operator-twin').toBeGreaterThanOrEqual(0);
    // Reason #4 is live today (the doctrine-recorded unregistered hooks, D event+matcher ×2):
    expect(counts['unregistered'], 'D: adopt-orchestrator-prompts + worktree-setup are deliberately unregistered').toBeGreaterThan(0);
  });
  ...
  it('arm D: a fixture with one missing field fails naming the file; the same fixture complete passes', async () => {
    ...
    const cases: { id: string; build: (root: string) => void }[] = [
      {
        ...
        build: (root) => {
          scaffold(root);
          writeFileSync(join(root, 'setup.d/10-skills.sh'), '#!/usr/bin/env bash\n# setup.d/10-skills.sh — copy the skill trees.\n');
          writeFileSync(join(root, 'setup.d/20-agents.sh'), '#!/usr/bin/env bash\nNO HEADER HERE\n');
        },
      },
      {
        id: 'B',
      ...
      },
      {
        ...
        build: (root) => {
          scaffold(root);
          writeFileSync(join(root, '.claude/hooks/my-hook.sh'), '#!/usr/bin/env bash\n# no grammar here\n');
        },
      },
      {
        id: 'E',
        build: (root) => {
          scaffold(root);
          writeFileSync(join(root, 'packages/core/templates/shared/x.toml'), 'k = 1\n');
          writeFileSync(join(root, 'packages/core/templates/templates.manifest.json'), JSON.stringify([
            { path: 'packages/core/templates/shared/OTHER.toml', description: 'present but for another file' },
          ]));
        },
      },
      {
        ...
        build: (root) => {
          ...
          writeFileSync(join(root, 'scripts/wired.sh'), '#!/usr/bin/env bash\necho hi\n');
          writeFileSync(join(root, 'package.json'), JSON.stringify({
            name: 'r', private: true, scripts: { w: 'bash scripts/wired.sh' },
          }));
        },
      },
      {
        ...
        build: (root) => {
          scaffold(root);
          writeFileSync(join(root, 'packages/core/package.json'), JSON.stringify({ name: '@r/core', private: true }));
        },
      },
      {
        id: 'H',
        build: (root) => {
          scaffold(root);
          writeFileSync(join(root, 'packages/runtime-bridge/src/cli/answer.ts'), '/**\n * no grammar here\n */\nexport {}\n');
        },
      },
    ];
    for (const c of cases) {
      ...
      c.build(bad);
      let msg = '';
      try { g.buildFamily(bad, c.id); } catch (e) { msg = (e as Error).message; }
      expect(msg, `family ${c.id}: absence-by-omission must fail the build`).toMatch(/—|missing|header|description/);
      expect(msg.length).toBeGreaterThan(0);
      expect(msg, `family ${c.id}: the error names a file`).toMatch(/\.(sh|md|json|toml|ts)/);
    }

    // The same fixtures WITH the field present pass.
    const good = fixtureRoot('positive');
    scaffold(good);
    writeFileSync(join(good, 'setup.d/10-skills.sh'), '#!/usr/bin/env bash\n# setup.d/10-skills.sh — copy the skill trees.\n');
    mkdirSync(join(good, 'skills/alpha'), { recursive: true });
    writeFileSync(join(good, 'skills/alpha/SKILL.md'), '---\nname: alpha\ndescription: Do the alpha thing end to end.\n---\n# alpha\n@harness-posture: portable\n');
    mkdirSync(join(good, '.claude/skills/alpha'), { recursive: true });
    writeFileSync(join(good, '.claude/skills/alpha/SKILL.md'), '---\nname: alpha\ndescription: Do the alpha thing end to end.\n---\n# alpha\n@harness-posture: portable\n');
    ...
    writeFileSync(join(good, '.claude/hooks/my-hook.sh'),
    ...
    writeFileSync(join(good, '.claude/settings.json'), JSON.stringify({
      hooks: { UserPromptSubmit: [{ hooks: [{ type: 'command', command: 'bash "$CLAUDE_PROJECT_DIR/.claude/hooks/my-hook.sh"' }] }] },
    }));
    writeFileSync(join(good, 'packages/core/templates/shared/x.toml'), 'k = 1\n');
    writeFileSync(join(good, 'packages/core/templates/templates.manifest.json'), JSON.stringify([
      { path: 'packages/core/templates/shared/x.toml', description: 'The x config template.' },
    ]));
    writeFileSync(join(good, 'scripts/wired.sh'), '#!/usr/bin/env bash\n# wired.sh — wire the thing on every push.\necho hi\n');
    writeFileSync(join(good, 'package.json'), JSON.stringify({
      name: 'r', private: true, scripts: { w: 'bash scripts/wired.sh' },
    }));
    writeFileSync(join(good, 'packages/core/package.json'), JSON.stringify({
      name: '@r/core', private: true, version: '0.0.0-fixture', description: 'The core package.',
    }));
    writeFileSync(join(good, 'packages/runtime-bridge/src/cli/answer.ts'), '/**\n * answer.ts — answer one open question from the queue.\n */\nexport {}\n');
    for (const id of ['A', 'B', 'D', 'E', 'F3', 'G', 'H'] as const) {
      expect(() => g.buildFamily(good, id), `family ${id}: the complete fixture passes`).not.toThrow();
    }
  });
  ...
  it('arm D: absence tokens are input-discriminated, not string-matched (G18/TD2-3)', async () => {
    ...
    const make = (withFingerprintLine: boolean, withTwin: boolean) => {
      ...
      if (withTwin) {
        ...
        writeFileSync(join(root, '.claude/skills/solo/SKILL.md'), '---\nname: solo\ndescription: A shipped skill with no operator twin.\n---\n# solo\n@harness-posture: portable\n');
      }
      const tplRel = 'packages/core/templates/shared/solo.toml';
      writeFileSync(join(root, tplRel), 'k = 1\n');
      writeFileSync(join(root, 'packages/core/templates/templates.manifest.json'), JSON.stringify([
        { path: tplRel, description: 'The solo template.' },
      ]));
      if (withFingerprintLine) {
        const h = sha256(join(root, tplRel));
        writeFileSync(join(root, 'tests/install-sh/baselines/ts-server/greenfield.fingerprint'), `${h}  .getff/solo.toml\n`);
      }
      return root;
    };
    ...
    const noFp = g.buildFamily(make(false, false), 'E').find((m: { name: string }) => m.name === 'shared/solo.toml');
    expect(noFp.installedTo).toEqual({ absent: 'not-byte-copied' });
    const withFp = g.buildFamily(make(true, false), 'E').find((m: { name: string }) => m.name === 'shared/solo.toml');
    ...
    const noTwin = g.buildFamily(make(false, false), 'B').find((m: { name: string }) => m.name === 'solo');
    expect(noTwin.extras['operator-twin']).toEqual({ absent: 'no-operator-twin' });
    const withTwin = g.buildFamily(make(false, true), 'B').find((m: { name: string }) => m.name === 'solo');
    expect(withTwin.extras['operator-twin']).not.toHaveProperty('absent');
    expect(String(withTwin.extras['operator-twin'])).toMatch(/\.claude\/skills\/solo\/SKILL\.md$/);
    ...
    writeFileSync(join(lanedRoot, 'tests/install-sh/baselines/ts-server/greenfield.fingerprint'),
      `${'0'.repeat(64)}  .claude/skills/solo/SKILL.md\n`);
    const laned = g.buildFamily(lanedRoot, 'B').find((m: { name: string }) => m.name === 'solo');
    ...
    scaffold(handTok);
    writeFileSync(join(handTok, 'packages/core/templates/shared/h.toml'), 'k = 1\n');
    writeFileSync(join(handTok, 'packages/core/templates/templates.manifest.json'), JSON.stringify([
      { path: 'packages/core/templates/shared/h.toml', description: 'x', installedTo: { absent: 'made-up-reason' } },
    ]));
    let handMsg = '';
    try { g.buildFamily(handTok, 'E'); } catch (e) { handMsg = (e as Error).message; }
    expect(handMsg, 'a hand-declared installedTo must fail the build naming the manifest').toContain('templates.manifest.json');
    expect(handMsg, 'the failure names the offending key').toContain('installedTo');
    ...
    const ajv = new Ajv({ allErrors: true, strict: false });
    const validate = ajv.compile(JSON.parse(readFileSync(join(SCHEMA_DIR, 'E.schema.json'), 'utf8')));
    const badDoc = {
      schemaVersion: 1, family: 'E', familyName: 'templates', generator: 'scripts/render-reference.mjs',
      members: [{
        name: 'shared/h.toml', kind: 'template', shipsTo: { tier: 'framework' },
        description: 'x', source: { path: 'packages/core/templates/templates.manifest.json', line: 1 },
        installedTo: { absent: 'made-up-reason' }, extras: { format: 'toml' }, example: null,
      }],
    };
    expect(validate(badDoc), 'a hand-typed token must fail the schema, not pass through').toBe(false);
    ...
    const dMake = (registered: boolean, marker: boolean) => {
      ...
      scaffold(root);
      const lines = ['#!/usr/bin/env bash', '# d-hook.sh — emit the fixture session start.'];
      if (marker) lines.push('# @cc-only-rationale: fixture hook, deliberately unregistered by default.');
      lines.push('');
      writeFileSync(join(root, '.claude/hooks/d-hook.sh'), `${lines.join('\n')}\n`);
      if (registered) {
        writeFileSync(join(root, '.claude/settings.json'), JSON.stringify({
          hooks: { SessionStart: [{ hooks: [{ type: 'command', command: 'bash "$CLAUDE_PROJECT_DIR/.claude/hooks/d-hook.sh"' }] }] },
        }));
      }
      return root;
    };
    const unreg = g.buildFamily(dMake(false, true), 'D').find((m: { name: string }) => m.name === 'd-hook');
    expect(unreg.extras.event, 'an unregistered hook carries the unregistered token, not an event list')
      .toEqual({ absent: 'unregistered' });
    expect(unreg.extras.matcher).toEqual({ absent: 'unregistered' });
    const regd = g.buildFamily(dMake(true, true), 'D').find((m: { name: string }) => m.name === 'd-hook');
    expect(regd.extras.event, 'registering the same hook flips the token back to the event value').toEqual(['SessionStart']);
    expect(regd.extras.matcher).toEqual([]);
    let dNeg = '';
    try { g.buildFamily(dMake(false, false), 'D'); } catch (e) { dNeg = (e as Error).message; }
    expect(dNeg, 'unregistered AND marker-less is an omission — the build fails naming the file').toContain('.claude/hooks/d-hook.sh');
    ...
    const dValidate = new Ajv({ allErrors: true, strict: false })
      .compile(JSON.parse(readFileSync(join(SCHEMA_DIR, 'D.schema.json'), 'utf8')));
    const dMember = {
      name: 'd-hook', kind: 'hook', shipsTo: { tier: 'framework' }, description: 'emit the fixture session start.',
      source: { path: '.claude/hooks/d-hook.sh', line: 2 },
      extras: { event: { absent: 'no-lane' }, matcher: [], delivery: ['@cc-only-rationale'] }, example: null,
    };
    expect(dValidate({
      schemaVersion: 1, family: 'D', familyName: 'hooks', generator: 'scripts/render-reference.mjs', members: [dMember],
    }), 'a borrowed reason on extras.event must fail the D schema').toBe(false);
    expect(dValidate({
      schemaVersion: 1, family: 'D', familyName: 'hooks', generator: 'scripts/render-reference.mjs',
      members: [{ ...dMember, extras: { event: { absent: 'unregistered' }, matcher: [], delivery: ['@cc-only-rationale'] } }],
    }), 'the unregistered token itself validates on extras.event').toBe(true);
  });
  ...
  it('arm E: reference F.1 rows are identical to the 00-rule-index.md rows', async () => {
    const g = await gen();
    const families = g.buildAllFamilies(REPO_ROOT);
    const index = readFileSync(join(REPO_ROOT, '.claude/rules/00-rule-index.md'), 'utf8');
    const indexRows = new Map<string, { cls: string; fires: string; channels: string }>();
    for (const m of index.matchAll(/^\| `([^`]+\.md)` \| ([ABC]) \| (.+?) \| (.+?) \|$/gm)) {
      indexRows.set(m[1].replace(/\.md$/, ''), { cls: m[2], fires: m[3], channels: m[4] });
    }
    expect(indexRows.size, 'the committed index must be non-empty for this arm to mean anything').toBeGreaterThan(20);
    for (const m of families.F1) {
      ...
      expect(m.extras.channels, `F.1 ${m.name} channels parity`).toBe(row!.channels);
    }
  ...
  it('arm F: every wired script, layer and CLI command carries its per-glob header; escapes only where allowed', async () => {
    ...
    expect(g.HEADER_TABLE, 'the header table must be exported for the shared-gate contract').toBeTypeOf('object');
    ...
    const src = (name: string) => readFileSync(join(REPO_ROOT, 'scripts', name), 'utf8');
    const headerOk = (name: string) => {
      const body = src(name);
      if (name.endsWith('.sh')) {
        return new RegExp(`^# ${name} — .{10,}$`, 'm').test(body.split('\n').slice(1, 3).join('\n'));
      }
      const lines = body.split('\n');
      return lines[1] === '/**' && new RegExp(`^ \\* ${name.replace(/\.mjs$/, '')} — .{10,}$`).test(lines[2] ?? '');
    };
    ...
    for (const s of scripts) {
      ...
      if (isWired && !headerOk(s.name)) violations.push(`wired script lacks header: scripts/${s.name}`);
      if (!isWired && !headerOk(s.name)) {
        const escaped = src(s.name).match(/^# getff:no-card — (.+)$/m);
        if (!escaped || escaped[1].trim().length < 20) {
          violations.push(`unwired script lacks header AND a ≥20-char escape token: scripts/${s.name}`);
        }
      }
    }
    expect(
  ...
  it('arm G: one real execution shape per kind DOES wire — command position, verb, quoted path token, quoted assignment, import closure', async () => {
    ...
    const hitsOf = (text: string) => {
      ...
      censusMod.scanShellText(text, (n: string, l: number, s: string) => hits.push({ name: n, shape: s }), known);
      return hits;
    };
    ...
    expect(hitsOf('bash scripts/target.sh\n')).toEqual([{ name: 'target.sh', shape: 'invocation-verb' }]);
    // invocation verb, second member of the verb set
    expect(hitsOf('exec scripts/target.sh --flag\n').map((h) => h.shape)).toEqual(['invocation-verb']);
    // command substitution + for-in list
    expect(hitsOf('V="$( node scripts/target.sh )"\n').map((h) => h.shape)).toEqual(['command-substitution']);
    expect(hitsOf('for f in scripts/target.sh; do :; done\n').map((h) => h.shape)).toEqual(['for-in-list']);
    // S9/Q2: the quote opens AT the path token
    expect(hitsOf('run "$ROOT/scripts/target.sh"\n').map((h) => h.name)).toEqual(['target.sh']);
    ...
    expect(hitsOf('PARITY="$ROOT/scripts/target.sh"\n').map((h) => h.name)).toEqual(['target.sh']);
    ...
    scaffold(root);
    writeFileSync(join(root, 'scripts/wired.mjs'), '#!/usr/bin/env node\n/**\n * wired.mjs — wire the closure fixture.\n */\nimport { x } from "./census-like.mjs";\nx();\n');
    writeFileSync(join(root, 'scripts/census-like.mjs'), '#!/usr/bin/env node\n/**\n * census-like.mjs — the imported module the closure must reach.\n */\nexport const x = 1;\n');
    writeFileSync(join(root, 'package.json'), JSON.stringify({ name: 'r', private: true, scripts: { w: 'node scripts/wired.mjs' } }));
    const wired = censusMod.computeWiredSet(root, []);
    expect(wired.has('wired.mjs'), 'the entry script is wired via package.json').toBe(true);
    expect(wired.has('census-like.mjs'), 'closure reaches ES-imported non-test scripts (S6)').toBe(true);
    expect(wired.get('census-like.mjs')?.some((e: { via: string }) => e.via === 'closure')).toBe(true);
  });

  it('arm G: the real advisory block at check-hook-marker.sh wires render-harness-config NOTHING (live Q1 proof)', async () => {
    const censusMod = await censusModule();
    const marker = readFileSync(join(REPO_ROOT, '.claude/hooks/check-hook-marker.sh'), 'utf8');
    const known = new Set(censusMod.listScripts(REPO_ROOT).map((s: { name: string }) => s.name));
    const hits: string[] = [];
