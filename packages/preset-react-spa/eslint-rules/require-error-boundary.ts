...
export const requireErrorBoundary = createRule({
  ...
  meta: {
    ...
    messages: {
      missingErrorBoundary:
        'App-root component must render its content inside an ErrorBoundary JSX element (R-SPA-EB). Add <ErrorBoundary> as an ancestor in the JSX tree. To opt out intentionally, add // audit:exempt on the same line as the first JSX element.',
    },
    schema: [],
  },
  defaultOptions: [],
  create(context) {
    ...
    let firstJSXNode: TSESTree.JSXOpeningElement | null = null;
    ...
    return {
      ...
      'Program:exit'(program: TSESTree.Program) {
        ...
        const reportNode: TSESTree.Node = firstJSXNode ?? program;
