...
- After the consumer confirms the paste → run `bash /tmp/getff/scripts/getff-glm-onebutton.sh provision`. The helper provisions the runtime profile, sets per-mode defaults via the aif project config, prints a key-reachability wiring instruction (adding the env file to the aif deployment's env_file list — §7b), and validates the route end-to-end via the aif bridge. Report the `GLM_PROVISION: DONE` or `GLM_PROVISION: FAILED` output verbatim. If the helper prints the key-reachability wiring instruction, read it aloud to the consumer — they must apply it before validation can succeed.
