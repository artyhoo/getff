...
_go_deliver_ci() {
  ...
  if [ -e "$wf_dst" ]; then
    if grep -q 'generated by getff' "$wf_dst" 2>/dev/null; then
      if [ "${GETFF_TOOLCHAIN_REFRESH:-}" = "1" ]; then
        ...
        # preserving the getff-go.yml.override.md Layer-3 escape hatch.
  ...
  # triggers on the right branch. A plain copy_safe here shipped a getff-go.yml that a
  # non-`main` consumer installs and that then NEVER RUNS — the exact silent-signal defect
  # S4 removed on the python + cargo lanes while the go lane kept the pre-S4 shape.
  # Byte-identical to template when the default IS main or when detection fails (PARK
  # Option A); see helper docstring in setup.d/lib.sh.
