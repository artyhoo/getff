...
#   (rules-lock) staleness ledger         → ALWAYS write .ai-factory/synthesizer-output/rules-lock.cargo.json
#                                           (emittedAt + sourceFingerprint of the delivered clippy.toml) —
#                                           the cargo variant of the rules-lock the deps-hash staleness
#                                           router globs (rules-lock*.json) and the rule-tests skill reads.
...
# .ai-factory/synthesizer-output/rules-lock.cargo.json recording the delivered ruleset's emittedAt +
# sourceFingerprint (sha256 of the delivered clippy.toml). This is the cargo sibling of the JS/python
# rules-lock the deps-hash staleness router globs (`.ai-factory/synthesizer-output/rules-lock*.json`,
# rule-tests-surface spec §6) and the rule-tests skill reads. Framework-owned, refresh-overwritten.
_cargo_write_rules_lock() {
  ...
  local clippy="$PROJECT_ROOT/clippy.toml"
  [ -e "$clippy" ] || clippy="$PROJECT_ROOT/getff-clippy.toml"
  ...
  cat > "$lock" <<EOF
{
  ...
  "note": "getff cargo lane delivery lock (ecosystem-wiring W4). sourceFingerprint hashes the delivered clippy.toml; deps-hash staleness routing globs rules-lock*.json, the rule-tests skill reads emittedAt/sourceFingerprint."
}
EOF
  _cargo_log "cargo rules-lock → .ai-factory/synthesizer-output/rules-lock.cargo.json (staleness ledger)"
}
...
# binding STOP line), runs `cargo clippy --message-format=json` with the DELIVERED clippy.toml, and
...
_cargo_firing_self_check() {
  echo ""
  echo "▶ getff firing self-check — proving the delivered clippy.toml FIRES (planted violation in an OS temp dir)"
  local _pass=0 _silent=0 _degraded=0
  local _clippy="$PROJECT_ROOT/clippy.toml"
  [ -e "$_clippy" ] || _clippy="$PROJECT_ROOT/getff-clippy.toml"
  ...
  if command -v cargo >/dev/null 2>&1 && [ -e "$_clippy" ]; then
    ...
    if printf '%s' "$_out" | grep -q '"clippy::disallowed_methods"'; then
      ...
      echo "  ✗ cargo clippy did NOT fire on a planted violation — the delivered clippy.toml is SILENT (delivery bug)"
      _silent=$((_silent+1))
    fi
    rm -rf "$_t"
  else
    echo "  ⚠ cargo not on PATH (or clippy.toml missing) — firing NOT proven (degrade, NOT green). Verify manually from your crate root:"
  ...
  if [ "$_silent" -gt 0 ]; then
    echo "⚠  getff self-check: $_pass fired · $_silent SILENT — the delivered clippy.toml did NOT fire on bad input; review above before relying on it."
    ...
    echo "✓ getff self-check: the delivered clippy.toml fired RED on a planted violation — enforcement is live."
  fi
  return 0
}
