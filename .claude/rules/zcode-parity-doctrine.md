...
| 3 | `check-doc-authority-header` | PostToolUse:Edit\|Write\|MultiEdit | works; consumer install via `setup.d/10-skills.sh:380-390` AND plugin twin shipped (zcode-plugin-firstclass Stage 2, 2026-09-12 — verified live: `ls plugin/hooks/ \| grep -x check-doc-authority-header` → the file, generated identity-mode; registered plugin-internal in `PLUGIN_INTERNAL_HOOKS` because the framework must not run the consumer reimplementation on itself) → reachable on ZCode via plugin channel | `parity` |
...
| 13 | `inject-project-digest` | UserPromptSubmit **AND** SubagentStart | mixed: UserPromptSubmit arm works — **LIVE-VERIFIED end-to-end 2026-09-10** (ZCode probe: the digest block was delivered as injected `additionalContext` into the probing session's next prompt, content observed verbatim — Project/Repo map/Hard pointers lines; evidence: #1703 comment 2026-09-10; supersedes the live-inert baseline recorded at [`2026-09-10-zcode-live-smoke.md:138-140`](../../docs/meta-factory/research-patches/2026-09-10-zcode-live-smoke.md)); SubagentStart arm impossible (event ∉ `ZCODE_EVENTS`); fallback = row 15 | `zcode-gap` |
| 14 | `inject-session-bootstrap` | UserPromptSubmit | works | `parity` |
| 15 | `inject-subagent-context` | PreToolUse:Agent\|Task | works (Agent aliases to Task on ZCode) — **LIVE-VERIFIED end-to-end 2026-09-10** (ZCode probe: a live-dispatched subagent received the augmentation — anchor present after the `---` delimiter below the task prompt, i.e. dispatch → hook → updatedInput → subagent context, all observed; evidence: #1703 comment 2026-09-10; supersedes the binary-verified-only reading recorded at [`2026-09-10-zcode-live-smoke.md:141-142`](../../docs/meta-factory/research-patches/2026-09-10-zcode-live-smoke.md)); cwd nuance: which tree feeds the anchor depends on where the session's cwd sits — the injection source followed the session shell's cwd (the worktree carrying the digest block), not a fixed project root (the main checkout's `session-bootstrap.md` lacked the block at probe time; how ZCode derives the hook env is unverified — `CLAUDE_PROJECT_DIR`-from-cwd is the hypothesis; #1703 comment); documented role = ZCode's SubagentStart fallback | `parity` (with role annotation) |
...
| 21 | `precompact-residue` | PreCompact | impossible (event ∉ `ZCODE_EVENTS`); ZCode shipped compaction as a turn-kind (S-verify, 2026-09-10) but exposes no compaction-lifecycle hook of any kind — PreCompact is absent from the runtime binary itself — so there is no degraded arm to ship, not even an inert one (S2b, 2026-08-17; S-verify, 2026-09-10) | `cc-only` |
| 22 | `inject-handoff-on-compact` | SessionStart (matcher `compact`) | inert (event ∈ `ZCODE_EVENTS`, unlike row 21 — but the installed runtime dispatches SessionStart only with `startup`\|`resume`, so `source=compact` never fires even though compaction itself now exists; the bundled zcode-guide doc listing `compact` as a match value is ahead of its own runtime — verify in the binary, not the doc (S-verify, 2026-09-10); and the hook is deliberately absent from `plugin/hooks/`, the only channel that reaches ZCode: consumers receive no residue writer, so there would be no file to inject, spec D20, 2026-09-08) | `cc-only` |
| 23 | `glossary-inject` | UserPromptSubmit | works (event ∈ `ZCODE_EVENTS`; `_emit_ctx` strict-JSON branch) — **dormant by default on BOTH channels by design**: no `hooks.json` entry ships, no registration is automatic, and the operator hand-action `scripts/register-glossary-hook.sh` IS the delivery (`--user` default → USER settings.json, the one registration every session on the machine reads; `--project` → the SSOT + renderer `--write`, which materializes `plugin/hooks/hooks.json`) — the WorktreeCreate row-20 anti-pattern is the named alternative shape (plain-words-recap-v2 S3, 2026-09-14) | `parity` (dormant-by-default symmetric) |

**Classification rollup:** `parity` (strict) = 12 rows (2, 3, 5, 6, 8, 10, 11, 12, 14, 18, 19, 23); `parity` with role annotation = 1 (15); `framework-internal` = 2 (1, 4); `plugin-gap` = 0 (row 3 flipped by zcode-plugin-firstclass Stage 2 — the class is empty); `zcode-gap` = 4 (7, 9, 13, 17 — row 9 flipped by the D7 context-arm per the row-13 one-working-arm precedent); `cc-only` = 4 (16, 20, 21, 22). Total = 23 = `ls .claude/hooks/*.sh \| wc -l` (census baseline at [census.md §Population enumeration](../../docs/meta-factory/research-patches/2026-07-18-zcode-full-parity-census.md)); Wave B (#1043/#1044/#1046/#1047) flipped rows 12/19 since census time; row 3 flipped to `parity` by zcode-plugin-firstclass Stage 2 (2026-09-12) — Stage 6 had shipped row 4's `check-doc-authority` twin, NOT row 3's `check-doc-authority-header`; row 23 added by plain-words-recap-v2 S3 (2026-09-14) as `parity` with dormancy symmetric on both channels.

**Digest-anchor delivery asymmetry — ACCEPTED DIVERGENCE (decision D7, #1703 ZCode half, 2026-09-15):** the CC subagent anchor (`inject-subagent-digest`, row 16 → SubagentStart) delivers the INLINE bootstrap digest; the ZCode anchor (`inject-subagent-context`, row 15 → PreToolUse:Agent|Task) delivers the digest BLOCK. Decision: accept the divergence — both arms are live-verified end-to-end on their own harness (rows 13/15 annotations above; #1703 comments 2026-09-10 and 2026-09-15), and unifying the delivery shapes is new mechanism for zero consumer-visible gain. This records the decision that [`2026-08-02-per-role-digest-fork.md:203`](../../docs/meta-factory/research-patches/2026-08-02-per-role-digest-fork.md) deliberately left open (a recommendation-free fork whose design space is bisected by exactly this CC/ZCode portability asymmetry). *Reopen-if:* a future harness arm makes inline delivery cheap — revisit unification then. The CC-half verification checklist for #1703 stays tracked in-issue.
...
**Row 16 — `inject-subagent-digest` (SubagentStart):** CC dogfood-only; SubagentStart is inexpressible on ZCode (`ZCODE_EVENTS` excludes it). The role is replaced on ZCode by row 15 `inject-subagent-context` via the [`render-harness-config.mjs:265-266`](../../scripts/render-harness-config.mjs) backup path — `PreToolUse:Agent+updatedInput` delivers the digest one-shot as the subagent's first message, NOT persistent-lifecycle as on CC. **Stage 7B (#1047)** upgraded this fallback from minimal backup to full payload parity. **Status:** parity via `PreToolUse:Agent+updatedInput` full-payload delivery.

**Row 19 — `warn-subagent-report` (SubagentStop):** declared CC-only at step-1 patch §"Bespoke #2 — REJECTED" ([`2026-07-18-zcode-parity-step1.md`](../../docs/meta-factory/research-patches/2026-07-18-zcode-parity-step1.md)). **Stage 5 (4D hybrid, #1046)** closed this: the Stop hook arm receives `transcript_path` (R3 verified) and reads the finished report via rollout scan; PostToolUse:Agent arm reads the 120KB payload directly (R4 verified — ZCode is *better* than CC's 4KB). **Status:** `parity via 4D hybrid (#1046)`. The runtime loud-declaration in [`scripts/render-harness-config.mjs:262-263`](../../scripts/render-harness-config.mjs) states the 4D-hybrid parity (`parity via 4D hybrid (#1046): warn-subagent-report-zcode arms …`) since D3 was resolved (2026-09-11, §3 header note) — rendered from the `backup` map, so the declaration and this row state the same fact.

**Row 21 — `precompact-residue` (PreCompact):** the hook writes a per-session residue file before the context window is compacted (S2b of [the pipeline-chips ADR](../../docs/superpowers/specs/2026-08-09-pipeline-chips-session-bus-design.md) §D8). `PreCompact` is absent from `ZCODE_EVENTS`, and unlike rows 13/16 there is no adjacent event to relay through: ZCode's expressible set contains no compaction-lifecycle hook of any kind, so the capability has no degraded arm — not a weaker one, none. Compaction the FEATURE shipped on ZCode as a turn-kind with `manual|auto` origin and a `/compact` command — but it fires no hook: `PreCompact` has zero occurrences in the runtime bundle itself ([S-verify 2026-09-10](../../docs/meta-factory/research-patches/2026-09-10-zcode-compaction-hook-verification.md) §2–§3), so the feature arriving did not reopen this row. Declared rather than assumed: [`scripts/render-harness-config.mjs`](../../scripts/render-harness-config.mjs) `emitPlugin` used to `continue` silently past every off-set event; S2b made that skip a loud note naming each dropped event, so the next hook registered on an inexpressible event surfaces at render time instead of vanishing. **Status:** accepted-degradation; revisit when a PreCompact-class event appears in the runtime enum — probe the binary (`grep -ac PreCompact zcode.cjs`), not the vendor doc (S-verify 2026-09-10 §6).

**Row 22 — `inject-handoff-on-compact` (SessionStart, matcher `compact`):** the reader half of the handoff-currency pipe — it feeds the residue back as `additionalContext` on the post-compaction session (spec D20 of the [handoff-currency-gate design](../../docs/superpowers/specs/2026-09-08-handoff-currency-gate-design.md)). Its event IS expressible on ZCode, which is what separates it from row 21: the gap is not the event but the pipe. Compaction exists on ZCode now, but `source=compact` still never arrives: the installed runtime's single SessionStart dispatch site (`runSessionStartHooks`) is called only with `startup` and `resume` ([S-verify 2026-09-10](../../docs/meta-factory/research-patches/2026-09-10-zcode-compaction-hook-verification.md) §3.2), and the bundled zcode-guide doc's `compact` match-value claim has no dispatch site behind it — doc-ahead-of-runtime; verify in the binary, not the doc. The hook is also deliberately not twinned into `plugin/hooks/` — the plugin `session-start` slot is already held by the bootstrap block, and consumers receive no PreCompact writer (row 21), so an injector there would read a file nothing writes. [`scripts/render-harness-config.mjs`](../../scripts/render-harness-config.mjs) declares that skip loudly at render time via its `PLUGIN_INCOMPATIBLE` map rather than dropping it silently. **Status:** accepted-degradation; revisit together with row 21 when the runtime actually dispatches `source=compact` (probe: a `runSessionStartHooks("compact"` call site in the binary) — the writer must land first, or the reader has nothing to read.
...
**Row 12 — `inject-output-language` (`plugin-gap` closed):** CC ships via `setup.d/10-skills.sh` `register_cc_hook`; ZCode reaches hooks ONLY via the plugin channel ([`render-harness-config.mjs:230-231`](../../scripts/render-harness-config.mjs)). **Stage 6 (#1043)** shipped the plugin twin — verified live: `ls plugin/hooks/inject-output-language` returns the file. Row 12 is load-bearing for [language-discipline.md §2](language-discipline.md). Classification is now `parity` (§2 census).

**Row 3 — `check-doc-authority-header` (`plugin-gap` CLOSED):** Stage 6 shipped row 4's twin (`plugin/hooks/check-doc-authority`), NOT row 3's `check-doc-authority-header` — leaving row 3 the one retained `plugin-gap`. **zcode-plugin-firstclass Stage 2 (2026-09-12)** closed it: the twin ships at `plugin/hooks/check-doc-authority-header` (generated identity-mode via `scripts/generate-plugin-twins.sh` from the unchanged consumer-shippable source, per its `@cc-only-rationale` marker), and the registration lives in `PLUGIN_INTERNAL_HOOKS` ([`scripts/render-harness-config.mjs`](../../scripts/render-harness-config.mjs), PostToolUse `Edit|Write|MultiEdit`) rather than the framework model — the framework dogfoods `check-doc-authority` and must not run the consumer reimplementation on itself. `POST_MUTATION_GATES` grew 4→5 in the same commit so the renderer's advisory-only declaration stays truthful. **Status:** `parity` via plugin channel + consumer install (`setup.d/10-skills.sh:380-390`).
...
|---|---|---|---|
| **Supported today** | Claude Code | Primary dogfood harness; deepest coverage (all 23 hooks; principle tests + 4-layer enforcement) | [README.md#why-this-exists](../../README.md); §2 census |
| **Supported today** | ZCode | Full parity via plugin channel + `_zcode-emit` helper; 18 framework hook twins shipped (re-counted live: `ls plugin/hooks/ \| grep -vE '^_zcode-emit|^hooks.json|^lang|^lib|^run-hook.cmd|^session-start' \| wc -l` → 18, post-plain-words-recap-v2 S3 (2026-09-14); 17 at post-zcode-plugin-firstclass Stage 2, when the `^lib` exclusion was added — `lib/` (#1628) is shared helper code, not a twin, and without it that command over-counted, the latent drift the recount corrects). Four rows remain `cc-only` (16, 20, 21, 22); the four CC-only *events* (`SubagentStart`/`SubagentStop`/`WorktreeCreate`/`PreCompact`) are still inexpressible on ZCode, but SubagentStop now has a parity variant via row 19's 4D hybrid (Stage 5, #1046) — accepted-degradation rationale for rows 16/20/21/22 per §4 | §2 census; [`scripts/render-harness-config.mjs:46-54`](../../scripts/render-harness-config.mjs); step-1 patch #1031 |
...
- **Renderer SSOT:** [`scripts/render-harness-config.mjs`](../../scripts/render-harness-config.mjs) — `ZCODE_EVENTS` (lines 46-54), `ZCODE_UNSUPPORTED_TOOLS` (line 63), backup-path loud-declarations (lines 257-269).
- **Runtime-probe gate (Fork B, 2026-09-11):** [`scripts/probe-zcode-runtime.sh`](../../scripts/probe-zcode-runtime.sh) + `packages/core/hooks/zcode-runtime-probe.test.ts` — the parity SSOT's binary claims (event enum, trust-channel markers, hook I/O literals, plugin compatibility) asserted against the INSTALLED bundle; skips loudly where the app is absent. Drift fails here before any doc is trusted — the executable form of §4 Row 21's «probe the binary, not the doc».
...
- [`scripts/render-harness-config.mjs`](../../scripts/render-harness-config.mjs): §4 + §6 cite its `ZCODE_EVENTS` SSOT (lines 46-54), `ZCODE_UNSUPPORTED_TOOLS` (line 63), backup-path loud-declarations (lines 257-269). Deliverable D3 (loud-declaration sync) resolved 2026-09-11 (see §3 header note) — line refs re-verified against the file after the Stage-2 renderer insertion (2026-09-12). ✓
