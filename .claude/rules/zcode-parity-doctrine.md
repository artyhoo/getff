...
| 21 | `precompact-residue` | PreCompact | impossible (event ∉ `ZCODE_EVENTS`); ZCode has no compaction-lifecycle event at all, so there is no degraded arm to ship — not even an inert one (S2b, 2026-08-17) | `cc-only` |
| 22 | `inject-handoff-on-compact` | SessionStart (matcher `compact`) | inert (event ∈ `ZCODE_EVENTS`, unlike row 21 — but ZCode has no compaction, so `source=compact` never fires; and the hook is deliberately absent from `plugin/hooks/`, the only channel that reaches ZCode: consumers receive no residue writer, so there would be no file to inject, spec D20, 2026-09-08) | `cc-only` |
...
**Row 21 — `precompact-residue` (PreCompact):** the hook writes a per-session residue file before the context window is compacted (S2b of [the pipeline-chips ADR](../../docs/superpowers/specs/2026-08-09-pipeline-chips-session-bus-design.md) §D8). `PreCompact` is absent from `ZCODE_EVENTS`, and unlike rows 13/16 there is no adjacent event to relay through: ZCode's expressible set contains no compaction-lifecycle hook of any kind, so the capability has no degraded arm — not a weaker one, none. Declared rather than assumed: [`scripts/render-harness-config.mjs`](../../scripts/render-harness-config.mjs) `emitPlugin` used to `continue` silently past every off-set event; S2b made that skip a loud note naming each dropped event, so the next hook registered on an inexpressible event surfaces at render time instead of vanishing. **Status:** accepted-degradation; revisit if ZCode ships a compaction event.

**Row 22 — `inject-handoff-on-compact` (SessionStart, matcher `compact`):** the reader half of the handoff-currency pipe — it feeds the residue back as `additionalContext` on the post-compaction session (spec D20 of the [handoff-currency-gate design](../../docs/superpowers/specs/2026-09-08-handoff-currency-gate-design.md)). Its event IS expressible on ZCode, which is what separates it from row 21: the gap is not the event but the pipe. ZCode never compacts, so `source=compact` never arrives; and the hook is deliberately not twinned into `plugin/hooks/` — the plugin `session-start` slot is already held by the bootstrap block, and consumers receive no PreCompact writer (row 21), so an injector there would read a file nothing writes. [`scripts/render-harness-config.mjs`](../../scripts/render-harness-config.mjs) declares that skip loudly at render time via its `PLUGIN_INCOMPATIBLE` map rather than dropping it silently. **Status:** accepted-degradation; revisit together with row 21 if ZCode ships a compaction event — the writer must land first, or the reader has nothing to read.
...
| **Supported today** | ZCode | Full parity via plugin channel + `_zcode-emit` helper; 16 framework hook twins shipped (re-counted live: `ls plugin/hooks/ \| grep -vE '^_zcode-emit|^hooks.json|^lang|^run-hook.cmd|^session-start' \| wc -l` → 16, post-Stage 6). Three rows remain `cc-only` (16, 20, 21); the four CC-only *events* (`SubagentStart`/`SubagentStop`/`WorktreeCreate`/`PreCompact`) are still inexpressible on ZCode, but SubagentStop now has a parity variant via row 19's 4D hybrid (Stage 5, #1046) — accepted-degradation rationale for rows 16/20/21 per §4 | §2 census; [`scripts/render-harness-config.mjs:46-54`](../../scripts/render-harness-config.mjs); step-1 patch #1031 |
