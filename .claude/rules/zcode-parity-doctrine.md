...
**Classification rollup:** `parity` (strict) = 11 rows (2, 3, 5, 6, 8, 10, 11, 12, 14, 18, 19); `parity` with role annotation = 1 (15); `framework-internal` = 2 (1, 4); `plugin-gap` = 0 (row 3 flipped by zcode-plugin-firstclass Stage 2 — the class is empty); `zcode-gap` = 4 (7, 9, 13, 17 — row 9 flipped by the D7 context-arm per the row-13 one-working-arm precedent); `cc-only` = 4 (16, 20, 21, 22). Total = 22 = `ls .claude/hooks/*.sh \| wc -l` (census baseline at [census.md §Population enumeration](../../docs/meta-factory/research-patches/2026-07-18-zcode-full-parity-census.md)); Wave B (#1043/#1044/#1046/#1047) flipped rows 12/19 since census time; row 3 flipped to `parity` by zcode-plugin-firstclass Stage 2 (2026-09-12) — Stage 6 had shipped row 4's `check-doc-authority` twin, NOT row 3's `check-doc-authority-header`.
...
|---|---|---|---|
| **Supported today** | Claude Code | Primary dogfood harness; deepest coverage (all 22 hooks; principle tests + 4-layer enforcement) | [README.md#why-this-exists](../../README.md); §2 census |
| **Supported today** | ZCode | Full parity via plugin channel + `_zcode-emit` helper; 17 framework hook twins shipped (re-counted live: `ls plugin/hooks/ \| grep -vE '^_zcode-emit|^hooks.json|^lang|^lib|^run-hook.cmd|^session-start' \| wc -l` → 17, post-zcode-plugin-firstclass Stage 2; the `^lib` exclusion was added in that commit — `lib/` (#1628) is shared helper code, not a twin, and without it this command printed 17 on a 16-twin tree, the latent drift the recount corrects). Four rows remain `cc-only` (16, 20, 21, 22); the four CC-only *events* (`SubagentStart`/`SubagentStop`/`WorktreeCreate`/`PreCompact`) are still inexpressible on ZCode, but SubagentStop now has a parity variant via row 19's 4D hybrid (Stage 5, #1046) — accepted-degradation rationale for rows 16/20/21/22 per §4 | §2 census; [`scripts/render-harness-config.mjs:46-54`](../../scripts/render-harness-config.mjs); step-1 patch #1031 |
