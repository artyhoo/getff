...
> **Fires:** a per-backend toolchain-freshness gate goes RED (`checkToolchainFreshness` — the committed evidence `toolchain` string ≠ the version resolving at test time), OR a rendered-not-fired matrix cell needs its first live-fired evidence.
