...
> **Class:** A — the §5 drift-check + §6 marker-presence sketches ship as an executable check: [tests/agnosticism/probes/channel-coverage.sh](../../tests/agnosticism/probes/channel-coverage.sh) (Surface 8, CI-run by [packages/core/principles/21-agnosticism-conformance.test.ts](../../packages/core/principles/21-agnosticism-conformance.test.ts); seeded-break paired-negative in [tests/agnosticism/harness-self.test.sh](../../tests/agnosticism/harness-self.test.sh)) + the edit-time gate [.claude/hooks/check-hook-marker.sh](../hooks/check-hook-marker.sh) (§6, PostToolUse). Of the §8 *semantic* anti-patterns, `#brand-name-detection` is **gated** — [tests/agnosticism/probes/brand-detection.sh](../../tests/agnosticism/probes/brand-detection.sh) (Surface 10 of principle 21; both-direction paired-negative in [tests/agnosticism/harness-self.test.sh](../../tests/agnosticism/harness-self.test.sh)) — and `#two-prompts-drift` + `#sync-by-copy-paste` are carried by the NAMED cold agent [agents/dual-channel-drift-auditor.md](../../agents/dual-channel-drift-auditor.md), which is measured-un-gateable, not merely un-attempted (§8, §9). No part of §8 rests on bare reviewer attention ([attention-is-not-a-mechanism.md §1](attention-is-not-a-mechanism.md)). Promotion history: §9.
...
**`#two-prompts-drift`** — two artefacts for the «same» capability diverge by ≥3 substantive lines (excluding boilerplate, comments, path references). Falsification: `diff -u agent.md hook-spec-comment.md | grep '^[+-]' | grep -v '^[+-][+-][+-]'` shows >3 non-boilerplate differences. **Detection layer:** the named cold agent [agents/dual-channel-drift-auditor.md](../../agents/dual-channel-drift-auditor.md) (`DRIFT` verdict) — «a reviewer will diff them» is not a mechanism ([attention-is-not-a-mechanism.md §1](attention-is-not-a-mechanism.md)). Counter: extract the canonical spec to one SSOT file; the other channel is a thin pointer.

**`#brand-name-detection`** — code branches on a brand string (`"claude"`, `"cc"`, `ANTHROPIC`) in runtime logic. **Gated** (the one syntactic member of §8): [tests/agnosticism/probes/brand-detection.sh](../../tests/agnosticism/probes/brand-detection.sh), Surface 10 of principle 21, over every tracked `*.sh`. The falsification command previously printed here — `grep -rn '"claude"\|"cc"\|ANTHROPIC' .claude/hooks/` — was **not** shippable verbatim: measured 2026-08-09 it flags `[[ -n "$ANTHROPIC_API_KEY" ]]`, a §4-legitimate capability check. The shipped detector matches a brand literal only in *comparison* position (`==` / `!=` / `=~` / space-delimited `=` / `case … in claude)`), with whole-line comments stripped: 4/4 true-positive, 0/3 false-positive, 0 hits across 267 tracked scripts. Counter: capability-check per §4.
...
**`#sync-by-copy-paste`** — two channel artefacts share semantic text via manual copy rather than a SSOT pointer. Shape: two artefacts in a `@dual-pair` share ≥5 consecutive non-boilerplate lines verbatim AND neither carries a `# spec: <path>` (or `<!-- spec-of: <path> -->`) cross-reference. Counter: extract shared spec; one channel becomes the SSOT, the other points to it via `spec:` marker. This is the enforcement-side of §7.

**Detection layer: the named cold agent** [agents/dual-channel-drift-auditor.md](../../agents/dual-channel-drift-auditor.md) (`COPY-RISK` verdict). **This is un-gateable by measurement, not by assumption** — the grep sketch that used to sit here (flag any `@dual-pair` member lacking a `spec:` pointer) was run against all 24 anchor groups on 2026-08-09 and scored **5 true positives / 8 false positives / 2 false negatives** — 38% precision. It flagged pairs sharing at most one line, and passed `deps-hash-check-dogfood`, whose two members share **257** verbatim lines. Inverting to a raw «≥5 verbatim lines» threshold fails the opposite way: it fires on the `runtime-bridge` `vendor/` copies, which are byte-identical *by construction* (the CLAUDE.md vendor carve-out). Separating a deliberate generated/vendored twin from accidental copy-rot needs a **mechanism** (a regenerating hook, a byte-identity test) to be identified and judged — no surveyed clone detector does this ([SSOT #242](../../docs/meta-factory/prior-art-evaluations.md): jscpd, PMD CPD, Vendetect all defer it to manual ignore-config). Full measurement: [research-patches/2026-08-09-dual-impl-s8-detection-layer.md §2](../../docs/meta-factory/research-patches/2026-08-09-dual-impl-s8-detection-layer.md).
...
The original triggers that *would* have fired this promotion — 3 `#two-prompts-drift`/`#cc-only-without-rationale` violations in a 6-month window, OR the 5th dual-channel artefact — are now moot for §5/§6. A dedicated TypeScript principle slot stays unneeded unless drift complexity outgrows the bash probe (then: promote the §5/§6 sketches to TS at the lowest-free slot, extensible to AST inspection).

**§8 semantic half — closed 2026-08-09 (detection layer, not attention).** §8 previously ended at «reviewer-time judgment», which [attention-is-not-a-mechanism.md §1](attention-is-not-a-mechanism.md) names as `#hope-as-gate`: bare attention may be merge *authority*, never the *detection* layer. Each anti-pattern was measured before being assigned a channel, in both directions (fires on a known violation AND stays silent on the legitimate shape):

| Anti-pattern | Measured | Channel shipped |
|---|---|---|
| `#brand-name-detection` | refined detector 4/4 TP, 0/3 FP, 0 hits / 267 tracked `*.sh` | **gate** — [brand-detection.sh](../../tests/agnosticism/probes/brand-detection.sh), Surface 10 |
| `#sync-by-copy-paste` | `spec:`-pointer sketch = 5 TP / 8 FP / 2 FN (38% precision) over 24 anchor groups; verbatim-threshold inverse fires on by-construction vendor twins | **named cold agent** — [dual-channel-drift-auditor.md](../../agents/dual-channel-drift-auditor.md) |
| `#two-prompts-drift` | «substantive vs boilerplate» divergence is semantic; no deterministic form survives [no-paid-llm-in-ci.md](no-paid-llm-in-ci.md) | **named cold agent** — same |

The un-gateable verdict for the latter two is **evidence-backed, not assumed** — the falsification data lives in [research-patches/2026-08-09-dual-impl-s8-detection-layer.md](../../docs/meta-factory/research-patches/2026-08-09-dual-impl-s8-detection-layer.md) and [SSOT #242](../../docs/meta-factory/prior-art-evaluations.md). **Re-gate trigger:** a clone detector ships an intentional-vs-accidental classifier; OR the anchor-group population exceeds 60 (cold-agent review stops scaling); OR 3 `#sync-by-copy-paste` incidents reach a PR despite the agent — then revisit a threshold gate with a measured allowlist.
...
- [attention-is-not-a-mechanism.md §1](attention-is-not-a-mechanism.md) — the rule that forbids bare reviewer attention as a detection layer; the §8 channels above are its (a)/(b) forms
- [agents/dual-channel-drift-auditor.md](../../agents/dual-channel-drift-auditor.md) — the named cold agent carrying §8's two judgment anti-patterns
- [tests/agnosticism/probes/brand-detection.sh](../../tests/agnosticism/probes/brand-detection.sh) — Surface 10 gate for `#brand-name-detection`
- SSOT #242 (clone detectors REFERENCE + judgment-layer BUILD) — the §8 detection-layer prior-art consult
