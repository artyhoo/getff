...
> **Fires:** a CONFLICTING PR, or any rebase/force-push urge.
...
4.  Conflicts ONLY in generated artefacts? Check:
      git diff --name-only --diff-filter=U \
        | grep -v -e 'tests/install-sh/baselines/' -e 'plugin/hooks/' -e 'plugin/skills/'
...
6.  Regenerate: SNAPSHOT_MODE=capture bash tests/install-sh/snapshot.sh
    (plugin/hooks twins are regenerated by the generate-plugin-twins pre-commit hook;
     plugin/skills twins by the generate-plugin-skills pre-commit arm — PR #1732.
     Either also regenerates standalone: scripts/generate-plugin-twins.sh /
     scripts/generate-plugin-skills.sh, if re-running the commit is undesirable.)
...
The conflicts in this recurrence class are almost always **generated artefacts**: `tests/install-sh/baselines/*/*.fingerprint` (8 files), `plugin/hooks/*` twins, and `plugin/skills/*` twins — any two PRs touching shipped hooks or skills move the same files. Generated conflicts are resolved by **regeneration** (§2 steps 5-6), never by hand-merging hunks.
