...
_rule_ref() { # .claude/rules/<name>.md -> "(.claude/rules/<name>.md)" | "(rule <name>)"
  if _has ".claude/rules/$1.md"; then
    printf '(.claude/rules/%s.md)' "$1"
  else
    printf '(rule %s)' "$1"
  fi
}
_rule_bare() { # ".claude/rules/<name>.md" -> ".claude/rules/<name>.md" | "rule <name>" (no parens)
  if _has ".claude/rules/$1.md"; then
    printf '.claude/rules/%s.md' "$1"
  else
    printf 'rule %s' "$1"
...
_rule_name() { # bare ".claude/rules/<name>.md" -> "<name>.md" | "<name>" (no dead path)
  if _has ".claude/rules/$1.md"; then
    printf '%s.md' "$1"
  else
    printf '%s' "$1"
...
STEP0_LIST=''
for _t in README.md .claude/session-bootstrap.md CLAUDE.md; do
  if _has "$_t"; then
    if [ -n "$STEP0_LIST" ]; then STEP0_LIST="$STEP0_LIST → $_t"; else STEP0_LIST="$_t"; fi
  fi
done
if [ -n "$STEP0_LIST" ]; then
  STEP0_LINE="Step-0 reading order: $STEP0_LIST → task-specific docs."
else
  STEP0_LINE='Step-0 reading order: task-specific docs.'
fi

TRAPS_NAME=$(_rule_name ai-laziness-traps)
if _has .claude/rules/phase-research-coverage.md; then
  PRC_REF='(.claude/rules/phase-research-coverage.md §1.7)'
else
  PRC_REF='(rule phase-research-coverage §1.7)'  # §1.7 qualifier survives the degradation
fi
H1_LINE="Recommendation discipline (H1): before issuing a verdict/recommendation (ADOPT/BUILD/REJECT/DEFER, «we should X», «use Y», «pick A over B») — (1) cite SSOT/prior-art by ID, (2) give file:line or command-output evidence, (3) state what would falsify it («wrong if …»), (4) for «nothing exists» claims run the 6-item search check. An unbacked verdict is provisional, not load-bearing. This is a reminder, not a gate. (see also $(_rule_bare recommendation-laziness-discipline) + T-trap in $TRAPS_NAME §2) $PRC_REF"

if _has .claude/session-bootstrap.md; then
  FOOTER_LINE='Full bootstrap + reviewer drift-prevention flowchart: .claude/session-bootstrap.md'
else
  FOOTER_LINE='Full bootstrap + reviewer drift-prevention flowchart: session-bootstrap digest only (bootstrap file absent in this tree)'
fi
...
# #F10AUTONOMY anchor — second channel token for .claude/rules/autonomous-loop-continuity.md
# (`<!-- channel: digest .claude/hooks/inject-session-bootstrap.sh#F10AUTONOMY -->`) points HERE:
# the AUTONOMY BLOCK below (the opt-in AIF_AUTONOMOUS=1 digest extension at the bottom of this
# hook) — the always-on second delivery surface named in that rule's Class-B
# header. The rule's first marker (#F10 → end-of-turn-reminder.sh) names the Stop-hook arm;
# this one names the digest-block arm. Both must be declared separately so the rendered index
# reports the full delivery surface (S-F channel-truth fix, 2026-08-06).
...
#
# R4: the autonomy block carries its own path-shaped citations (ai-laziness-traps.md T19/T21,
# packages/runtime-bridge/src/cli/await.ts, CLAUDE.md) — same degradation applies: absent
# target -> bare name, invariant text kept verbatim.
if [ "${AIF_AUTONOMOUS:-0}" = "1" ]; then
  if _has packages/runtime-bridge/src/cli/await.ts; then
    AWAIT_REF='`packages/runtime-bridge/src/cli/await.ts`'
  else
    AWAIT_REF='`await.ts`'
  fi
  if _has CLAUDE.md; then
    CMD_SOURCE='CLAUDE.md, '
  else
    CMD_SOURCE=''
  fi
