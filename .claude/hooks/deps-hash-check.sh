...
# rust (Cargo.toml — DETECTION ONLY per kickoff §DH-S2: no setup.d/NN-rust.sh delivery lane
# is built here). At each session start it sha256-hashes the consumer's DECLARED deps per
# present stack and compares against per-stack baselines in .ai-factory/tool-decisions.md;
...
#   - pnpm v11 relocated `overrides` into pnpm-workspace.yaml, out of the package.json surface
#     this hook hashes — out-of-band until a pnpm-workspace.yaml lane is added.
...
_drifted "package.json" "$(_memo npm "$(_memo_key package.json)" _npm_final)" "$NPM_STORED"
