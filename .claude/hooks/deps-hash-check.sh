...
#     only through a symlinked directory, stay invisible — and the memo key (_npm_memo_key,
#     find -maxdepth 10) over-covers the same tree, so the two bounds cannot diverge into a
#     TTL-window blind spot (key superset ⊇ extractor input set, by construction).
...
# `{a,b}` braces, `!` exclusions) over candidate dirs ≤8 levels deep, node_modules/.git/
# dot-dirs skipped, symlinks NOT followed (lstat — mirrors find -P, so the extractor's input
# set stays a subset of the _npm_memo_key find below).
#
...
function memberDirs(globs){const inc=[],exc=[];for(const g of globs){const neg=g.charAt(0)==="!";const body=neg?g.slice(1):g;for(const p of braces(body))(neg?exc:inc).push(g2r(p));}if(!inc.length)return[];const cand=[];dirsUnder(".",1,cand);const seen={},out=[];for(const d of cand){let ok=false;for(const r of inc)if(r.test(d)){ok=true;break;}if(!ok)continue;let bad=false;for(const r of exc)if(r.test(d)){bad=true;break;}if(bad)continue;if(seen[d])continue;seen[d]=1;if(fs.existsSync(d+"/package.json"))out.push(d);}return out.sort();}
function parseWs(text){const ws={packages:[],overrides:{},catalog:{},catalogs:{}};let sec=null,cat=null;const kv=/^("[^"]*"|\u0027[^\u0027]*\u0027|[^:]+):(.*)$/;for(const raw of text.split(/\r?\n/)){const t=raw.trim();if(!t||t.charAt(0)==="#")continue;const ind=(raw.match(/^ */))[0].length;if(ind===0){sec=null;cat=null;const m=t.match(/^([A-Za-z0-9_-]+):(.*)$/);if(!m)continue;const rest=m[2].trim();if(m[1]!=="packages"&&m[1]!=="overrides"&&m[1]!=="catalog"&&m[1]!=="catalogs")continue;sec=m[1];if(rest.charAt(0)==="-"){const it=sq(rest.replace(/^-\s*/,""));if(sec==="packages"&&it)ws.packages.push(it);}else if(rest.charAt(0)==="["&&rest.slice(-1)==="]"){if(sec==="packages")for(const it of rest.slice(1,-1).split(",")){const s=sq(it);if(s)ws.packages.push(s);}sec=null;}else if(rest)sec=null;continue;}if(sec==="packages"){const m=t.match(/^-(.*)$/);if(m){const s=sq(m[1]);if(s)ws.packages.push(s);}}else if(sec==="overrides"||sec==="catalog"){const m=t.match(kv);if(m)ws[sec][sq(m[1])]=sq(m[2]);}else if(sec==="catalogs"){const m=t.match(kv);if(!m)continue;if(ind===2&&!m[2].trim()){cat=sq(m[1]);ws.catalogs[cat]={};}else if(cat){ws.catalogs[cat][sq(m[1])]=sq(m[2]);}}}return ws;}
...
# npm-lane memo key (GH #1264): a SUPERSET of the extractor input set — every package.json
# and pnpm-workspace.yaml under cwd within 10 levels (find -P semantics: node_modules/.git
# pruned, directory symlinks not followed — matching the extractor's lstat walk, which is
# additionally bounded at 8 levels), cksum'd in sorted order. The old key covered only the
# ROOT package.json, so a workspace-member edit inside the _MEMO_TTL window served the
# stale cached hash even after enumeration landed (the in-issue review called this the trap
# of the fix). Superset direction is fail-safe: a non-member package.json edit over-
# invalidates (forces one recompute, answer unchanged) — never a blind hit.
_npm_memo_key() {
  local f sig=""
  [ -f package.json ] || { printf ''; return; }
  ...
  done < <(find . -maxdepth 10 \( -name node_modules -o -name .git \) -prune -o \
             -type f \( -name package.json -o -name pnpm-workspace.yaml \) -print 2>/dev/null | LC_ALL=C sort)
  printf '%s' "$sig"
}
