...
# Code projects directory — never on a brand string, never on a specific harness identifier.
# Mirrors inject-matching-rule.sh's structure (session-cache, JSON additionalContext contract).
#
...
# Path-based capability check: does the target path have a "/memory/" segment? This matches
# the user-scope agent-memory convention (~/.claude/projects/<slug>/memory/*.md) without
# hardcoding a home-directory path or any brand-specific prefix — any harness that stores
# per-project agent memory under a "memory/" directory is covered the same way.
case "$ABS_PATH" in
  */memory/*) ;;
  *) exit 0 ;;
esac
...
# Once per session — mirrors inject-matching-rule.sh's session-cache pattern.
