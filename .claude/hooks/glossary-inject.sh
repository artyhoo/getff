...
#
# Scans the operator's prompt for glossary terms (CONTEXT.md, `**Term**:` headings) and
# their `_Operator says_` raw words; for each match injects one line
#   "<raw word>" = <term>: <definition>
# (the operator→agent direction) and counts ONE usage per term per prompt. A synonym is
# NOT a usage: only the term name itself and the listed raw words match. The Stop-side
# twin (the glossary arm inside end-of-turn-reminder.sh) reads the per-session pending
# file this hook writes and counts the agent→operator direction (the fixed
# «term (explanation)» form). BOTH sides derive the counters file as
# `_residue_dir()/_glossary-counts.json` — a path split between the two hooks is the
# kickoff's item-4 falsifier («the hook is registered and a session still never sees an
# injected line»).
#
