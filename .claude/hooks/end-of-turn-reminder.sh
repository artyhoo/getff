...
# ── SDK-entrypoint guard — a non-interactive session has no reader for a recap ────────────
# The Agent SDK spawns the CLI with CLAUDE_CODE_ENTRYPOINT=sdk-ts (sdk-py for Python) and, with
# `settingSources:["project"]`, loads this Stop hook. Measured 2026-09-09 in the aif container:
# the hook blocked the review sidecar's final turn (`## Blocking Findings` already emitted),
# the model answered the block with a recap-only message, and aif read that LAST message as the
# sidecar result — `parseStructuredSidecarOutput` returned null on 503 of 503 review-gate runs
# ever recorded, every task fell to the legacy parser and parked at manual review. The recap is
# for a human at a terminal; in an SDK session the caller's output contract owns the last
# message. POSITION IS LOAD-BEARING (same reason as the F10 arm below): it must precede every
# blocking arm, F10 included — an aif-driven session is not the unattended orchestrator F10
# protects, and blocking it re-creates the exact defect. AIF_EOT_SDK_RECAP=1 restores the demand
# for an SDK harness that deliberately wants it. Prefix match, not a literal: the class is
# «SDK-driven», not one language binding.
case "${CLAUDE_CODE_ENTRYPOINT:-}" in
  sdk-*) [ "${AIF_EOT_SDK_RECAP:-0}" = "1" ] || exit 0 ;;
esac
...
# ── Out-of-band /compact guard (D37) — the inverse of D36 ────────────────────
# D36 has the block hand the operator a ready-to-paste `/compact` command. The band is the
# ONLY place that command is wanted: below the floor a compaction throws away a window that
# was not full. Measured 2026-09-14 (getff.ai design session `5ab4a9ca`): 11 turns ended with
# the command, 9 of them legitimately in the band and 2 below it (193749 and 153658 tokens
# against a 201000 floor) — BOTH on the first turn after a compaction. The operator ran one of
# them and lost context for nothing.
#
# Cause, and why prose cannot hold it: the gate asks for the command on EVERY in-band turn, so
# a compaction summary faithfully generalises «every turn ends with /compact» from the turns it
# saw, and the post-compaction model obeys that below the floor. D36's own wording is prose —
# it is the thing that got paraphrased away — so the counter cannot be more prose in the same
# channel (`attention-is-not-a-mechanism.md` §1: the detection layer may not be attention). The
# defect is therefore detected where it LANDS, in the turn's final text, whichever channel
# planted the habit: the summary, a stale handoff, or a Keep-list the model wrote itself.
#
# Armed-only BY PLACEMENT (`AIF_HANDOFF_GATE`): unarmed sessions never see the D36 hint, so
# they cannot acquire the habit, and fixture 9's unarmed byte-identity is untouched.
# `ctx_tokens` / `gate_floor` exist only when the D7 context arm ran (:363+) — outside it they
# are unset and `set -u` (:9) would abort the hook, so the `:-` guards are load-bearing.
# `ctx_line` is empty here BY CONSTRUCTION: gate_floor is clamped to ≤ ctx_soft (:449-450), so
# below the floor no tier fired — D21's «one reason» contract needs no extra clearing, and the
...
if [ "${AIF_HANDOFF_GATE:-0}" = "1" ] && [ -z "$gate_line" ] &&
  [ -n "${ctx_tokens:-}" ] && [ -n "${gate_floor:-}" ] &&
  [ "$ctx_tokens" -lt "$gate_floor" ] &&
...
# simply stay 0.
if [ -n "$text" ] && [ -f "${CLAUDE_PROJECT_DIR:-$(pwd)}/CONTEXT.md" ]; then
  ...
  if [ -f "$_gl_pending" ]; then
    ...
    while IFS=$'\t' read -r _gl_term _gl_word; do
      ...
      if [ ! -f "$_gl_flag" ]; then
        ...
        if [ -n "$glossary_line" ]; then
          glossary_line="${glossary_line}"$'\n'"$(aif_msg_glossary_demand "$_gl_term" "$_gl_word")"
        else
          glossary_line="$(aif_msg_glossary_demand "$_gl_term" "$_gl_word")"
        fi
      fi
    done < "$_gl_pending"
...
if [ "$asked" = "true" ] && [ "$long_text" = "false" ]; then
  ...
  if [ -z "$text" ] || [ "$has_askuserquestion" = "true" ]; then
    ...
    # Stage 9C (zcode-parity-s9c-multiturn-zcode-arm) — source for prev_line:
    # CC reads its native transcript (multi-line per session); ZCode's synthetic
    # transcript has 1 line per turn (step-1 evidence, zcode.cjs:~1072550 $_n
    # producer), making tail-2|head-1 structurally inert on ZCode. When _is_zcode,
    # prefer the rollout file (~/.zcode/cli/rollout/model-io-*.jsonl) which has
    # multi-line per session (R1 INHERITED-INFERRED per research-patch
    # 2026-07-18-zcode-parity-s9-multiturn-anchor.md §3.3; own-verification BLOCKED
    # on container access — runtime gated on rollout reachability, degrades
    ...
    if _is_zcode; then
      ...
      _zc_file=""
      if [ -d "$_zc_dir" ]; then
        # Prefer session-id-pinned file; fall back to newest model-io-*.jsonl.
        if [ -n "$session_id" ] && [ "$session_id" != "nosession" ] \
           && [ -f "$_zc_dir/model-io-$session_id.jsonl" ]; then
          _zc_file="$_zc_dir/model-io-$session_id.jsonl"
        else
          _zc_file=$(ls -t "$_zc_dir"/model-io-*.jsonl 2>/dev/null | head -1 || true)
        fi
      fi
      if [ -n "$_zc_file" ] && [ -f "$_zc_file" ]; then
        # Need ≥2 assistant turns in rollout for prev_line to be meaningful;
        # otherwise tail-2|head-1 equals tail-1 (degenerate == current turn).
        _zc_n=$(grep -cE '"(type|role)":"assistant"' "$_zc_file" 2>/dev/null || echo 0)
        if [ "${_zc_n:-0}" -ge 2 ]; then
          _prev_src="$_zc_file"
        fi
        unset _zc_n
      fi
      unset _zc_dir _zc_file
    fi
    prev_line=$(grep -E '"(type|role)":"assistant"' "$_prev_src" 2>/dev/null | tail -2 | head -1 || true)
    unset _prev_src
