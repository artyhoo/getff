#!/usr/bin/env bash
# check-kickoff-traps.sh — PostToolUse gate — kickoff T-enumeration floor (ai-laziness-traps §3)
...
if ! . "$_HOOK_LIB" 2>/dev/null; then
  ...
  printf '{"hookSpecificOutput":{"hookEventName":"PostToolUse","additionalContext":"%s"}}\n' \
    '⚠ check-kickoff-traps: lib/hook-emit.sh could not be sourced — this gate DID NOT RUN for this edit. This is a SKIP, not a pass; reinstall the hooks.'
  exit 0
fi
...
if [[ -z "$HV_RUNNER" ]]; then
  # Applicability BEFORE loudness. This arm enforces
  # .claude/rules/destination-environment-verification.md §1, and neither that rule nor
  # scripts/host-verify.sh is delivered to consumers by any channel: `git ls-tree -r <sha>
  # plugin setup.d install.sh packages/core/templates | grep -c host-verify` is 0, setup.d and
  # install.sh ship no .claude/rules/, and the plugin has no .claude/ directory. So on every
  # plugin consumer this branch fired a LOUD "install the runner" skip on every kickoff write —
  # one the consumer could never satisfy (#1597 review ledger A3-6). A permanent unsatisfiable
  # notice is the mirror image of the silent no-op: both stop carrying information.
  #
  # Split by whether the project actually carries the rule this arm enforces:
  #   - rule present (framework repo, or a consumer that vendored it) + runner missing → a real
  #     regression, still announced LOUDLY exactly as before;
  #   - rule absent → NOT-APPLICABLE, silent. There is no contract in this project for the
  #     kickoff to be missing, so there is nothing to report.
  if [[ -f "$REPO_ROOT/.claude/rules/destination-environment-verification.md" ]]; then
