...
#   with — the non-hook equivalent is the TS dispatch CLI below, and packages/**
#   TS is outside delivery-channel triage (dual-implementation-discipline.md
#   §2(iv)). This marker is NOT a harness claim: corrected 2026-09-11 from
#   «deterministic trigger only available in CC harness» (stale since the plugin
#   twin) — the twin fires the same PostToolUse + PostToolUseFailure moments on
#   ZCode (doctrine row 17: degraded, Write+Edit + failure arm live), and
#   setup.d/55-runtime-bridge-vendor.sh ships this hook to factory consumers.
