...
**Prerequisite:** step-1 B1 source-level fix has landed (umbrella `zcode-parity-step1-emit-wrapper`). This stage uses the fixed source.
...
**Prerequisite:** step-1 `_zcode-emit` helper has landed + Stage 2 verdict (Fork 2A lint ensures migration doesn't regress).
