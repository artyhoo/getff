...
| S-D′ | per-seat SUBTRACTION maps — reopened scope, operator override 2026-08-06 (#234 trigger (a) fired). **Rev 6 (2026-08-07): ADR-8's A/B arm DESCOPED per the operator's §5 = Option A → S-K; this stage ships maps + review-seat agent definitions + the #234 annotation, and a PR with no evaluation arm is conformant** | S-E + S-H + **S-L merged** (consumes P11 probe + P3d numbers + S-L's re-priced P14 ranking — two-gate form each, §3). **S-L PROMOTED TO A GATE — operator verdict 2026-08-07, resolving PR #1255 `DECISION-NEEDED` 1:** S-L recalculates the P14 conversion this stage ranks by, and a falsified conversion falsifies the *ranking*, which is this stage's product (spec `:387`, row P13). S-E (#1237), S-H (#1239 + #1249) and **S-L (#1263)** all merged 2026-08-07 → every gate MET on both arms. **CLOSED — dispatched (aif `acc1cd1d`, no marker) and MERGED 2026-08-08 as PR #1290 (squash `be087d3585`), fidelity GO round 2.** Open residue lives in the PR's Parked questions: the Tier-0-swap fork (spec §3.1 `DECISION-NEEDED`, operator-reserved) + the header INPUT-CONDITION `KICKOFF-AMBIGUOUS` | 2 | **NO** (map authoring = un-spent judgment) | ADR-1 |
| S-M | **Tier-0 swap** — `build-first-reuse-default.md` (the #2 always-on lever, MEASURED 4,800 tok) re-scoped always-on → `paths:(7)` + edit-time inject. Operator verdict 2026-08-08 resolving S-D′ spec §3.2 = **Option B, dedicated stage**; kickoff rev 3 passed Phase -1 (2 seats × 2 rounds + narrow verify). **CLOSED — dispatched (aif `75442280`, GLM marker) and MERGED 2026-08-08 as PR #1302 (squash `6878c41f50`), fidelity GO r2; resident set 43,135 → 30,481 B (registry measure, T-SM-B).** Watch-list W-1..W-6 in the PR body | S-D′ merged (#1290) — **MET** | 1 | **YES** `Z.AI GLM-5.2 SDK` | ADR-1 |
| S-K | **RETIRED at scoping 2026-08-08 (ADR-8's third and closing deviation, operator-ruled)** — the A/B is replaced by the incident-triggered rollback watch at [`calibration.md`](calibration.md) Item 2-R. Grounds: power analysis over 11 recent harvests (mean ≈ 3.9 review-defects/dispatch, SD ≈ 4.3 → a 10v10 window detects only Δ ≥ 5.4, more than the entire mean); NOT feasibility — the rev-6 task-id blocker dissolved at scoping (a paused task's `description` IS server-editable; `packages/api` audit). Full record: [`done.md`](done.md) | S-D′ merged — **MET** | — | — | ADR-8 |
...
**Retired 2026-08-08 (operator-ruled) — ADR-8's THIRD and closing deviation.** The A/B is retired
at S-K scoping under the operator's pre-registered decision rule (measure historical defect-count
noise first); replacement mechanism + full evidence record: [`done.md`](done.md) + [`calibration.md`](calibration.md) Item 2-R.
