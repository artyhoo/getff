...
| S2 | Pipeline presets + status | A4: 4 declarative presets (`aif`/`night`/`economy`/`sdd`) in `references/`, list verb, `--preset` flag, TTY proposal in launch-table; A5: `/pipeline status` (read-only, sectioned, next-command lines) | preset activation works flag-only (non-TTY); status renders all three sections against live bricks (`questions.ts`, bridge REST, `gh pr list`) | `economy` emits the bridge-profile marker — semantics per acceptance-contour D1: **blocked-pending-neighbor** for the marker-emitting branch only |
