...
Command — the kickoff's `host-verify` block verbatim, no environment forced:
...
**On the host's rust default, and a correction.** The host default is
`rustc 1.98.1 (48a229cea 2026-09-01)`, so the first harvest run forced `RUSTUP_TOOLCHAIN=1.96.1`
on the theory that the lane would otherwise fire the wrong compiler. That theory was wrong, and
the correction is load-bearing enough to record: each firing fixture carries its own
`rust-toolchain.toml` with `channel = "1.96.1"`
(`packages/core/backends/cargo/fixtures/firing/{invalid,valid,valid-clean}/rust-toolchain.toml`),
and `packages/core/backends/cargo/firing.test.ts:5-11` states exactly that mechanism — «the
fixtures each pin `rust-toolchain.toml` channel = 1.96.1 … no false-RED from a runner's default
rust». `RUSTUP_TOOLCHAIN` takes precedence **over** that file, so the env pin did not protect the
run — it disabled the thing that was already protecting it. Both runs are recorded: with the env
pin and without it the result is identical (`37 passed | 3 skipped`), and inside the fixture
directory `rustc --version` resolves `rustc 1.96.1 (31fca3adb 2026-06-26)` with no environment
set. The numbers above are the un-forced run.
...
All 40 firing tests across the five suites passed with **zero skips** in this container (tools present; the loud-skip arms in each `firing.test.ts` never fired). The host run is not zero-skip — three golangci arms skip there for an absent tool (§host-verify).
...
|---|---|---|---|---|
| npm | `firing.test.ts:8` — in-process RuleTester, **always-on** («…so this whole suite is always-on in CI») | **excluded by design** — the runner's header (`:1-8`) names itself «the consumer-side companion to run-generated-rule-mutation.sh (npm lane)… THIS runner verifies the NON-npm test material» | n/a (npm lane verifies via mutation runner ≥60% kill floor) | none needed (no external tool) |
| astgrep | `firing.test.ts` — spawns the PATH binary, loud-skip `console.warn` at `:45-52`, gate `describe.skipIf(!toolPresent)` at `:57`, **no `!isCI`** | `_fire_astgrep` `:206` | via python lane (the astgrep adapter IS the python lane's structural surface) | `:294` `npm install -g @ast-grep/cli@0.44.1` |
| ruff | `firing.test.ts` — same posture: loud-skip `:51-60`, `describe.skipIf` `:62` | `_fire_ruff` `:273` | `_py_firing_self_check` `setup.d/45-python.sh:487` | `:304` `pip install ruff==0.15.21` |
| cargo | `firing.test.ts:5-7` — live-fire «wherever cargo is on PATH — including CI (ecosystem-wiring W4)», loud warn when absent | `_fire_cargo` `:384` | `_cargo_firing_self_check` `setup.d/46-cargo.sh:201` | `:333-334` `rustup toolchain install 1.96.1 … --component clippy` |
| golangci | `firing.test.ts:59-72` — loud-skip `console.warn`, **no `!isCI`**; contract non-vacuous guard `:145-150` | **absent by design** — the runner reads `.ai-factory/rule-tests/{astgrep,ruff,cargo}.json` (`:12`); go has no sidecar material | `_go_firing_self_check` `setup.d/47-go.sh:187` — dual-tool gate (go AND golangci-lint), planted violation + clean control, rc=0 loud degrade | `:368-372` `go install …golangci-lint@v1.55.2` + J3 live-fire arm `:383-419` |
...
1. **go ships exactly one ban — CONFIRMED.** `templates/go/.golangci.yml` = `disable-all: true` + `enable: [forbidigo]` (`:12-15`) + exactly one forbid entry `p: 'os\.Getenv'` (`:31`); the template dir holds exactly two files (`.golangci.yml`, `github-actions-ci.yml`). The cargo analogue is real: `templates/cargo/clippy.toml` carries exactly one `disallowed-methods` entry, `std::env::var`, with the paired discipline reason text (`:2-4`, the file is 4 lines). Both fire red/green as shown in §firing.
...
3. **golangci self-reports the identity-granularity cap — CONFIRMED; sweep found no violating doc.** `golangci/capability-matrix.json:6-12` records `syntax=partial` with the forbidigo-linter-name cap verbatim. Executed sweep of shipped + spec docs (`README.md`, `INSTALL-FOR-AI.md`, `INSTALL.md`, `docs/`) for per-rule-attribution claims on the go lane: **no violator found**. Corrected enumeration (the first pass named two of the three files): `grep -rn forbidigo README.md INSTALL-FOR-AI.md INSTALL.md docs/` returns 6 hits in 3 files — `docs/meta-factory/research-patches/2026-07-02-multi-toolchain-generalization.md:68,163` (the ecosystem tool's config surface, not our lane's attribution), `docs/meta-factory/research-patches/2026-07-26-lane-channel-parity-audit.md:97,150` (CI-firing parity, not per-rule attribution), and `docs/superpowers/specs/2026-08-18-pre-merge-carrier-design.md:113,171` (command examples). None claims per-rule attribution for the go lane. The `partial` stands honest and un-contradicted.
...
- **F5 — ruff provisioning in CI is pip-only; the wheel route is untested upstream.** This container fired ruff from the PyPI wheel (pip absent). Same version, same `--version` string; the CI resolver (`pip install ruff==0.15.21`) resolves the same wheel on linux. Residual risk is platform-shape drift on non-linux runners. Partly closed: the macOS host re-fire used a **third** install route again — `ruff 0.15.21` from a uv-managed tool install (`command -v ruff` → `/Users/art/.local/bin/ruff` → `/Users/art/.local/share/uv/tools/ruff/bin/ruff`; no Homebrew keg exists) — and all 7 ruff arms fired identically to the container's PyPI-wheel route (§host-verify). Three routes, one `--version` string, one diagnostic set.
...
2. **Green-with-skips is possible on the host.** The dispatch's host-verify one-liner does not install the tools. On a host without golangci-lint/ruff/cargo/ast-grep on PATH, 4 suites' live blocks loud-skip and the command exits green having fired only npm. Falsifier: the acceptance run must assert on **skip counts**, not the exit code — quoting `Tests  N passed` alone is exactly the `#container-green-as-acceptance` shape. **Amended at harvest** (the executed run, §host-verify): the criterion as first written was «skip count = 0», which the real host cannot meet for a tool it does not carry; a skip count of 0 or **every skip named, attributed to a missing tool, and given a disposition** is the criterion the executed contract was judged against. Skip count was 3, all three named, and the go lane is marked NOT host-confirmed rather than quietly counted as green — the failure this falsifier exists to catch did not occur, but the report no longer claims a clean zero either.
