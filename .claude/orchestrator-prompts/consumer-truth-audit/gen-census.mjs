...
// Rows are derived from the consumer trees + git-tracked lists measured this session;
// verdicts/citations come from the logs in ./logs/. One row per artefact (kickoff §4).
import { execSync } from 'node:child_process';
import { writeFileSync } from 'node:fs';

const REPO = '/home/www/rules-as-tests-aif-feature-consumer-truth-audit-55b9ad-55b9ad6a-8c8d-47de-8672-bbe63c892370';
const CORE = '/tmp/census-consumer-core-wYlJfn';
const ENV = '/tmp/census-consumer-env-VXYTkl';
const FACT = '/tmp/census-consumer-factory-Iwbf1j';
const sh = (c) => execSync(c, { encoding: 'utf8' }).split('\n').filter(Boolean);
const inTree = (root, rel) => execSync(`test -e '${root}/${rel}' && echo y || echo n`, { encoding: 'utf8' }).trim() === 'y';
...
const deliveredHooks = { core: sh(`cd ${CORE} && find .claude/hooks -type f | sed 's|.claude/hooks/||' | sort`),
                         env:  sh(`cd ${ENV} && find .claude/hooks -type f | sed 's|.claude/hooks/||' | sort`),
                         factory: sh(`cd ${FACT} && find .claude/hooks -type f | sed 's|.claude/hooks/||' | sort`) };
const deliveredChecks = sh(`cd ${FACT} && find packages/core/hooks/checks -type f | xargs -n1 basename | sort`);
const deliveredScripts = { core: sh(`cd ${CORE} && find scripts -type f | sort`),
                           env:  sh(`cd ${ENV} && find scripts -type f | sort`),
                           factory: sh(`cd ${FACT} && find scripts -type f | sort`) };
const dl = (list, name) => ({ core: list.core.includes(name), env: list.env.includes(name), factory: list.factory.includes(name) });
const dbool = (list, name) => ({ core: list.includes(name), env: list.includes(name), factory: list.includes(name) });
...
add({ artefact: `${rootGetff}/`, class: 'skill', has: true,
  delivered: dbool(deliveredSkills.factory, 'getff') && dbool(deliveredSkills.core, 'getff') ? { core: true, env: true, factory: true } : { core: false, env: false, factory: false },
...
for (const a of trackedAgents) {
  const d = dbool(deliveredAgents.core, a) ? { core: true, env: true, factory: true }
    : dbool(deliveredAgents.factory, a) ? { core: false, env: false, factory: true } : { core: false, env: false, factory: false };
...
for (const r of trackedRules) {
  add({ artefact: `.claude/rules/${r}`, class: 'discipline-rule', has: true,
    ...
    aged_install: AGED, verdict: 'BY-DESIGN',
    by_design_citation: 'setup.d/LAYERS.md:79 («rules/ is not shipped» — transform_internal_refs rewrites .claude/rules/ refs in delivered docs to upstream blob URLs); corroborated at setup.d/20-agents.sh:45',
...
for (const c of prodChecks) {
  const delivered = deliveredChecks.includes(c);
  const f = checkFacts[c];
  if (f) {
    add({ artefact: `packages/core/hooks/checks/${c}`, class: 'hook-check', has: true,
      delivered: { core: delivered, env: delivered, factory: delivered },
      works: f.w, aged_install: AGED, verdict: f.v,
...
// ── C7 templates (7 rows: 5 template dirs + root stack configs) ────────────
const templateRows = [
  ['packages/core/templates/shared/', { core: true, env: true, factory: true }, '11-13 .ai-factory files per tree (M7)', 'setup.d/LAYERS.md:27 (40-configs §5a shared templates)'],
  ['packages/core/templates/ts-server-configs/', { core: true, env: true, factory: true }, 'eslint.config.mjs, vitest.config.ts, tsconfig.json, .dependency-cruiser.cjs, stryker.config.json, tests/setup.ts at consumer root (install-core.log)', 'setup.d/LAYERS.md:27 (§6a stack configs)'],
  ['packages/core/templates/cargo/', { core: false, env: false, factory: false }, 'not delivered on the npm flow (ts-server fixture): GETFF_TOOLCHAIN=cargo lane only — INERT by design', 'setup.d/LAYERS.md:29 (46-cargo.sh «INERT on the npm flow»)'],
  ['packages/core/templates/go/', { core: false, env: false, factory: false }, 'not delivered on the npm flow: GETFF_TOOLCHAIN=go lane only', 'setup.d/LAYERS.md:29 (47-go.sh analog)'],
  ['packages/core/templates/python/', { core: false, env: false, factory: false }, 'not delivered on the npm flow: GETFF_TOOLCHAIN=python lane only', 'setup.d/LAYERS.md:28 (45-python.sh «INERT on the npm flow»)'],
  ['packages/core/templates/react-next/', { core: false, env: false, factory: false }, 'not delivered to the ts-server fixture (stack-gated; delivered on react-next stacks — outside this census\'s fixture stack)', 'setup.d/LAYERS.md:10 (stack-aware gating)'],
  ['templates/ts-server/', { core: true, env: true, factory: true }, 'github-actions-ci.yml + github-actions-workflow-integrity.yml delivered as .github/workflows/ci.yml + workflow-integrity.yml; stack configs at root', 'setup.d/40-configs.sh:448,454 (deliver_getff_workflow from templates/ts-server/)'],
];
for (const [t, d, w, c] of templateRows) {
  add({ artefact: t, class: 'template', has: true, delivered: d, works: w, aged_install: AGED,
    verdict: 'BY-DESIGN', by_design_citation: c,
    evidence: `M2/M7 consumer inventories + install logs; spec-path correction: the kickoff's «packages/core/templates/**/workflows» matches nothing (population-enumeration.md §C10b)` });
}

// ── C8 lint bundles (8 rows) ────────────────────────────────────────────────
const lintRows = [
  ['packages/core/eslint-rules/ (4 rules + index)', { core: true, env: true, factory: true }, 'works — delivered as packages/core/eslint-rules/ (5 files) AND bundled into consumer eslint-rules-local/ (13 files, generated barrel «4 rules» per install log)', 'setup.d/LAYERS.md:27 (§5b\' ESLint rules)', 'M4/M7: present in all 3 trees'],
  ['packages/core/templates/python/.getff/astgrep-rules/ (4 yml)', { core: false, env: false, factory: false }, 'python lane only', 'setup.d/LAYERS.md:28', 'C8b: no .getff at repo root — spec path corrected'],
  ['packages/core/templates/cargo/clippy.toml', { core: false, env: false, factory: false }, 'cargo lane only', 'setup.d/LAYERS.md:29', 'C8'],
  ['packages/core/templates/cargo/deny.toml', { core: false, env: false, factory: false }, 'cargo lane only', 'setup.d/LAYERS.md:29', 'C7 tree'],
  ['packages/core/templates/go/.golangci.yml', { core: false, env: false, factory: false }, 'go lane only', 'setup.d/LAYERS.md:29', 'C8'],
  ['packages/core/templates/python/ruff.toml', { core: false, env: false, factory: false }, 'python lane only', 'setup.d/LAYERS.md:28', 'C7 tree'],
  ['templates/ts-server/eslint.config.mjs (synth-wire baseline)', { core: true, env: true, factory: true }, 'delivered; synth-wire emitted no rules for ts-server preset baseline (install log), live-research augmentation path documented in log', 'setup.d/LAYERS.md:27', 'install-core.log [synth-wire] lines'],
  ['packages/core/eslint-rules-local (consumer barrel)', { core: true, env: true, factory: true }, 'works — 13 files (4 rules × ts/mjs/d.ts + index.mjs) delivered; RULES the consumer code', 'setup.d/LAYERS.md:27', 'M2: eslint-rules-local = 13 files all profiles'],
];
for (const [t, d, w, c, e] of lintRows) {
  add({ artefact: t, class: 'lint-bundle', has: true, delivered: d, works: w, aged_install: AGED,
...
for (const c of companions) {
  add({ artefact: `companions.manifest row: ${c}`, class: 'companion', has: true,
    ...
    evidence: 'M5: no companion/mcp files anywhere in consumer trees; install log companion section = 6 print lines only' });
}

// ── C10 CI workflows (5 rows) ───────────────────────────────────────────────
const ciRows = [
  ['.github/workflows/ci.yml', { core: true, env: true, factory: true }, 'delivered (ts-server); uses «main» default-branch fallback (no origin — warning printed at install)', 'setup.d/40-configs.sh:448'],
  ['.github/workflows/workflow-integrity.yml', { core: true, env: true, factory: true }, 'delivered', 'setup.d/40-configs.sh:454'],
  ['.github/workflows/getff-cargo.yml (lane)', { core: false, env: false, factory: false }, 'cargo lane only', 'setup.d/46-cargo.sh:162'],
  ['.github/workflows getff-go (lane)', { core: false, env: false, factory: false }, 'go lane only', 'setup.d/47-go.sh:4'],
  ['.github/workflows getff-python (lane)', { core: false, env: false, factory: false }, 'python lane only', 'setup.d/45-python.sh (github-actions-ci.yml template)'],
];
for (const [t, d, w, c] of ciRows) {
  add({ artefact: t, class: 'ci-workflow', has: true, delivered: d, works: w, aged_install: AGED,
    verdict: 'BY-DESIGN', by_design_citation: c,
...
for (const s of scriptsAll) {
  const d = dl(deliveredScripts, s.replace(/^scripts\//, ''));
  const name = s.replace(/^scripts\//, '');
  if (d.core || d.env || d.factory) {
    add({ artefact: s, class: 'script', has: true, delivered: d,
      works: scriptWorks[name] || 'delivered; not individually exercised in V0 (same layer as the 12 exercised scripts — L2e)',
      aged_install: AGED, verdict: 'BY-DESIGN',
      by_design_citation: scriptWorks[name] ? shippedScriptCite : 'setup.d/LAYERS.md:27 + :10',
...
writeFileSync(`${REPO}/.claude/orchestrator-prompts/consumer-truth-audit/census-v0.json`, JSON.stringify(census, null, 2) + '\n');
