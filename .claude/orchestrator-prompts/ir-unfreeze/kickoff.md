<!-- scope: kickoff — ir-unfreeze umbrella (OWNER-FORK-1: unfreeze the neutral ConventionNode IR to a relational shape, or stay narrow). Status: R-phase LANDED (docs/meta-factory/research-patches/2026-07-21-ir-unfreeze.md on feat/fork1-rphase); dispatch is BLOCKED on an owner precondition — the Option-A-vs-B IR-shape decision (§1 of the R-patch). Base = frozen ConventionNode (ir/types.ts:3, verified origin/staging 8747d8453) + the LG-S1 python census data (40/93, +53.3pt relational delta). NO stage may unfreeze the IR before the owner resolves the design-fork. -->
