...
- **Commit trailer — use the positive form, not the escape hatch.** The new test file from criterion 3 is a new file ≥80 LOC under `packages/`, which makes this a **capability commit** by `packages/core/hooks/checks/prior-art.ts:129-136`; on a capability commit a `Prior-art: skipped` payload is rejected with code 2 (`prior-art.ts:186-200`) and `pre-push.ts:337-360` turns that into exit 1. Since gate bypass is forbidden below, the escape hatch would park the run after all the work is done. Use:
