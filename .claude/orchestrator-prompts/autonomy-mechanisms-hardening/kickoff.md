...
- **Commit trailer — use the positive form, not the escape hatch.** The new test file from criterion 3 is a new file ≥80 LOC under `packages/`, which makes this a **capability commit** by `packages/core/hooks/checks/prior-art.ts:129-136`; on a capability commit a `Prior-art: skipped` payload is rejected with code 2 (`prior-art.ts:186-200`). **Which gate that blocks at (updated 2026-07-25, handoff item 4):** BOTH channels now. Locally, pre-push — its substance arm reads `PA_SUBSTANCE_WARN_ONLY ?? 'false'` (`pre-push.ts` `priorArtSection`), enforcing by default since 2026-07-25 (the earlier `?? 'true'` warn-only asymmetry vs the S17 arm was the wave-8 retro's unfulfilled auto-flip; this paragraph previously documented that asymmetry, corrected in #1144, now closed). Server-side, `.github/workflows/pr-body-prior-art.yml` re-runs the same check against the **PR body** (a squash drops branch-commit trailers, so the PR body is what survives) — that backstop is unchanged. Either way the escape hatch fails the PR after all the work is done. Use:
