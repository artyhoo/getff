...
2. **Workflow-security scanners (`actionlint`, `zizmor`, the ci-tool-pinning unpinned-install gate) do NOT run on a consumer.** They are `owner: maintainer`. This is a deliberate **scope-OUT** (per `ci-tool-pinning.md §2` — the framework does not gate a consumer's own files), NOT a "handed to another channel" claim: **workflow-security scanning of the consumer's OWN workflows is currently provided by no shipped channel** — neither this push hook nor any shipped CI template runs `zizmor`/`actionlint` (verified below). A consumer who wants action-pinning enforcement adds it to their own CI.
...
4. **A clean-tree `git push` is ALLOWED — on the TS-core channel.**
...
- **(i)** clean-tree consumer `git push` to a bare remote, **on the TS-core channel** → **allowed** (no workflow scan; rule-globs/lint-staged/lychee do not block a clean tree). S2 must run the TS hook directly (or assert `tsx` resolvable), not the bash fallback, for this assert.
...
- It **extends an already-implemented doctrine** rather than inventing one. `ci-tool-pinning.md §2` ("the rule is scoped to THIS repository — a consumer's own scripts must not be gated by our discipline") is ALREADY enforced for the *shell-script* slice: `unpinnedToolInstallSection` uses `isFrameworkRepo ? shellScriptFiles() : []` (`pre-push.ts:577`). F-push is that same doctrine leaking on the *workflow* slice; the fix makes the workflow slice match the shell slice.
...
- **Attention-is-not-a-mechanism compliant.** This is NOT "degrade to warn" (a warning nobody reads is not a mechanism, §1). It is an explicit scope-OUT: the framework does not gate the consumer's own workflows. **Correction (BLOCKER from cold-review):** an earlier draft justified this by claiming "the shipped workflow-integrity CI template runs zizmor — the correct channel." That is **factually false** — `grep -rln "zizmor\|actionlint" templates/ts-server/*.yml packages/preset-*/templates/*.yml` → **zero matches**; the shipped `github-actions-workflow-integrity.yml` carries "the R11 branch-protection self-assertion only" (its own header) and the shipped `ci.yml` `security` job runs only `npm audit --audit-level=high` + gitleaks (`:173-188`). `zizmor`/`actionlint` live only in the **maintainer's** `.github/workflows/audit-self.yml`, never shipped. So this is a pure scope-OUT: workflow-security scanning of the consumer's own workflows is provided by **no** shipped channel — the consumer adds it to their own CI if they want it. The adjudication (don't hard-block the consumer's first push on their own pre-existing workflows) stands; only the false "shipped CI covers it" justification is removed.
...
- `unpinnedToolInstallSection` shell slice (`pre-push.ts:577`, `...(isFrameworkRepo ? shellScriptFiles() : [])`) — the precedent; **SWEPT-CLEAN**, now the workflow slice + zizmor + actionlint match it.
