...
## Decision 1 — Signal 4 project checkout: derive from task `worktreePath` (kickoff) or read `GET /projects/<id>` → `rootPath`
...
- B — one extra `GET /projects/<RUNTIME_BRIDGE_AIF_PROJECT_ID>` → `rootPath`. Exact,
  task-independent, no naming heuristic; one more HTTP call on a probe that already pays one.
  Both options need `RUNTIME_BRIDGE_AIF_PROJECT_ID` (explicit `reason=no-project-id` +
  `AIF_REPO_PATH` override land either way).
...
back to A silently; the payload makes the worker re-probe the field at execution time and park
