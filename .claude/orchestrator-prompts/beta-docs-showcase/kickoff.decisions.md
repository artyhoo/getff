...
dispatched 2026-09-02T04:28:31Z as aif task `f1010da4-1ada-40f5-ae90-d5f2b2322073` on project `361685f1` (getff-landing), `runtimeProfileId=null` → project defaults (plan/review `53eca24c` GLM-5.3 SDK, task `088182b8` GLM-5.3 Flash); stage kickoff `kickoff-b0.md` (#1561 `d813773aa3`, corrected by #1562 `b68443e094`, #1565 `5705f8f594`, #1567 `bb073afb29`). Supersedes two earlier dispatches, both cancelled pre-work through `claim.ts cancel` and confirmed 404: `371242f1-…` (03:51Z — kickoff pinned the landing base at stale `733197e` while the container sat at `b65ff4b`; #1565) and `70844c29-…` (04:09Z — entry-probe row 1 ordered a STOP on every healthy worktree, because a fresh aif task worktree always carries untracked `.ai-factory/` + `AGENTS.md`; #1567 names that set as expected and STOPs only on a modified/deleted tracked file). Lesson carried forward: the entry-probe row of a stage kickoff is written against a MEASURED tree (inspect the live task worktree), never against what the tree should look like — both defects died only when someone looked at the real thing. **LEG A: GREEN — prototype builds, in-container route gate quoted, deploy pending operator** (task `f1010da4` `done` 2026-09-02 ~06:3xZ; all six gate rows re-verified by the dispatch session against the delivered build, not its report; harvested via bundle to `artyhoo/getff-landing` branch `bs0-fumadocs-smoke` = `93948f857e`, 24 files all under `smoke/`, landing `main` untouched at `b65ff4b534`, zero open landing PRs). **NOT BS0 green:** the live gate (deployed URL, HTTPS, Orama on the deployed site, noindex in fetched HTML) still gates BS0→BS1 and waits on the operator floor (create public `artyhoo/getff-docs-smoke`, lift `smoke/` to its root, enable Pages; `beta.getff.ai` DNS optional — github.io fallback). Findings (R-B1 does NOT fire, Starlight rollback not indicated): `rewrites()` unsupported under `output: 'export'` → `.md` twin emitted via literal route segment `app/docs/<slug>.md/route.ts` from the same source; `oramaStaticClient` is a deprecated alias (engine ZBSearch, provider `provider/next`, `createFromSource` from `search/server`) — the T12 hedge earned its keep; `trailingSlash: true` required for a static host; `lib/site.ts` needs explicit `api`/`from` (fumadocs-core default reads Vite's `BASE_URL`). Near-miss recorded: the first verification pass hit the ORPHAN's serve on :4173 (llms.txt advertised a page absent from the new `out/`); redone on the real build.
