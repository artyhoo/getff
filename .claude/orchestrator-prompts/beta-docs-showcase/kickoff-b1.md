...
| 5   | Two-layer panel: `grep` the built `out/index.html` for «AI DX on both sides of the keyboard», «Rules from live docs», «The AI factory», the two badges, and the B-D2 subtitle verbatim; confirm the jargon is not LEAKED — «killer» → **0 files**, and every occurrence of «environment» sits inside the D3 lede string, which mandates it verbatim (`grep -rl environment out/` then check each hit is the lede; a standalone use is the failure). **Do not read this row as «the substring must be absent»** — D3 requires the lede character-for-character, so an absence check is unpassable by construction and was flagged `KICKOFF-AMBIGUOUS` in every BS1 audit round       | the panel renders with the approved copy, and the two internal-jargon words appear only where the mandated copy puts them                                                                                                                              |
...
| 9   | **Anti-carry-over:** no `noindex` on any of the eleven census pages — fetch each and `grep -c noindex` → **0** each. **Not** `grep -ril noindex out/` → 0: Next hard-stamps `noindex` on its OWN not-found artefacts (`404.html`, `404/`, `_not-found/`, a shared runtime chunk — 7 files, measured), which is correct SEO for a 404 and is not removable without fighting the framework, so the whole-tree form is unpassable by construction and was flagged `KICKOFF-AMBIGUOUS` in every BS1 audit round; `cat out/robots.txt` → the production `Allow: /` form; `cat out/CNAME` → `getff.ai`; `grep -r 'getff-docs-smoke' out/ .` → nothing outside `package-lock.json` history | none of D1's five carry-overs survived                                                                                                                                                                                                                 |
