...
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Container base clone is `/home/www/getff-landing` at `main` = **`b65ff4b`** (= `origin/main`, verified 2026-09-02 in the base clone, inside the container, and via `gh api repos/artyhoo/getff-landing/commits/main`); your task runs in an ISOLATED per-task worktree (`parallelEnabled: true`), not in that clone | `git -C <your worktree> log --oneline -1`, `git status --short` | **A fresh task worktree is NOT clean, and that is normal** — it carries exactly two untracked entries, `.ai-factory/` and `AGENTS.md`, which aif puts in every worktree (measured 2026-09-02 in this stage's own worktree). Those are not dirt and are not a STOP. **STOP and report** only if (a) any TRACKED file is modified or deleted (`git status --short` showing `M`/`D`), or (b) your worktree path is the base clone `/home/www/getff-landing` itself — the base clone carries a modified `package.json` plus ~16 untracked installer entries (`.husky/`, `eslint.config.mjs`, `packages/`, `scripts/`, `.github/workflows/*` …) that must never enter your commit. In every case: stage only paths under `smoke/`, **never `git add -A`** |
...
Active traps for this stage: **T2**, **T3**, **T7**, **T10**, **T12**, **T14**, **T16**, **T19**.
...
- **T10** — enumerate before claiming. Run `ls -R out` and paste the route inventory BEFORE
  asserting which routes exist. Gate row 4 is exactly where an assumed route survives as prose;
  an enumeration makes a missing file impossible to miss.
- **T12** — Fumadocs/Next/Tailwind specifics from live docs at build time, never memory. Two
  symbols will shorten the search, but confirm both against current docs rather than trusting
  this line: the static search client (`fumadocs-core/search/client`, the static/Orama variant)
  and the llms-text helper commonly shown as `getLLMText`. If either no longer exists under
  that name, that is a doc-drift finding worth one line in the report.
- **T14** — coverage bounds the verdict. «Gate rows pass» plus «all six rows actually ran» =
  leg A green. «Gate rows pass» plus «two rows were skipped» = «coverage insufficient to
  conclude», not green. Say which of the six ran.
