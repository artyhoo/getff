<!-- scope: kickoff — adapter-jig stage J3 (first stamped family: go). Binding design: docs/superpowers/specs/2026-07-22-adapter-jig-design.md §2 (frozen rows F1-F11) + §3 (22 arms) + §9 J3; umbrella scope: .claude/orchestrator-prompts/adapter-jig/kickoff.md §1 J3 / §2 / §5. Routing: OPERATOR OVERRIDE 2026-07-25 — dispatched WITH a bridge-profile marker (`Z.AI GLM-5.2 SDK`), i.e. the executor tier plans as well as implements. The default read of CLAUDE.md task-tier routing would have been Tier 2 (no marker; top tier plans) because §2.1's F3 host-derivation node is a genuine design judgment — the operator called it the other way, which is theirs to call. The consequence is deliberate and stated so the worker knows what it is carrying: the design judgment in §2.1 now sits with the executor tier, so its park triggers (§3) are the load-bearing safeguard rather than a formality. Marker value verified unique at authoring time against the live `/runtime-profiles` list: `Z.AI GLM-5.2 SDK` matches exactly 1 profile by exact name AND 1 by substring, so it resolves unambiguously under either arm. `AifHandoffBackend._resolveProfileId` (packages/runtime-bridge/src/AifHandoffBackend.ts:137-141) is TWO-step — an exact case-insensitive name match short-circuits, falling back to a case-insensitive substring match — so the prefix collision (`Z.AI GLM-5.2` is a strict prefix of `Z.AI GLM-5.2 SDK`) is saved at runtime; an ABBREVIATION still matches ≥2 under the substring fallback and aborts with `dispatch_failed`, which is why the authoring rule still binds. -->
