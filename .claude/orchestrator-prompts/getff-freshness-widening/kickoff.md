...
> **js/rust/go** parity — locks record reality, staleness becomes addressable, the two-client
> ledger seam for level 2 lands, and the ruff/cargo committed-fixture debt closes. Spec BINDING;
...
> manifests (`:9-10`); **two** `.sh` hook copies asserted byte-identical by
> `deps-hash-check.test.ts:515-526`, plus a THIRD copy `plugin/hooks/deps-hash-check`
> **generated from the `.claude/` copy** — see S2 for the mechanism that actually exists;
> `rules-lock.python.json` `"version": null` (probe);
...
- **S2 — targeted staleness (spec §7.2).** Widen the deps-hash WARN lock glob beyond
  `.ai-factory/synthesizer-output/` to the python lock home in `.getff/`. **Edit BOTH `.sh`
  copies** — `packages/core/hooks/deps-hash-check.sh` AND `.claude/hooks/deps-hash-check.sh` —
  and **stage the `.claude/` one**, then let `plugin/hooks/deps-hash-check` regenerate. Why
  exactly that, verified live: `deps-hash-check.test.ts:515-526` only **asserts** the two `.sh`
  copies are identical (`expect(dogfood).toBe(source)`) — it is a check, not a sync, and **no
  sync from `packages/core/` to `.claude/` exists**, so editing one copy alone turns it RED.
  The third copy is generated by `scripts/generate-plugin-twins.sh`, whose
  `SRC_DIR="$REPO_ROOT/.claude/hooks"` (`:17`) means it reads the **dogfood** copy, and
  `.husky/pre-commit:158` fires the generator **only** when a `.claude/hooks/*.sh` path is
  staged — so a `packages/core`-only edit never regenerates the twin either. Never hand-edit
  the twin. (The hook header's «3-way guard» is one claim over **two** mechanisms — an
  assertion plus a generator — not one byte-identity test across three files.) Lock-diff names
  WHICH rules cite the changed package; WARN
...
- **S4 — js/rust/go parity of the trace (spec §4.1/§5/§6 parity clauses).** Thread ResolveCtx
  ...
  mirror — R1 §5.1 routed it as a distinct item. **Also settle the parked
  `branches: [main]` template-substitution question for the cargo and go lanes** — the trace
  umbrella states it «remains open for those lanes» and that its python fixture «does NOT
  mechanically settle it for cargo/go» (`getff-any-stack-trace/done.md`). It belongs here
  rather than in a stage of its own: it is part of the cargo/go **rung parity** this stage
  already owns, not a fourth concern.
...
+ the two-copy byte-identity assertion); [2] S4's per-lane firing for the lanes that genuinely
live-fire without an extra toolchain; [3] S1's cross-lane lock-schema alignment.

**Deliberately EXCLUDED — cargo and golangci live-fire.** On a host without `cargo` /
`golangci-lint` those suites report **PASS while their live-fire assertions SKIP** (measured:
`golangci/firing.test.ts` → `10 passed | 3 skipped`, file still reported as passed). Naming them
here would buy a green exit code that asserts nothing about the go/cargo lanes — the T-S0-B /
T-FW-A shape this umbrella exists to prevent. Their evidence obligation lives in §2 instead, where
it is **quoted firing with the tool's exit code**, and it carries an explicit precondition: the
stage claiming go/cargo parity must run on a host with those toolchains present and say so.

```bash host-verify
npx vitest run packages/core/hooks/deps-hash-check.test.ts
npx vitest run packages/core/backends/ruff/firing.test.ts packages/core/backends/astgrep/firing.test.ts
bash tests/install-sh/rules-lock-schema-parity.test.sh
```

## §5 aif agent — fork discipline (non-negotiable) — the «§4c park-don't-guess» contract

> Peer skills and the meta-launch record cite this contract as «§4c» (its section number in
> `.claude/skills/pipeline/templates/meta-kickoff.template.md`). In THIS kickoff it is §5;
> a reader searching for «§4c» should land here.
...
- Upstream closure + the routed-onward list this umbrella inherits (cited by S4, S4b and S5):
  [getff-any-stack-trace/done.md](../getff-any-stack-trace/done.md). **«R1 §5.1»** throughout this
  file means §5.1 of the trace umbrella's round-2 lane × channel-rung parity audit (PR #1254),
  whose routed-onward items that `done.md` records.
