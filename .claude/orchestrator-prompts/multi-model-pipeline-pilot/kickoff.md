...
> **Umbrella:** `multi-model-pipeline-pilot`. **Status:** authored + staged — **operator launches via `/pipeline multi-model-pipeline-pilot`** after the z.ai quota window refreshes; dispatch OFF-PEAK only (§5). A premature direct-dispatch task (`c0e83c44`, 2026-07-23) was paused at `planning` with **0 tokens spent** and cancelled — superseded by the /pipeline launch path; its preflight evidence is banked in §3b.
...
## §1b Two-part protocol (who audits which half)

- **Part 1 — design contour (DONE, audited at authoring, 2026-07-23):** `/arch` idea→design → two-altitude cold review (REVISE round caught a real factual defect — B1) → kickoff authored → staging placement (#1108/#1109) → tier-routing D1 marker (ambiguity caught + fixed at preflight) → dispatch-transport legs proven on the cancelled 0-token task. **Its audit result IS this kickoff**: every Part-1 row is banked in §3b with evidence and flows into the final S4 checklist unchanged. The authoring session tested the first half of the pipeline by driving it.
- **Part 2 — execution + acceptance contour (OPERATOR-LAUNCHED, fresh session):** the operator runs `/pipeline multi-model-pipeline-pilot` after the z.ai quota window refreshes → dispatch → aif/GLM executes §2b (the in-container audit) → shields at commit/push → S3 acceptance surface (fidelity seam, gate GREEN+RED, acceptance-package sections, D1 calibration spot-check) → S4 synthesis merging Part-1 banked rows + Part-2 observations. That session audits the second half by driving it.
...
**S0 — channel inventory + carrier-reachability tagging (host-side, no model spend).** Derive rows from SSOTs, not memory: the 20-row hook census (row set only — T-AIP-D); pre-push shields (`packages/core/hooks/pre-push.ts` + `checks/*.ts`); **the full Phase A+B acceptance-contour artifact set as individual rows** (`agents/fidelity-auditor.md`; `pr-body-fidelity` check module + workflow; the PR-template acceptance-package sections; `/harvest §4` fidelity step; `/dispatcher §2.4` pre-egress gate + REVISE cap + §3 routing seats; the D1 calibration spot-check; the `/arch §3` always-marker route; the CLAUDE.md Tier-2 exception; the night-mode PR-gate); the coordinator skills (`/harvest`, `/dispatcher`); **the project's own invariant gates** (build-vs-reuse/capability consult, `make self-audit`, §1.7, prior-art trailer, 6-item search) — else they drop upstream of the no-silent-rows guard; **and the pipeline legs themselves** (§3b). Tag each row `{inject | shield | skill | acceptance-gate | invariant-gate | pipeline-leg}`, its CC event, and its **reachability by this carrier**: `reachable` vs `unreachable-by-carrier` (with the carrier shape that WOULD reach it). Deliverable: the frozen, reachability-tagged skeleton.
...
**S3 — harvest + acceptance contour (host-side, on the REAL evidence PR) — the full Phase A+B acceptance surface, not just the gate.** Every acceptance mechanism shipped in PRs #1102/#1106 is itself a checklist row here:

1. **Pre-egress fidelity seam** (`/dispatcher §2.4`): cold `agents/fidelity-auditor.md` on kickoff-§2b + container diff → FIDELITY block → PR body.
2. **`pr-body-fidelity` gate, both directions:** GREEN on the valid block; RED via a momentary stale-`Audited-SHA` edit, then restore (acceptance-layer paired-negative).
3. **Acceptance-package PR sections filled FOR REAL** (template D4): `Provenance` = aif task id + `bridge-profile` name + **models-per-stage from aif usage events** (this doubles as the plan-stage-fallback probe: did planning run GLM or fall back to the project-default Opus profile? — a parity row in itself); `Review findings` = factory review outcome + cold code-review; `Parked questions` = the real list (any §2b.6 park).
4. **D1 calibration plan spot-check** (Phase B, `/dispatcher §2.4` calibration window <5 merged stage-PRs — this IS one of the first): top-tier read-only spot-check of the task's plan (`GET /tasks/:id` → plan), recorded in `## Review findings`.
5. **REVISE rework loop** (`answer.ts --decision request_changes` → Round 2, cap 2): exercised only if fidelity Round 1 genuinely returns REVISE — do NOT stage a fake divergence; if Round 1 = GO, the row is COVERAGE-LIMITED (would-reach-it: a task whose diff truly diverges from its kickoff).
6. The PR is **real** — the S2 evidence patch — and merges on GO.
...
- **Dispatch-preflight legs (banked from the cancelled `c0e83c44` direct-dispatch, 2026-07-23, 0 tokens):** `refresh-aif-base.sh` container sync worked (staging → `114a164`); dispatch self-heal flipped project `parallelEnabled false→true` (Finding A — intended bridge behavior, left in place); marker name→id resolution SUCCEEDED post-fix (`runtimeProfileId=53eca24c` = the SDK profile); state machine `backlog→planning` transition + `paused:true` control both worked; the task was stopped at 0 tokens (`tokenTotal=0`, GLM SDK profile `lastUsageAt=null`). One open observation carried to S3.3: the task's `planRuntimeProfileId` was `null` → planning may fall back to the project-default Opus profile, potentially contradicting the whole-pipeline-on-executor D1 intent — usage events on the real run decide the row.
- **Coverage note (unreachable-by-this-run rows, pre-declared):** the night-mode PR-gate leg (Phase B) is structurally unreachable here (no unattended overnight session) → COVERAGE-LIMITED, would-reach-it carrier = a night-mode run; same for `/harvest` standalone §4 (this run uses the `/dispatcher §2.4` seam — the shared fidelity step is exercised, the standalone-skill wrapper is not).
