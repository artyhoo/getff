...
| Shape                             | **new workspace package `packages/getff/`** whose tarball lays out the repo root: `install.sh`, `setup`, `setup.d/`, `agents/`, `skills/`, `templates/`, `.claude/{hooks,skills,templates}`, `scripts/` shipped subset, `packages/{core,preset-*,runtime-bridge}`, `.prettierrc.json`. `lint-config` is NOT shipped (zero `PKG_ROOT` reads; freeze §1 keeps it private) and the `scripts/` subset is exactly the six files the installer reads (`check-ask-files.sh`, `run-local-ci-sweep.sh`, `create-worktree.sh`, `worktree-node-modules.sh`, `link-coordination.sh`, `getff-work.sh`) — corrected 2026-09-05 after the round-1 fidelity audit flagged both as KICKOFF-AMBIGUOUS. **Assembled at pack time, never hand-authored; `install.sh` does not move** (215 test references) | freeze §2 §0.5 RESOLVED (2026-08-17) — rejected alternatives listed there |
...
- `tests/consumer-matrix/getff-dist-cell.sh`, modelled on `npm-tarball-cell.sh`:
  ...
  canonical R2 violation the sibling cell already plants (`tests/consumer-matrix/pnpm-monorepo-cell.sh:200-202`, `OrderSchema.parse(req.body)` under `src/routes/` — no sourceable helper exists under `tests/install-sh/`; corrected 2026-09-05 after the round-1 fidelity audit) → `npm run lint`
