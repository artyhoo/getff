...
- Evidence (T3b): `.github/workflows/audit-self.yml:464`, quoted verbatim: `` # and 34 `describe.skipIf(!JQ)` blocks across 19 suites in packages/core/hooks/ `` (context line of the skipIf-guard step). Measured on the audited head — this report's own measurement; the comment itself carries no breakdown: exact-form `describe.skipIf(!JQ)` = **34 occurrences across 17 files** + 1 it-form file (`packages/core/hooks/check-worker-dispatch-channel.test.ts`, 2 `it.skipIf(!JQ)` guards) = **18 suites**, all under `packages/core/hooks/` — nothing measured in the tree sums to the comment's 19. File-count claim, re-scoped (an earlier draft's «spans 20 files» is retired): `grep -rl 'skipIf(!JQ)' --include='*.test.ts'` matches **18 code files**; the loose string additionally appears in 4 non-code files that mention it in prose (this workflow's own comment, `.claude/orchestrator-prompts/story-recap/kickoff.md`, `docs/meta-factory/research-patches/2026-06-17-hook-test-suite-rot.md`, `docs/superpowers/plans/2026-06-16-session-story-recap.md`) — 22 tracked files in all.
