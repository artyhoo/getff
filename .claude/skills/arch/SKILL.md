...
**Spec-template obligation (2026-08-10, [advisor-pattern-design §7/§8](../../../docs/superpowers/specs/2026-08-10-advisor-pattern-design.md)):** every spec this contour produces carries an **operator-premise register** (verbatim-faithful = faithful to MEANING in context, never a naked transcript; transfer by copy-or-pointer, never paraphrase) and **per-decision falsifiers**. The register is the layer the recorded-premise test stands on ([reviewer-discipline.md](../../rules/reviewer-discipline.md)): spec ≠ recorded idea — both required.

**Frontier pacing (ADAPT of mattpocock `grill-me`→`grilling`, [SSOT #253](../../../docs/meta-factory/prior-art-evaluations.md), 2026-08-17).** Two deltas over the wrapped brainstorming loop, which keeps its one-question-per-message rule for questions whose answers may reshape each other: (a) **batch the frontier** — questions whose prerequisites are already settled are asked together in one round, each carrying a reasoned recommendation (H1 posture; T8 «ask once, batched»); facts never enter the frontier — anything a probe can answer is probed, not asked (T20). (b) **enumerate-before-done** — the dialogue closes only when the decision frontier is empty: list the design's decisions and show each is either answered or recorded as an explicit operator fork. «Nothing left silently assumed» is T10 applied to the dialogue's decision population.
