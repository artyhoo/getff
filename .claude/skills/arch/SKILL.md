...
**Seat instantiation — operator model ladder (fixed 2026-07-23):** the contour runs a three-role ladder — *top designs, mid verifies, factory executes*. When the authoring session itself occupies the top tier (e.g. Fable), BOTH review seats default to the mid tier (`model: opus`): the review's power is cold-by-construction (artifact-only input, no authoring dialogue), not the reviewer's tier, and top-tier tokens are not spent on volume verification. The factory side of the ladder (executor tier, currently GLM) is owned by [CLAUDE.md «Task-tier routing»](../../../CLAUDE.md), not this skill. The operator may explicitly request a top-tier review seat for an unusually hard design.
