...
- **Class rule for new hooks:** any helper-invoking hook must resolve by tier and announce a miss loudly ([attention-is-not-a-mechanism.md §1](../../rules/attention-is-not-a-mechanism.md) — a skip whose only consumer is «nobody looked» is not a mechanism); the reference implementation is `check-kickoff-traps.sh:169-196`.
