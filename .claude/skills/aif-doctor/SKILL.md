...
| Base currency (§3.4)     | `gh api repos/<repo>/git/refs/heads/staging --jq .object.sha` vs `docker exec <agent> git -C <repo> rev-parse staging` | container base == live GitHub tip? mismatch → §3.4 stale base (the #1 cause of false-`done` garbage)                  |
...
- **Detect (read-only):**
  - **Mismatch:** §1 base-currency probe — `gh api repos/<repo>/git/refs/heads/staging --jq .object.sha` ≠ `docker exec aif-handoff-agent-1 git -C /home/www/rules-as-tests-aif rev-parse staging`.
...
- **Root cause:** the agent branches every task off local `staging`; that base is hand-synced host→container (the container can't git-fetch through the tunnel) and **drifts stale within days**. A task on a stale base produces garbage that can't be harvested.
- **Fix — `refresh-aif-base.sh` (Tier 1 when `activeTaskCount=0`; reversible):** `bash .claude/skills/aif-doctor/helpers/refresh-aif-base.sh [staging]` — the in-repo **portable** helper shipped to consumers (the maintainer's tunnel-tuned copy at `~/.claude/refresh-aif-base.sh` is equivalent). gh-API real tip → **fast no-op if already current (~1s)** → **PRIMARY (portable):** in-container `git fetch origin <branch>` (works when the container can reach GitHub) → **FALLBACK (tunnel/airgap):** host objects (host current: `git bundle`; host also stale: `~/.claude/sync-branch-from-api.sh` FF-only first — an _optional_ operator-local dep, [`feedback_sync_branch_down_via_gh_api`]) → `docker cp` → container `git branch -f` + tracking ref → verify. **Reversibility:** prints the old SHA → `docker exec aif-handoff-agent-1 git -C <repo> branch -f staging <old>`; non-destructive to task records/worktrees. **In-flight guard:** the heal entrypoint (`.claude/skills/aif-doctor/helpers/heal.sh`) applies the refresh only when `/agent/status` `activeTaskCount=0` (do NOT yank the base under a running task); otherwise it skips.
