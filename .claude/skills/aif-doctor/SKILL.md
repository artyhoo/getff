...
- **The failure class:** tasks pile up at raw `status:"review"` with `manualReviewRequired:true` while auto-review closes nothing. Four defects stack into one pyramid (evidence + full causal chain: [2026-09-10 recovery handoff](../../../docs/superpowers/specs/2026-09-10-aif-review-contract-recovery-handoff.md); every `packages/…` path below is **upstream, cited from the running aif image** — not a file of this repo):
