...
**Single SSOT:** both helpers live at `.claude/skills/pipeline/helpers/` only — no consumer mirror under `skills/pipeline/` per [install.sh](../../../../install.sh) authority. `delta-diff.sh` (read side) is genuinely new this stage; `update-delta.sh` (metadata writer) shipped Stage 2B and is UNCHANGED in this stage.
