...
> **Type:** {{KICKOFF_TYPE}} (generated by `/pipeline {{UMBRELLA}}` on {{DATE}}).
...
**Anti-duplication discipline:** the block content names sub-waves / AC items / file:line / REPORT-traces / dispatch-state. It does NOT restate the per-turn self-checks the hook already carries — "were the alternatives really weighed, or did you take the first that came to mind", "did you decide a fork SILENTLY" (`.claude/hooks/lang/en.sh:129-130`) — that is the hook's job (Branch A/C). If your block content is verbatim-copyable from `end-of-turn-reminder.sh` reminder text, you have drifted into `#two-prompts-drift` per `.claude/rules/dual-implementation-discipline.md §4` — fix by replacing with orchestrator-specific substance.
