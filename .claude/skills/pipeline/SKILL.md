...
**Step 1 — inject umbrella kickoff + dispatch state + dependency frontier:**
...
```!
bash "${CLAUDE_SKILL_DIR}/helpers/dispatch-from-state.sh" "${umbrella:-}"; bash "${CLAUDE_SKILL_DIR}/helpers/frontier.sh" "${umbrella:-}" 2>/dev/null
```
...
| Stage            | **Mechanical, not eyeballed (D-H13):** read `FRONTIER:` / `BLOCKED:` off the Step-1 [`frontier.sh`](helpers/frontier.sh) output — it derives the edges from the kickoff stage table's incumbent `Depends on` column (the ONE spelling; a second `Blocked-by:` spelling for the same edge is `#parallel-evolution-creep`). `BLOCKED: S4(unmet:S2,S3)` → S4 is later than S2/S3; `FRONTIER:` names what may dispatch now. Then obey the qualifiers, they are the honest half: `ATTN:` (marker-based dones to confirm at §6 · prose edges the table does not carry · rows with no cell) · `DEGRADE:`/`WARN:` (no column, unrecognised shape, cycle) → ordering is yours again · `UNRESOLVED:`/`RESIDUE:` → the cell named something outside the table, judge it. Grammar + measured reach + ceilings: [`references/frontier.md`](references/frontier.md) |
...
Substitute every `{{<PLACEHOLDER_NAME>}}` token in both templates. The canonical 40-token list — grouped by source (plan-currency / launch-table / AI-traps / slicing / state-companion) with one-line resolution rules per token — lives in [`references/placeholders.md`](references/placeholders.md). Read it once, then substitute from §1+§3 output. The mechanical check that 1:1 enumeration matches the live templates is the maintainer's responsibility (re-run a `comm -23` between the templates' placeholders and the references file when either changes).
...
Replace `<stage-N-branch>` with the actual head branch from the Stage N sub-wave (derived from kickoff or launch-table). See T-MOB-B below for the recycled-branch case. **Frontier coupling (D-H13):** §3 Step 1's `frontier.sh` says which stages are _eligible_ by `Depends on` edges, and its `done=yes basis=marker-unverified` (echoed on an `ATTN:` line) is a kickoff-row READ, never a merge proof — THIS command is the authority. Feed its verdict back with `MO_FRONTIER_DONE=<merged ids>` / `MO_FRONTIER_OPEN=<ids whose marker lied>` and re-read the frontier before dispatching.
...
- [references/plan-cache.md](references/plan-cache.md) + [references/master-backlog-delta.md](references/master-backlog-delta.md) — §2.5 Step 1/8 + §10 item 5 · [references/frontier.md](references/frontier.md) — §3 Step 1 + §6 dependency frontier (D-H13)
