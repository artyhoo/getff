...
| 6 | addendum **C11** — "no per-role filtering at any layer" (hook, rule, dispatcher, kickoff) — `(host path: substrate doc 4)` | deep-research **S8 + S11 + S12** — per-role shaping DOES exist via `agents/*.md` (16 framework + 19 pipeline agents, every file a deliberately-shaped role persona), `CLAUDE.md` Task-tier routing (`:104-133`), `reviewer-discipline.md` (1 role-shaping rule), 4 skills (`claude-glm-executor-handoff`/`night-mode`/`arch`/`pipeline`), and the superpowers plugin's 5 prompt templates — `(source: §3.2 of this bundle)` | **Highest-load-bearing contradiction.** C11 measures the **always-on auto-injected load** (hook layer) — correct there. S8/S11/S12 measure **deliberate per-role persona content** in agents + CLAUDE.md + plugin templates — also correct there. These are **two different definitions of "per-role context"**: (a) auto-injected at session start (uniform, C11 right); (b) hand-authored role-specific content the agent loads via its kickoff/template (differentiated, S8/S11/S12 right). Opus: resolve this **definition fork** before any fabla decision — the operator's verbatim question ("inject only needed") maps to (a); the operator's framing ("roles receive different") could map to either. |
