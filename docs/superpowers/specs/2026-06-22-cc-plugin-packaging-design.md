...
> **Skills row extended (2026-09-11, plugin-skills-generator Stage 2) — the CORE four ship; the `rule-research`/`rule-tests` exclusion in the previous block is superseded.** The operator GO of 2026-09-11 (umbrella kickoff §0/§7 — «the CORE four are the extension set», do-not-re-litigate) postdates that resolution and directs the installer's CORE tier onto the plugin channel. The honesty ground is dissolved mechanically, not waved away: (a) the `./setup --full` step `rule-research/SKILL.md:24` ends in resolves on-channel because the plugin bundles the installer (`plugin/install/`); (b) every repo-internal reference is rewritten to a blob URL by the generator's transform (`scripts/generate-plugin-skills.sh`, parity mirror of `setup.d/lib.sh:144-165`), so the per-backend paths `rule-tests/SKILL.md:23` names resolve to their sources instead of dangling. The installer has shipped these exact four as «always shipped, consumer-facing» CORE since 2026-08-01 (`setup.d/lib.sh:61` `GETFF_SKILLS_CORE`, same link-only depth) — the plugin being narrower than the installer had become the anomaly. Membership stays machine-checked: the generator's `ENTRY_TABLE` is the recorded decision, mirrored in principle 24 `M1_SET` and gated by arms (g)/(h). Adjudication + per-skill link-only proof: [`docs/meta-factory/research-patches/2026-09-11-plugin-skills-generator-stage0-reverif.md` §3/§4 F5](../../meta-factory/research-patches/2026-09-11-plugin-skills-generator-stage0-reverif.md).
