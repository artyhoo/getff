...
| D-Q18 `sources:` provenance | decided (umbrella D43) | `sources:` is DERIVED, not authored: the D29 renderer `--write` fills it as body anchors ∪ card sources; `docs-check.mjs --check` fails on drift (anchors ⊄ sources). Authors may ADD entries, never remove a derived one. The residual — a source the page depends on but neither anchors nor cites in a card — is a band-C evidence defect under the claims auditor (D-Q3), a known limit, not a gate. | A page fails `--check` after a clean `--write` → the two tools disagree on the anchor grammar; fix the seam, never hand-edit `sources:` to green. |
...
| TD BLOCKER-1 — «seven pages» (`§Face page`) presumes a writer for the four `/docs/quickstart-<stack>/` face pages that no package item names | **PENDING the operator's D48 answer** — (a) gold 7→11, (a′) `quickstart-ts` to gold + a deterministic C12 skeleton for the stack sub-kind, (b) GLM, (c) collapse; wording lands with batch 2, nothing folded here | Face page bullet |
