...
| D-Q16 Writer's self-filled card has a channel | decided (round-1 MAJOR-6) | The done-checklist's card lands as a commit trailer `Docs-card: C1 PASS, C2 PASS, … C13 N/A` on every commit touching `docs/site/**/*.md` prose, checked at pre-push AND in `audit-self.yml` over the PR's commit range (the CI arm is the D29a umbrella requirement — aif container seats never run pre-push). **NOT through `prior-art.ts`:** D-Q15 above records that `prior-art.ts` never fires on `.md` living outside `packages/`, which is exactly this population, so the earlier «both arms of `prior-art.ts`» named a gate that can never fire (`#hope-as-gate`, `attention-is-not-a-mechanism.md §2`). The trailer check is its OWN section in both channels; naming that section is OWED to the D30 owner (cold round, 2026-09-14). Checks: trailer present, all C-ids listed, escape `Docs-card: skipped — <≥20-char reason>`. The auditor diffs its verdict against the trailer (T15 seam). | Trailers are all-PASS on ≥90 % (`uncalibrated`) of family-1 commits while the auditor FAILs ≥1 criterion per page → the self-fill is theatre; drop the trailer, keep the auditor. |
...
- Framework precedents: `claims-conformance-auditor.md`, `template-audit` P2/P3/P5,
  ...
  (trailer gate), `.github/workflows/audit-self.yml:756` (binary pin by sha256), `reviewer-discipline.md §6/§6.2`,
...
| BU M6 — lychee warn-and-skip, unpinned | **FIXED** — pinned per `.github/workflows/audit-self.yml:756` (re-pointed from `:748-749` by the cold round, 2026-09-14); absent binary = ERROR | D-Q2, F10 |
...
| BU MINOR-2 — lychee pin cited at `audit-self.yml:720-721` | **FIXED, then RE-CORRECTED** (cold round, 2026-09-14): the round landed `:748-749`, which is the `--strip-components` comment block, and its «measured» claim was FALSE; the `sha256sum -c` line is `.github/workflows/audit-self.yml:756` — every citation re-pointed there | F10, D-Q2, Prior art, round-1 row |
