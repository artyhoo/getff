...
| D-H11 domain-modeling pairing | answered 2026-08-18; **superseded 2026-09-13 → ADAPT (plain-words-recap-v2 R-4)** | ADOPT skill + CONTEXT.md as-is (generation REJECTED on the merits: glossary is authored, not derived; the skill's value is live inline challenge-and-write). Our delta = pointer rule + principle test (§5.3). ADR dir REJECT. **Superseded 2026-09-13 (plain-words-recap-v2 R-4):** the pairing is ADAPT, not ADOPT-as-is — the challenge-and-write rule is NOT imported: the agent answers with the term plus its inline explanation and never corrects the operator's word (`CONTEXT.md` carries the own `_Operator says_` field; spec: docs/superpowers/specs/2026-09-13-plain-words-recap-v2-design.md §D-F) **Superseded in part 2026-09-22 ([reuse spec D9](2026-09-21-recap-wait-what-reuse-design.md)):** `/arch` idea sessions adopt the whole «During the session» chapter, ADRs included — «ADR dir REJECT» is reversed; the operator's words stay under `_Operator says_`. | wrong if entries duplicate owner-doc definitions despite the rule → degrade CONTEXT.md to a generated pointer index |
