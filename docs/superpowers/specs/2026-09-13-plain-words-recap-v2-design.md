...
> The round cap is reached: the residual forks R-14 / R-15 go to the operator before the exit.
...
from a cited transcript; slice 1's fixture reproduces it synthetically and the PR quotes one live
instance or records that none was found.
...
| R-14   | Arm a BLOCKING gate on consumer turns with operator-only calibration (1607 RU blocks, one operator) — round 2 TD-N8         | operator-fork                                | ESCALATED unpriced; decides whether slice 1 ships `--full` arming or ships the flag with arming deferred | a `--full` consumer reports the gate blocking a well-formed EN turn before any consumer-side measurement exists                                                                                    |
...
Every row carries a Status. `operator-fork` rows (R-8, R-9, R-12, R-14) are OPEN and named — the
frontier is not empty; asked once, batched, at the slice that needs them (T8; R-14 at the exit).
