...
**What the cap counts (r4 clarification, from a real self-application violation — see §7).** The cap bounds *failure to converge*: two REVISE verdicts in a row where the work under audit is the same scope. It does NOT count an audit re-run whose subject changed since the last verdict — a GO followed by newly-added code (e.g. hardening applied after a code review) starts a fresh convergence attempt, not a second failed one. The counter therefore **resets on any GO** and on any scope addition; only back-to-back REVISEs on unchanged scope accumulate toward the cap. Rationale: the failure mode worth escalating is «the implementer cannot satisfy the kickoff», not «the diff kept growing and was audited each time». Without this distinction the cap punishes exactly the behaviour the contour wants (re-auditing after every change) — the `Audited-SHA` staleness guard (D3) already forces a re-audit on every new commit, so an audit-counting cap would make a compliant PR unmergeable by construction.
...
- **Self-application violation that produced the D6 clarification (2026-07-23, recorded honestly rather than silently fixed):** while implementing Phase A of this very contour, the session ran fidelity audits at rounds 1 (GO), 2 (REVISE — on hardening added after a cold code review), and 3 (GO after the fix). Under D6 as originally written («Round = count of audits; cap 2; round-2 REVISE → STOP»), round 3 was a **violation of the session's own discipline**. Two things were wrong at once: the session should have escalated rather than continued, AND the rule counted the wrong thing — the `Audited-SHA` staleness guard forces a re-audit after every new commit, so an audit-counting cap makes any PR that takes review feedback unmergeable. The rule was amended (D6 «What the cap counts») rather than the history rewritten. This is the contour catching its own author, which is the intended failure mode of recursive self-application — but it also shows a cap whose semantics were never dogfooded before shipping.
