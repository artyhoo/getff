...
|---|---|---|---|---|
| D1 | Always-on head trim | **Bounded trim of `CLAUDE.md`; `ai-laziness-traps.md` whole for now — sub-fork D1b OPEN.** Trim scope = ONLY (a) prose duplicating another authoritative home and (b) prose whose enforcement is mechanical. **Judgment-bearing sections are a keep-list, out of trim scope** (`CLAUDE.md:106` tier routing «a judgment, never an automated classifier»; `:132` marker-value «belt-and-braces»; PR strategy; un-gated operational conventions). Keep-list authored INTO the executing kickoff. **D1b (pending operator):** replace the resident traps copy in expensive seats with a hot digest (trap names + one-line counters, ~2k vs 6.6k tokens), full text via `paths:`/skill-embed; the executor-bound channel already exists mechanically — principle test 12 forces every kickoff to cite + enumerate applicable T-numbers. Default until the operator confirms: traps stay whole. | The mechanical-enforcement argument holds only for the gated share; #1188 banked the easy half. `AGENTS.md` (8,861 B vs 23,740 B) is the pointer-form *pattern*, not a size target. D1b's ground is §0.5: the full lazy-executor manual resident in the smartest seat is the inversion. | A post-trim session bypasses a convention the trimmed prose carried → restore that section. D1b wrong if a senior-seat R-phase bypasses a trap the digest under-carried. |
...
| P5 | Bounded `CLAUDE.md` trim per D1 (keep-list mandatory in kickoff) | **S-G** | READ + WRITE [W] | `CLAUDE.md` = 16.7% [A]; headroom = duplicated/gated share, sized at kickoff (post-#1188 baseline) |
...
- **Operator, manual:** (1) P1 — the committed `.claude/settings.json` half; (2) per the
  ...
  cold seat (`cold-seat-economy.md §3`); (3) the D1b answer (traps digest vs whole).
