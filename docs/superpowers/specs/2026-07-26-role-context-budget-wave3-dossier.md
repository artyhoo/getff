# Wave 3 Dossier — In-flight compatibility + scratchpad discovery + GLM hypotheses

> **Companion to:** [`2026-07-26-role-context-budget-design.md`](./2026-07-26-role-context-budget-design.md) (the main design doc). This file holds §10 of that doc, split out to respect the repo's 600-line markdown limit (`.husky/pre-commit:64`).
>
> **Author:** orchestrator session (GLM-5.2). Per the user's instruction «максимум гипотизы строить которые опус проверит а фабл решит», §10.5 hypotheses are for Opus to cold-verify — not load-bearing verdicts.
> **Date:** 2026-07-26
...
### 10.3 The wave-3 discovery changes 3 options in §6

1. **Option E (extend hook per-role) = Seat B's Option A.** Same shape, same delivery channel. Wave-2 independently rediscovered Seat B's Option A.
2. **Option F (NEW — hybrid one-line anchor) = Seat B's Option C.** Addresses the F2 tradeoff: keep anti-drift uniform content + add a per-role prime. This is a genuine third path neither wave-1 nor wave-2 had.
3. **Option C (surgical doc-only) ≈ Seat B's Option B.** Both are "acknowledge, don't change behaviour." Wave-2 said this violates invariant #4; Seat B frames it as the honest deferral when the tradeoff is unresolved.
...
**H2 — Option F (Seat B's Option C, hybrid one-line anchor) is the strongest candidate.**
...
### 10.7 Recommended next action (for the fabla, not for me)

1. **Opus cold-verifies H1-H4** (especially H1's drift-incident generalization and H2's size/benefit math — these are the load-bearing unknowns).
2. **Fabla decides:**
   - (a) absorb into `session-start-token-audit` as S4, or run as separate umbrella (H3);
   - (b) pick an option (A/B/C/D/E/F), with H2 recommending F pending Opus's verification;
   - (c) confirm the delivery channel per H4 (skill-context for aif, SubagentStart for CC).
3. **Only after (1) and (2)** does implementation begin, sequenced behind PR #1175 + `session-start-token-audit` S2 (per §10.4).
