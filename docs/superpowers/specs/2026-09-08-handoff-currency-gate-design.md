...
| Hook registration is rendered from a tracked SSOT                                                             | `.ai-factory/harness-model.json` (tracked; `hooks` keys incl. `PreCompact`, `SessionStart`); `scripts/render-harness-config.mjs:148` owns ONLY `hooks` in settings.json; drift gate `packages/core/hooks/harness-config-drift.test.ts`            | a new hook is an SSOT edit + `--write`, and BOTH files land in the SAME commit. The deny-list `:56-57` denies the `Edit`/`Write` channel on settings.json, not the renderer's output — PR #1443 (`b22ce980e5`) landed `.ai-factory/harness-model.json` + `.claude/settings.json` together for exactly this reason, and its own message states that a model-only edit goes drift-RED. The operator's hand-action is the `env` arming (D18), never the render; `scripts/register-precompact-hook.sh` (122 LOC) is the recipe precedent for a machine whose settings.json lacks the block |
...
- **`packages/core/hooks/harness-config-drift.test.ts`** — unchanged; goes red if settings.json and the SSOT disagree, which is why the render lands in the same commit as the SSOT edit rather than as a hand-patch or a deferred operator step.
...
correct reading of the _present state_ and an incorrect reading of the _intent_. The intent is now
...
3. **Arming default** — whether `AIF_HANDOFF_GATE` stays opt-in once a writer exists. D18's
   rejected alternative (arm on `autoCompactWindow` presence) was rejected _because_ there was no
