...
> **Cross-umbrella coupling (`trust-tiers-did-review`, in-flight 2026-07-02 — SHARED FILE, must serialize):** that separate re-adjudication of `.claude/rules/research-source-trust.md §5` edits **`allowlist-resolver.ts` (+ `allowlist-resolver.test.ts`)** — its item-B `loadAckFile` single-label-host reject (`AckFileError`, same fail-closed family as `FF2014`). **D1 Task 3.2 edits the SAME file** (`validateProvenance` at `allowlist-resolver.ts:319` — its `reason`-string → `FF2xxx` migration). The two umbrellas therefore **share `allowlist-resolver.ts` → serialize, never parallel-edit**: land `trust-tiers-did-review` first, then D1 **rebases onto staging** and migrates the *then-current* reason set — the single-label reject becomes another `AckFileError`/`FF2014`-family code (fold into `FF2014` or allocate a sibling). Re-derive FF2xxx from the post-rebase resolver. (Correction of an earlier "zero file overlap" claim — that held only for PR #855's docs, NOT for the D1 code, which edits the resolver per §3.3.)
