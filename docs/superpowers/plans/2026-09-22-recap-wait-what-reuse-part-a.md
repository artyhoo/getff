...
returns `{ spellings: [], repeatedSaysLines: [] }` and `crossEntryDuplicates` returns `[]`.

- [ ] **Step 3: Watch it fail** — run
`PC_LOCAL=1 npx vitest run packages/core/principles/42-context-md-spelling-uniqueness.test.ts`.
Expected: FAIL in the sentinel and in the five «flags» / «compares» / «strips» / «treats» arms.
The real-tree arm and the «passes» / «ignores» arms pass on the stub.

- [ ] **Step 4: Implement — the final file content**
 ...
 * is a violation here. Stricter than the hook in one place: it skips an entry that has no
 * `**Term**:` definition, while this test still counts that entry's spellings.
 *
...
const splitSays = (payload: string): string[] =>
  payload.replace(/\.$/, '').replace(/[«»]/g, '').split(',').map((w) => w.trim()).filter((w) => w !== '');
...
- [ ] **Step 5: Run it** — the Step 3 command. Expected: PASS, 10 tests. Then
`PC_LOCAL=1 npx tsc --noEmit -p packages/core` and `PC_LOCAL=1 npx eslint <the test file>`.
Expected: both exit 0.

- [ ] **Step 6: Prove the real-tree arm can fail** — with Edit, change line 86 of `CONTEXT.md`
...
```yaml
docs-refresh: deferred — re-verified 2026-09-22: the cited register gained rows 284-286 and a dated note on row 283, and CLAUDE.md gained one skill-routing line for domain-modeling; counts are renderer-owned and no prose here depends on either; clears at the next gold refresh of this page
```
