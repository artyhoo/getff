...
> **BUILT 2026-09-05 (npm-publish-getff-init S1+S2).** `packages/getff/` exists: `bin/getff` (`init` → the shipped `setup`), `files` = the ten payload roots, assembler [`scripts/build-getff-dist.sh`](../../scripts/build-getff-dist.sh) (copies the `git ls-files` payload at `prepack`, commits `MANIFEST.sha256`, `--check` = drift gate + `files`-coverage gate), consumer-matrix cell [`tests/consumer-matrix/getff-dist-cell.sh`](../../tests/consumer-matrix/getff-dist-cell.sh) (`npx getff init -y ts-server` from the installed tarball → the planted R2 violation fails eslint) wired into `ci-success.needs`. The three «still open» items below are decided there: explicit script **and** `prepack`; `files` set = every `PKG_ROOT` root, RED-provable through the cell's opt-in arms; version coupling deferred with the `@getff/core` publish (U9 — the beta publishes `getff` only, `packages/core` rides inside the tarball).
...
>
> Related and also out of R1 scope: `packages/core` declares **no** `peerDependencies` while four of its six siblings do — `preset-react-spa/package.json:15`, `preset-next-15-canonical/package.json:15`, `preset-react-native/package.json:11`, `meta-factory/package.json:13` (the remaining two, `lint-config` and `runtime-bridge`, declare none). Whichever option U10 picks, the peer-tier decision travels with it.
