...
| **D** | WARN emission | **one combined `_emit_warn` call** accumulating all drifted stacks (single JSON object — two would break ZCode `JSON.parse`, §3a M2), through the DH-S1-introduced `_emit_warn` (L21-27) | kickoff §1 line 29; R-phase §2; §3a M2 |
...
2. **`pyproject.toml` detection + two-tier extraction** (design §4 C-resolution): Tier-1 table-boundary hash of the **6 non-`[project]`** dep tables — `[project.optional-dependencies]` + `[dependency-groups]` + `[tool.poetry.dependencies]` + `[tool.poetry.dev-dependencies]` + all `[tool.poetry.group.*.dependencies]` + `[tool.hatch.envs.*]` (each if present). `[project]` itself is excluded from Tier-1 (its own name/version metadata would cry-wolf on every release) and covered deps-only by Tier-2 `tomllib` (`[project].dependencies` + `[project].optional-dependencies`).
...
- **Python 3.10→3.11 upgrade + `[project]`-bearing pyproject (round-3.5 review B2):** when tomllib appears (3.11+), Tier-2 starts hashing `[project].dependencies` for real. For a pyproject WITHOUT `[project]` deps, this is invisible — the hook uses a sentinel (`sha256("[][]")`, the exact no-`[project]` Tier-2 value) when tomllib is absent, so the upgrade does not shift such baselines. For a pyproject WITH `[project].deps`, the baseline DOES shift on upgrade (real coverage change — honest re-baseline). Accepted: this is the only residual cry-wolf source, it fires once per python-upgrade on `[project]`-bearing consumers, and it is honest (the deps surface genuinely gained coverage). Documented; not closed by design.
- **CRLF / Windows line endings (round-3.5 review B1):** the Tier-1 awk strips `\r` via `gsub(/\r/,"")` before the header parse, so a Windows consumer's CRLF `pyproject.toml` hashes identically to a Unix LF one. Verified by the `PYTHON-CRLF` regression test (round-3.5). Without the gsub, `substr($0,2,length-2)` on `[hdr]\r` leaked `]` and silently dropped 5/6 tables.
- **First-run migration for peerDeps-bearing consumers (round-2 M3):** reading the legacy `deps-hash:` key as the npm slot does NOT reconcile the VALUE — the legacy value is a 2-field (`dependencies`+`devDependencies`) hash; DH-S1 recomputes a 7-field hash. A consumer whose `package.json` declares `peerDependencies`/`overrides` (common — React libs, ESLint plugins) emits a "package.json deps changed since last tool-bootstrap" WARN on first run after upgrade — this is **honest, not spurious**: the deps surface genuinely widened. The WARN wording is the "deps changed" branch (the legacy value is a real `sha256-*`, so the `case` statement's `sha256-*` arm fires). Consumers with none of the new JS fields are unaffected (7-field merge is byte-identical to the 2-field merge → match → silent). Re-baseline via `/tool-bootstrapping` silences it. (Clarified post round-3.5 review: the "not yet baselined" wording is reserved for the install-seed `<pending …>` placeholder, which is the non-`sha256-*` `case` arm — GH #548.)
