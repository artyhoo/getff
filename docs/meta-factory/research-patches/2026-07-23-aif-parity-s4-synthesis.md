...
| `inject-output-language.sh` | inject | NOT-REGISTERED — **by design** (§5 Q2) | n/a | ◻️ COVERAGE-LIMITED (would-reach-it: interactive operator session) | S2 §2.2, §4.4 |
| `inject-memory-codification.sh` | inject | NOT-REGISTERED — **by design** (§5 Q2) | n/a | ◻️ COVERAGE-LIMITED (would-reach-it: host operator session with a memory store) | S2 §2.2, §4.4 |
| `inject-project-digest.sh` | inject | NOT-REGISTERED — low-value for this carrier (§5 Q2) | n/a | ◻️ COVERAGE-LIMITED | S2 §2.2, §4.4 |
| `check-worker-dispatch-channel.sh` | shield | NOT-REGISTERED — **by design** (§5 Q2; dispatcher-side gate) | n/a | ◻️ COVERAGE-LIMITED (would-reach-it: a session authoring kickoffs) | S2 §2.2 |
...
- ❌ **BROKEN: 0.**
- ◻️ **COVERAGE-LIMITED: 18** — with a named would-reach-it carrier each (below).

> **Reclassification note (honest, not a softening).** The worker's own §7 checklist graded three unregistered `inject-*` hooks ❌ BROKEN, and this file did too until the §5 Q2 analysis landed. `BROKEN` means *«should work here and does not»*; the Q2 analysis establishes that these mechanisms are **not applicable to an autonomous container carrier at all** (no operator to address, no memory store, no kickoffs authored) — one of them, `inject-output-language`, would be actively harmful if registered. Inapplicability is `COVERAGE-LIMITED` with a named would-reach-it carrier, not breakage. The counts moved because the evidence sharpened, not because a red row was made comfortable — the ⚠️ WORSE rows, which ARE real defects on this carrier, are untouched.
...
- **Q2 (5 shipped-but-unregistered hooks — intentional or drift?) — RESOLVED BY ANALYSIS.** The question was re-framed by the operator from «what was intended» (knowable only to a human) to «what *should* be true for aif to work well» — which IS answerable from what each hook does:

  | hook | should it run in an autonomous container worker? | why |
  |---|---|---|
  | `inject-output-language.sh` | **NO — correctly absent** | injects the operator-facing reply language. A container worker addresses no operator and writes repo artifacts, which [`language-discipline.md §1`](../../../.claude/rules/language-discipline.md) requires to be English. Registering it would actively push the worker toward non-English commit/PR/file content — **harmful, not merely useless.** |
  | `inject-memory-codification.sh` | **NO — correctly absent** | nudges codifying conventions into the operator's per-machine memory dir; a container worker has no such store. |
  | `check-worker-dispatch-channel.sh` | **NO — correctly absent** | a dispatcher-side gate on kickoff text; a worker authors no kickoffs. |
  | `inject-project-digest.sh` | **optional — low value here** | the kickoff already carries scoped context; a digest spends tokens without changing a decision. |
  | `inject-subagent-context.sh` | **YES — but only once workers spawn subagents** | this carrier stayed top-level so it changed nothing; it becomes load-bearing the moment a worker delegates. |

  **Verdict: «drift» is the wrong frame.** 3 of 5 are correctly absent (one would be actively harmful), 1 is low-value, 1 is a real gap only for a subagent-spawning worker. The container's inject-set is defensible as-is; the genuinely actionable container defect is the missing dependencies (§8 P0), not these registrations.
...
## §8 Actionable fixes (prioritised; NOT applied here — scope discipline)

Ordered by how much silent enforcement each restores. Per [CLAUDE.md](../../../CLAUDE.md) PR strategy these are **surfaced, not autonomously opened as PRs**.

1. **P0 — install `jq`, `gh`, `python3` in the aif container image.** Single root cause behind the most rows: `check-doc-authority` skips its own check (⚠️ WORSE), and the pre-commit JSON + YAML validity gates silently skip (◻️ ×2). A gate that is *registered but dependency-less* is the worst failure mode in this codebase's model — it reads as alive in every settings audit while enforcing nothing.
2. **P1 — emit PostToolUse violations via JSON `hookSpecificOutput.additionalContext`, not bare stderr.** Fixes the invisibility half of the headline finding and would make the five SILENT shields observable at all — retiring §4's most-uncertain row. `inject-matching-rule.sh` already proves the channel works.
3. **P1 — reconcile the fidelity anchor with API-harvest** (§3 item 5): either run the cold audit *after* harvest, or let `pr-body-fidelity` accept an `Audited-SHA` whose tree matches PR head.
4. **P2 — pre-warm the npm cache in the container image** so markdownlint is usable on round 1 without the agent discovering a workaround.
5. **P2 — exact-match short-circuit before substring in the runtime-profile-name resolver** (`AifHandoffBackend.ts:131`), so a marker naming a real profile can never resolve ambiguously.
6. **P3 — register `inject-subagent-context.sh`** if and when container workers begin spawning subagents (§5 Q2); no action while carriers stay top-level.
