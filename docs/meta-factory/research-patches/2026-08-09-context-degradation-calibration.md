...
> **Revision 4 (2026-08-10, operator-directed):** scope is deliberately narrow — **re-run the Fable-5 / Opus-5 tier sweep only**, land whatever the fresh sources support, and stop. No new audit round was requested and none was run; the numbers below enter under the same downstream consumption gate as rev 3's. What changed: the rev-2 `COVERAGE INSUFFICIENT` disposition on **Fable 5 is now partly discharged** — rows 1.10–1.13 add the first *numeric*, token-axis evidence for the top tier and for judgment-class degradation (1.10 and 1.11 dated 2026; 1.12 **undated** and directional only; 1.13 dated 2026 but numerically empty), so [§search-coverage](#search-coverage) item F no longer reads «none numeric». Consequence for D6: the `T_soft(1M)` = **300k** floor stops being purely operator-experience-derived and acquires a directional external anchor. The 500k mechanical ceiling stays **unbounded for its own task class** — no rev-4 source measures mechanical-tail work at any fill; 1.10's ~600k is a *judgment*-class ceiling and importing it here would be a T16 class-mismatch (RQ2's direction says mechanical tolerates more, not less). **No threshold moved.** *(Round-3 delta audit returned `REVISE` on two MAJORs — an undated source counted as dated, and exactly that ~600k class-transfer stated as a falsifier; both are closed in this revision and the corrections are recorded inline where they land.)*
...
peer-review-track 3 (1.8, 1.9 added rev 2; 1.11 added rev 4). Dated **2026: 11 of 13.** Row 1.8 is
**2025-11** — pre-2026, directional only, never a threshold source (as is the AWS 1M-GA link in §0,
2025-08, which is not an RQ1 row). Row 1.12 carries **NO publication date at all** and is therefore
excluded from the dated count rather than absorbed into it: the page returned HTTP 429, so no date
could be read off it. 0 `INCONCLUSIVE-needs-human`.

**Fetch-verification status (rev 4, T3).** Rows 1.10, 1.11, 1.13 were fetched and their figures read
off the page. Row **1.12 was not** — `morphllm.com` returned HTTP 429 — so it fails the §3 freshness
bar on two counts, not one: its band is search-snippet-level *and* it carries no verifiable date. It
is kept as directional corroboration only, is labelled as such everywhere it is used, and is counted
as undated above. No rev-4 number is load-bearing on it. *(Rev 4 round-3 correction: the first cut of
this paragraph disclosed the failed fetch but the recount above still counted 1.12 as «dated 2026» —
the exact W-7 shape, an aggregate predicate asserting precision its own row did not carry. Caught by
the round-3 delta seat.)*
...
| 1.12 *(rev 4)* | Third-party synthesis over the Chroma 18-model context-rot corpus: for 1M-window models «a clearly observable effect typically kicks in somewhere around 300,000–400,000 tokens»; distinguishes **positional** degradation (lost-in-the-middle, U-shaped, 20–30 points lower mid-window) from **length** degradation (accuracy falls as input grows with evidence fixed and favourably placed), with drops «sometimes by 30 to 50 percent well before the documented limit» | Morph, `morphllm.com/context-rot` — **snippet-level only; NOT verbatim-verified; PUBLICATION DATE UNKNOWN.** Direct fetch returned **HTTP 429**, so no date could be read off the page and none is asserted here; the quoted band is as surfaced by WebSearch, and the underlying Chroma corpus it re-reports is **2025**. Excluded from the dated-2026 count in the coverage recount above | Third-party aggregator, **unverified fetch, undated** | PARTIAL — cross-vendor, mixed task classes, and it re-reports a pre-2026 corpus. **It is the only source in this patch that names a token band coinciding with D6's 300k floor**, which makes it precisely the source not to lean on: aggregator class + failed fetch = directional corroboration, never the anchor. Re-fetch and re-verify before any consumer treats 300k as externally sourced |
...
| Working ceiling (mechanical tails) | ~500k | **500k retains** as PROVISIONAL ceiling for mechanical-tail work only — **still not externally bounded for this task class** | Wrong if mechanical-tail quality degrades measurably before 500k under in-house measurement. Rev 4 adds a **cross-class** upper caution, NOT a falsifier for this row: raising the ceiling past **~600k** would exceed 1.10's effective-reasoning estimate — but 1.10 measures **judgment-class** work (multi-document synthesis, multi-needle), which per RQ2 is the *harder* class, so it cannot falsify a **mechanical-tail** number in either direction | Coverage still INSUFFICIENT to derive the value (T14) — operator-experience-derived, not benchmark-derived, and **no rev-4 source measures mechanical-tail work at any fill.** ⚠️ **Task-class transfer is UNVERIFIED (T16):** 1.10's ~600k is a judgment-class ceiling; RQ2's measured direction is that gated/mechanical work tolerates *more* context than judgment work, so if the transfer holds at all it points to a mechanical ceiling **at or above** ~600k, not at it. Treating ~600k as this row's bound would import a harder class's limit onto an easier one. Directional caution for not raising 500k casually; **not** an external bound, and **not** a licence to raise it |
...
Condition **(c)** (<5 dated 2026 sources) did NOT fire — of the **30** distinct URLs now cited (26 as
of rev 3, plus rev 4's four), well over 5 carry a 2026 date, comfortably above the floor. *(Rev 3: rev
2 wrote «16 of the 21», carrying rev 1's URL count into a rev-2 sentence — the miscount its own next
bullet was written to fix. Rev 4 round-3: the same shape again — rev 4 added four URLs and left «26»
standing, so the count is refreshed here rather than inherited a third time.)* (Kickoff §6(c) is a floor, not a stopping point — see
