...
The consumer-facing UserPromptSubmit hook `packages/core/hooks/deps-hash-check.sh` (**72 lines** on staging — re-audited 2026-07-17 post-#1018; this doc's line-count has raced #1018 twice: the original 2026-07-16 said «72 lines», a first re-audit «corrected» it to «60 lines / no `_emit_warn`» which was true only in the pre-#1018 window, and #1018 then landed `_emit_warn` bringing it back to 72 with the function present — so the file is 72 lines AGAIN, now WITH `_emit_warn`; pure bash, `set -uo pipefail`) is the project's **staleness detector**: at each session start it sha256-hashes the consumer's declared dependencies and compares against a stored baseline in `.ai-factory/tool-decisions.md`; on mismatch it prints a one-line WARN via **`_emit_warn`** (shipped #1018, call-sites L65 drift / L68 unbaselined, def L21-26) into session context (the harness auto-injects stdout) telling the agent to re-run rule-research. This is the mechanism that realizes [README.md#why-this-exists](../../../README.md#why-this-exists) — «generated executable rules don't go stale» — at the earliest reachable channel (session start, not CI).

It is **deliberately JS-only today**, and that limitation is structural, not cosmetic:

- **L39-40 (re-audited 2026-07-17 post-#1018):** `[ -f package.json ] || exit 0` (L39) and `if ! command -v node >/dev/null 2>&1; then exit 0; fi` (L40) — the hook **hard-exits** when `package.json` is absent or `node` is not on PATH. On a python or rust consumer this means the staleness nudge is **silently never injected**.
- **L42-45 (the deps surface):** `node -e` reading `package.json`'s `dependencies` + `devDependencies`.
...
### Both tiers MUST route the verdict through `_emit_warn` (SHIPPED on staging via #1018)

> **Re-audit 2026-07-17 post-#1018 — this paragraph has now raced #1018 twice.** (1) The original 2026-07-16 text claimed `_emit_warn` was "already harness-portable" at `deps-hash-check.sh:21-27` — that was fabricated against the then-staging (the function was absent). (2) A first re-audit then "corrected" to "NO `_emit_warn` on staging, DH-S1 introduces it" — that was correct only in the pre-#1018 window. PR #1018 (zcode-parity) landed `_emit_warn` on staging (def L21-26, call-sites L65/L68) shortly after, so BOTH prior framings are now stale. **Current truth: `_emit_warn` SHIPS; DH-S1 REUSES it (does NOT introduce it).**

`_emit_warn` (L21-26) is harness-portable: under `ZCODE_PROJECT_DIR` + `jq` it emits strict-JSON `{hookEventName:"UserPromptSubmit", additionalContext:$c}`, else plain `⚠ …`. The two existing WARN call-sites (L65 drift, L68 unbaselined) already route through it. Any new stack path must route its mismatch verdict through this same function so CC-plain vs ZCode-JSON output stays unified. The always-exit-0, non-blocking contract (L73) holds.
...
**Option B — three hooks (one per stack) + a shared lib.** Cleaner per-stack isolation. Cons: **`install.sh` deliberately ships the hook standalone with NO `lib/`** (`install.sh:421-422` — re-audited 2026-07-17; the original 2026-07-16 text quoted `deps-hash-check.sh:20` «inlined, not sourced from lib/, because install.sh ships this file standalone to consumers», but that quote does not appear anywhere in the hook file — `grep "inlined, not sourced"` = empty — it was a paraphrase presented as a direct quote; the real standalone-ship logic lives in `install.sh:421-422`). Introducing a shared lib breaks this principle and adds a delivery-surface abstraction; three hooks triple the dual-pair byte-identity surface.
...
- `_emit_warn` REUSED (SHIPPED #1018, L21-26) as the single emit path for all stacks — the two existing call-sites (L65/L68) already route through it; DH-S1 extends the same path to the new stacks (it does NOT introduce `_emit_warn` — that framing raced #1018 and is stale).
...
- **REUSE:** the existing `deps-hash-check.sh` (the substrate, 72 lines on staging post-#1018), its `sha256sum`/`shasum` BSD/Linux fallback (**L48-52** — re-audited 2026-07-17 post-#1018; this line-ref has also raced #1018 twice), the `install.sh:421-422` standalone-ship contract, the `deps-hash-check.test.ts` dual-pair oracle pattern, AND the **`_emit_warn` helper (SHIPPED #1018, L21-26)** — the original 2026-07-16 text's "pre-existing `_emit_warn`" claim was fabricated against then-staging, but #1018 has since made it true; DH-S1 reuses it rather than introducing it.
...
- **Forward-check:** complies with [no-paid-llm-in-ci.md](../../../.claude/rules/no-paid-llm-in-ci.md) (the hook is a bash UserPromptSubmit context-injector; Tier-2 `tomllib`/`cargo metadata` are free stdlib/toolchain reads, zero paid-LLM/API); [doc-authority-hierarchy.md §2-3](../../../.claude/rules/doc-authority-hierarchy.md) (carries scope annotation + folder-authority header, claims authority for nothing beyond the gap); [build-first-reuse-default.md](../../../.claude/rules/build-first-reuse-default.md) (REUSE — extends the existing hook, introduces no parallel surface); [ai-laziness-traps.md](../../../.claude/rules/ai-laziness-traps.md) T3/T11 (every deps-field claim cites a spec; every node-free claim cites `install.sh:196-198` + `deps-hash-check.sh:27-28` (re-audited 2026-07-17; original cited «L39-40», corrected); the architecture fork is surfaced not silently resolved).
- **Backward-check:** class of this change = **additive extension of an existing single-stack detector to multistack**. Sibling surfaces where the same change-class would apply: the *response* side (tool-bootstrapping skill's «re-run rule-research» — does it know about per-stack baselines? it reads the same `tool-decisions.md`, so it composes), and the *rust delivery lane* (does not exist yet — this hook detects-only, so no collision). It supersedes nothing: the legacy bare `deps-hash:` key is read backward-compatibly as the npm slot.
- **Self-application (T15):** this patch applied its own «measure the real constraint, do not assume» discipline — the node-free claim is verified against the shipped `install.sh`/`lib.sh`, not assumed from the kickoff prose.
