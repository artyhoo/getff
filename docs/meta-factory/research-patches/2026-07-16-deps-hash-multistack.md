...
### Both tiers route the verdict through the existing `_emit_warn`

`deps-hash-check.sh:21-27` `_emit_warn` is already harness-portable: under `ZCODE_PROJECT_DIR` + `jq` it emits strict-JSON `{hookEventName, additionalContext}`, else plain `⚠ …`. Any new stack path must route its mismatch verdict through this same function so CC-plain vs ZCode-JSON output stays unified. The always-exit-0, non-blocking contract (L72) holds.
...
**Option B — three hooks (one per stack) + a shared lib.** Cleaner per-stack isolation. Cons: **`install.sh` deliberately ships the hook standalone with NO `lib/`** (`deps-hash-check.sh:20` «inlined, not sourced from lib/, because install.sh ships this file standalone to consumers»). Introducing a shared lib breaks this principle and adds a delivery-surface abstraction; three hooks triple the dual-pair byte-identity surface.
...
- **REUSE:** the existing `deps-hash-check.sh` (the substrate), its `_emit_warn` harness-portable emitter, the `sha256sum`/`shasum` BSD/Linux fallback (L48-54), the `install.sh` standalone-ship contract, and the `deps-hash-check.test.ts` dual-pair oracle pattern.
