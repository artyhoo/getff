...
The consumer-facing UserPromptSubmit hook `packages/core/hooks/deps-hash-check.sh` (60 lines — **re-audited 2026-07-17; the original 2026-07-16 «72 lines» claim was +12-drift, T3 self-violation corrected**, pure bash, `set -uo pipefail`) is the project's **staleness detector**: at each session start it sha256-hashes the consumer's declared dependencies and compares against a stored baseline in `.ai-factory/tool-decisions.md`; on mismatch it prints a one-line WARN via a bare `printf` (L52 drift / L55 unbaselined — **no `_emit_warn` wrapper exists on `staging` today; DH-S1 deliverable (0) introduces it**) into session context (the harness auto-injects stdout) telling the agent to re-run rule-research. This is the mechanism that realizes [README.md#why-this-exists](../../../README.md#why-this-exists) — «generated executable rules don't go stale» — at the earliest reachable channel (session start, not CI).

It is **deliberately JS-only today**, and that limitation is structural, not cosmetic:

- **L27-28 (re-audited 2026-07-17; original 2026-07-16 cited «L39-40», +12-drift corrected):** `[ -f package.json ] || exit 0` (L27) and `if ! command -v node >/dev/null 2>&1; then exit 0; fi` (L28) — the hook **hard-exits** when `package.json` is absent or `node` is not on PATH. On a python or rust consumer this means the staleness nudge is **silently never injected**.
- **L30-33 (original cited «L42-46», corrected):** the deps surface is `node -e` reading `package.json`'s `dependencies` + `devDependencies`.
...
### Both tiers MUST route the verdict through a single `_emit_warn` (introduced by DH-S1 deliverable (0))

> **Re-audit 2026-07-17 — CORRECTION.** The original 2026-07-16 text claimed `_emit_warn` was "already harness-portable" at `deps-hash-check.sh:21-27`. That was **fabricated**: no `_emit_warn` function exists in the `staging` hook (`grep -n _emit_warn packages/core/hooks/deps-hash-check.sh` = empty), no `ZCODE_PROJECT_DIR`/`jq`/`additionalContext`/`hookEventName` tokens appear anywhere in the file, and L21-27 actually contain `STORED_HASH=` parse + two `exit 0` early-returns. The `staging` hook emits its WARN via two **bare `printf`s** at L52 (drift) and L55 (unbaselined). DH-S1 deliverable (0) introduces the helper as NEW code with the JSON/plain fork. The architectural recommendation (single emit path for both tiers) still holds — what was wrong was the "pre-existing" framing.

The DH-S1-introduced `_emit_warn` will be harness-portable: under `ZCODE_PROJECT_DIR` + `jq` it emits strict-JSON `{hookEventName:"UserPromptSubmit", additionalContext:$c}`, else plain `⚠ …`. Any new stack path must route its mismatch verdict through this same function so CC-plain vs ZCode-JSON output stays unified. The always-exit-0, non-blocking contract (L60 — original wrongly cited «L72», file is 60 lines) holds.
...
- `_emit_warn` helper introduced (DH-S1 deliverable (0)) and used as the single emit path for all stacks — routing the two existing bare `printf`s (L52/L55) through it in the same change.
...
- **REUSE:** the existing `deps-hash-check.sh` (the substrate), its `sha256sum`/`shasum` BSD/Linux fallback (**L36-42** — re-audited 2026-07-17; the original 2026-07-16 text cited «L48-54», +12-drift corrected), the `install.sh:421-422` standalone-ship contract, and the `deps-hash-check.test.ts` dual-pair oracle pattern. **The `_emit_warn` helper is NOT REUSE — it is NEW (DH-S1 deliverable (0)); the original text's «`_emit_warn` harness-portable emitter» REUSE-claim was fabricated (no such function exists on `staging`).**
