...
- REPO probes run in the aif container against `origin/staging` = `aa87d0a47a6d8502f983cc9fe7284bd5dcb3d650` (fetched fresh 2026-09-12: `git fetch origin staging` → rc=0; `FETCH_HEAD == origin/staging == HEAD` verified — all three `aa87d0a47a…`). Probes quoted verbatim; verdict form per T3: CONFIRMED / FALSIFIED / INCONCLUSIVE-needs-HOST.
...
**F6 — §5 recount preview.** `plugin/hooks/` currently carries 17 hook files; the doctrine :118 recount pattern (`grep -vE '^_zcode-emit|^hooks.json|^lang|^run-hook.cmd|^session-start'`) excludes plugin-intrinsic `session-start` (no `.claude/hooks` source) and counts **16** framework twins today. Landing row 3 → **17**. The doctrine sync rewrites that count and its `ls` command is unchanged.
...
| Hooks | 17 files in `plugin/hooks/` (16 doctrine-counted framework twins + plugin-intrinsic `session-start`) + helpers (`hooks.json`, `run-hook.cmd`, `_zcode-emit`, `lang/` ×3, `lib/hook-emit.sh`) | row-3 twin is THIS umbrella's payload gap; the rest live via the plugin channel (doctrine §5) |
...
- T11 (build-vs-reuse SSOT consult, planning-time): `awk -F'|' '/^\| [0-9]+ /…' docs/meta-factory/prior-art-evaluations.md | grep -iE "zcode|plugin|marketplace"` → rows #2, #4, #34, #37, #49, #84, #121, #122, #134, #136 — **no row covers a ZCode consumer install path, marketplace publication, or manifest-slot decision**; nearest is #84 (CC `claude plugin install` CLI — CC channel, not ZCode). No capability commit is made in Stage 0/1 (docs only); the consult is recorded here for Stage 2's commit. ✓
...
- `zcode-parity-doctrine.md §2 row 3 / rollup / §4 row 3 / §5` — GAP-FOUND (row 3 still `plugin-gap`, :45/:66/:109; §5 count 16, :118) → this is Stage 2's payload; the doctrine remains the SSOT until then.
