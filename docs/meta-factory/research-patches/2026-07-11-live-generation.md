...
> **Post-merge status note (added 2026-07-17, archival convention — the 2026-07-11 body below is NOT rewritten, see census-patch §5):** this R-phase is a **pre-dispatch snapshot** — several of its load-bearing technical claims describe the state *before* the umbrella's stages landed and are now historically stale: (a) §0 Plane 3 «NO `live-research → ConventionNode` bridge exists» — the python bridge `packages/core/synthesizer/research-to-node.ts` shipped in LG-S1 (#1005); the rust sibling `research-to-clippy-node.ts` in LG-S3 (#1010); (b) §Qc «Python + Rust Tier-0 doc hosts absent → fails-closed» — python + rust Tier-0 keys were added as DATA in #1005/#1010; (c) §Qf «delivery seam does not exist yet» — python-delivery-v0 S3 merged (#997); (d) §Forks-2 was RESOLVED in #1006 (leave-as-is now, converge LATER — no longer «parked»); (e) the §Qf LG-S4 deps-hash-staleness scope was EXTRACTED into its own `deps-hash-multistack` umbrella (#1016 kickoff; the LG-S4 §2 scope pointer landed in #1017). **The live, authoritative stage status lives in the kickoff** ([`.claude/orchestrator-prompts/live-generation/kickoff.md`](../../../.claude/orchestrator-prompts/live-generation/kickoff.md), re-audited 2026-07-17 via #1021) — treat this R-patch body as the historical research record, not current state.
...
- Live on staging: `synthesizer/file-clients.ts` (both `FileResearchClient` + `FileGenerateClient`), `install/rule-bootstrap-cli.ts:56-57` (`--from-research`/`--from-selection`), `synthesizer/rule-bootstrap-live.test.ts`, `agents/rule-researcher.md`, `.claude/skills/rule-research/SKILL.md`.
