...
## §0 — Verified status snapshot (reconciled 2026-09-10)

> Verified against merged PRs (`gh pr list --state merged --search "merged:2026-06-29..2026-08-11"` + `"merged:2026-08-12..2026-09-10"` — 518+263 = 781 PRs in range, staging tip #1697) + per-umbrella `done.md` presence + `npm view getff version`. Supersedes any «pending/next» framing in older prose below — mirrors the `#stale-claim` correction the niche roadmap §STATUS-RECONCILIATION already needed once.
...
| **`universalization-fix` umbrella** (project-philosophy audit 2026-06-14 follow-on) | layout-coupling residue NOT closed by #511 (which closed #507/#508/#509 at the ESLint-glob+depcruise+nvmrc layer): **S1** probe-level silent-PASS reported green checking zero files — R4 (`packages/core/audit-self/audit-ai-docs.ts` on no `src/domain`), R17 (`audit-ai-docs.react-next.sh`), `test:integration` `src/**` glob → now WARN-not-PASS · **S2** `check-rule-globs` into **pre-push** (earliest-reachable channel; #511 had wired only validate+CI) · **S3** single `resolve-source-roots` seam (R-phase: ADOPT workspace-root resolver?) replacing N hardcodes · **S4** README "works as-is" layout-honesty | ✅ **DONE 2026-06-29** — S1 (#515), S2 (#517), S4 (#514), S3 `resolve-source-roots` seam (#526) all merged. Kickoff `.claude/orchestrator-prompts/universalization-fix/kickoff.md` | #515 (S1) / #517 (S2) / #514 (S4) / #526 (S3) |
| **`zcode-parity` program** | hook twins + harness-config renderer + ZCode-vs-CC doctrine; escalated 2026-08→09 across the parity thread | 🟡 **in-flight** — doctrine rows 21–22 refreshed by the compaction-hook verification (#1696, 2026-09-09: ZCode compaction ships without a hook lifecycle — doc-ahead-of-runtime corrected); `zcode-parity-step1-emit-wrapper` I-phase authored, execution pending; **`zcode-full-parity-mega-umbrella` R-phase (~L) open** — the umbrella-scale parity survey that decides the parity end-state | 16 `zcode-parity` PRs in #810–#1697 · #1696 · open kickoffs in `<orch-home>/zcode-full-parity-mega-umbrella/` + `zcode-parity-step1-emit-wrapper/` (no done.md, no merged closure) |
| **`consumer-truth-audit` umbrella** | five verification lanes (V0–V4) over three real consumer installs: has / delivered / works | 🟡 **in-flight** — V0 delivery census ✅ (#1682) · V2 multi-stack report + addendum ✅ (#1688/#1690) · V3R report landing ✅ (#1692, 2026-09-09) · **V1/V4 lanes authored (#1674), execution not started** (no V1/V4 PRs in the #810–#1697 corpus) | #1674 (kickoffs) · #1682 · #1688/#1690 · #1692 · no done.md |
| **`beta-docs-showcase` (BS0–BS3)** | public docs site on Fumadocs — the beta showcase surface | 🟡 **BS0–BS3 kickoffs all merged, site NOT deployed** — BS0 leg A #1561 · BS1 skeleton #1579 · BS2 content #1610 · BS3 cutover stage #1670; the `fumadocs-migration` deploy branch remains unmerged and the site 404s (cache 2026-09-08: `getff.ai/docs` 404; only kickoff PRs match `fumadocs` in `gh pr list`) | #1561/#1579/#1610/#1670 · no done.md |
| **`npm-publish-getff-init` (getff-to-prod U10)** | real `getff` package on npm (`npx getff init`) | 🔲 **NOT blocked — execution pending** — U10 stub expanded into the full stage kickoff (#1611); `npm view getff version` → **`0.0.1` placeholder** (verified 2026-09-10; the distribution-package work itself landed as #1613 but the publish step never ran) | #1611 (kickoff) · #1613 (dist package) · `npm view getff version` = 0.0.1 |
...
**What actually remains (as of 2026-09-10):** **no open PRs** (gh staging tip #1697); the active thread is the **beta-launch floor**: (1) **BS3 docs-site deploy** (`beta-docs-showcase` 🟡 — kickoffs merged, site undeployed); (2) **npm publish getff** (`npm-publish-getff-init` 🔲 — npm holds placeholder 0.0.1); (3) **promote staging→main #4** (~100 PRs accrued since promote #3, #1597, 2026-09-04); (4) **`/code-review` ultra-1597 deep reviews** — 0 done (maintainer-owned per rule 3; no review PRs surface in the #810–#1697 corpus). In-flight audits: `consumer-truth-audit` V1/V4 lanes · `zcode-parity` mega-umbrella R-phase + step1-emit-wrapper. Carried OPEN-PARTIAL (do NOT mark closed): `modular-install-fullpack` S5 Timeliner acceptance · `shipped-artifact-liveness-gap` §8 Ph2–3 · `getff-to-prod` U4–U17 (U10 materialized #1611) · `phase-10-foundations-audit` DECISION-NEEDED triage · `frontier-residue-sweep` S4/S5 parked-until-post-beta (#1609). Standing/deferred (unchanged): N2 application (maintainer-owned) · N7 organic SDD run · N8 C5 + cost-levers (trigger-gated) · `meta-orch-no-arg-overview` Stage 3 UX · Track M.5/M.6 · N4b H10 · N5 (blocked on N7). **Closure debt:** ~45 landed umbrellas lack `done.md` (priority-score 2026-09-10: 58 non-DONE candidates, ~45 are landed-but-unmarked, e.g. `getff-freshness-widening` S0/S1/S1b merged #1275–#1354) — backfill is its own sweep, not folded into reconcile rounds by default.

**Open frontier (live, as of 2026-09-10 — `gh pr list --search is:open` = [] + `/pipeline` no-arg completion-filter + per-umbrella done.md/npm verification): the beta floor, not a build-umbrella.** Nothing is mid-merge; the next-actions are operator floors (deploy docs site, publish npm package, click promote #4, run the ultra-1597 review convention) plus two in-flight audits (`consumer-truth-audit` V1/V4, `zcode-parity` R-phase) that are dispatchable now in parallel. After the floor: `getff-to-prod` U4–U17 unblocks (npm publish is its U10 dependency) and the OPEN-PARTIAL set. No clean single next-thread — maintainer picks; `/pipeline` no-arg 2026-09-10 ranked the floor items Wave 1.

**Closed since the 2026-06-29 snapshot (#810→#1697, 781 merged PRs — major umbrellas, individually verified via done.md):** `beta-delivery-ux` (#1358 done.md; 21 PRs, tier-home + degradation matrix) · `beta-ai-docs-agnosticism` (#1569 done.md; 17 PRs) · `arch-v2 context-pipeline` (#1308 done.md; design + S-A…S-G #1189–#1223; s-f/s-m stage kickoff dirs remain as unexecuted leftovers, not open work) · `triage-kernel-v2` (#1405 done.md) · `first-commit-passable` FC-1…FC-4 (#1555 done.md, capstone #1596) · install ultrareview P0/P1 batch (#942–#957) · `token-economy` S-A/S-B (#1203/#1204 done.md) · `getff-honest-signals` s1–s6 (#1175 done.md) · `getff-any-stack-trace` r1+s1–s4 (#1262 done.md) · `live-generation` (#1055 done.md) · `promote-gate-fixes` (#1600 done.md) · **promote staging→main series #2/#3** (#936, #1597 on 2026-09-04) · `getff` distribution package (#1613) · `ledger-1597-fixes` F1–F11 (#1620–#1681, done.md #1669) · `handoff-currency-gate` D13 (#1672–#1680, done.md #1680/#1681) · `aif review-contract recovery` (#1693–#1697, closed after GO 2026-09-10 — auto-review loop hardening + review-state return channel) · `zcode compaction doctrine fix` (#1696) · `getff-freshness-widening` S0/S1/S1b (#1275–#1354 — work complete, done.md backfill debt). Long tail: ~60 install/hooks/kickoff-scoped hardening PRs without a single umbrella home (consumer scoping, blob-URL links, lane-aware orphan scan, deps bumps) — no open claims attached.

*(Closed since the 2026-06-13 snapshot (#518→#809 — major umbrellas, not individually rowed above):* multi-stack react-spa/react-native preset wiring (#713/#714, closes #646) · `modular-install-fullpack` S1–S4 (#719/#723/#724/#725; S5 open) · `generator-forbid-mvp` (#696/#697) · `stage-2-generate-path` (#691/#692) · `generation-paths-comparison` (#700) · `generator-require-composite-tier` (#710) · `generator-install-wiring` (#746) · `react-spa-generate-parity` (#750) · `enforcement-liveness-fix` (#745/#753) · `plugin-loadability` (#794/#804) + `plugin-packaging` (#672) · `rule-research-live-adapter` (#805/#809) · `rule-bootstrapping-iphase-spike` (#801) · `self-enforcement-fixes` (#778) · `research-application-fixes` (#774) · `egress-host-push-default` (#760) · `zero-dep-synth-bundle` (#763) · channel-discipline M6 gate (#765/#769) · dogfood issues #735/#737/#730/#727 · `aif-heal-portable` (#505) · `pressure-scenario` automation+generator (#572/#582) · `agents-toolname-fix-551-552` (#571) · `pipeline-i18n-fix`→language-discipline (#583) · `642-r2-wirer-fix` (#647) · `consumer-findings-tail-verify` (#569) · `aif-init-passport-gen` (#610) · `universalization-fix` (#515/#517/#526/#514) · `inject-layer-extension` (#494) · `getff-to-prod` U1/U2 (#785/#736) · `multi-stack-monorepo` (#793/#796) · `generation-live-delivery` R-phase (#797). **17 missing `done.md` closure markers backfilled in this reconcile's PR** (642-r2-wirer-fix, agents-toolname-fix-551-552[+ -meta-launch], pipeline-i18n-fix, pressure-scenario-automation[+ -meta-launch], pressure-scenario-generator, generator-install-wiring, aif-heal-portable, consumer-findings-tail-verify, react-spa-generate-parity, aif-init-passport-gen-meta-launch, consumer-install-completeness-meta-launch, generator-forbid-mvp-meta-launch, stage-2-generate-path-meta-launch, multi-stack-monorepo, generation-live-delivery).*
