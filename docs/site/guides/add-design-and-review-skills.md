...
[skills](../terms.md#skill). That is enough to stop bad code. It does not yet help your
...
The [skills overview](../reference/B.md) says what each one is for.

## Prerequisites

- A project where getff is already installed at the `core` [depth](../terms.md#depth).
...
2. Run the install. The `-y` flag answers the prompts for you.
   ...
   The run ends with the installer's [self-check](../terms.md#self-check), the same
