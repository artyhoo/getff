...
docs-refresh: deferred — re-verified 2026-09-21, this range adds only the one-line story-skill description row to the cited docs/site/reference/B.md (regenerated byte-identical to B.json by the plain-words-recap-v2 S4 marker rework) and changes two hash values in the cited python install fingerprint (refresh-baseline.json and the tool-bootstrapping skill); every line and path this page cites is unchanged; clears at the next gold refresh of this page
---
