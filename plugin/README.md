...
- `install/`  — `fetch-and-wire.sh`, the hard-layer seam (S5). It **fetches** the official `install.sh` at consent time rather than bundling a ~2 MB copy, so this step needs network access.
