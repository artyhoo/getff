...
# Сессионный story-блок (D-G, plain-words recap v2): секции 1-4 несут свои имена;
# секция 5 переиспользует AIF_EOT_SEC_NEXT и общую закрывающую грамматику ниже.
AIF_EOT_STORY_SEC_WHY='**Зачем всё это было.**'
AIF_EOT_STORY_SEC_NOW='**Что теперь по-другому.**'
AIF_EOT_STORY_SEC_DECIDED='**Что решили и кто.**'
AIF_EOT_STORY_SEC_UNSURE='**Меньше всего уверен.**'
AIF_EOT_ME_PREFIX='Я:'
...
# Общее тело секций для двух контуров пересказа (D-G, plain-words recap v2): $1 = "turn"
# (ходовой контракт ниже) | "session" (блок /story). Секции 1-4 — по контуру; секция 5 —
# область ворот D-B (R-17: ворота читают последнюю строку) — общая дословно, чтобы оба
# контура учили одну закрывающую грамматику. Берёт скаляры из Task 1.2, а не пересказывает
# их, и зовёт шаблон карточки из Task 1.3 — дословная копия любого из них разъехалась бы
# с паком, который читают ворота.
aif_msg_eot_recap_sections() {
  _eot_recap_scope="${1:-turn}"
  if [ "$_eot_recap_scope" = "session" ]; then
    cat <<EOF
1. ${AIF_EOT_STORY_SEC_WHY} — всегда, одной фразой: зачем всё это было, по-человечески.
2. ${AIF_EOT_STORY_SEC_NOW} — по каждому изменению: как было, как стало, что тебе это даёт; без хронологии.
3. ${AIF_EOT_STORY_SEC_DECIDED} — по одной строке, и кто решил (ты / я).
4. ${AIF_EOT_STORY_SEC_UNSURE} — где проверено слабо (1 прогон, 1 случай), что ещё осталось.
EOF
  else
    cat <<EOF
...
# Ходовой контракт пересказа (D-A, D-B) — тело, которое каждая ветка Stop-хука ниже несёт
# дословно через command substitution, чтобы три ветки учили один контракт, а не три слегка
# разных. Секции живут в aif_msg_eot_recap_sections выше, общие с сессионным (story)
# контуром, — один текст, чтобы формы не разъезжались. Здесь остаются ходовые части: лимит
# строк и два пункта самопроверки. AIF_EOT_RECAP_MAX_LINES сознательно НЕ скаляр пака, чтобы
# env-оверрайд оператора выживал.
...
• Если человек в этом ходе спросил, что значит слово, — объясни в ответе и в этом же ходе запиши слово в CONTEXT.md: определение и то, как человек его пишет, под _Operator says_.
EOF
}
...
# Символьный класс границ слова для матчинга сырых слов глоссария — category-3 MATCH DATA
# (language-discipline.md §1), дом — паки, а не тело хука: Surface 1 принципа 22 держит
# machinery-shell без кириллицы вне паков, пак-дом — контрмера того же правила из §5
# против #match-data-translated-away. Значение в обоих паках одинаково по дизайну (класс
# не зависит от языка пака); хук строит из него границы [^A-Za-z<CLASS>0-9].
# Буквы ПЕРЕЧИСЛЕНЫ, не диапазон: GNU grep под glibc C.UTF-8 (дефолт ubuntu-CI) отвергает
# кириллический диапазон в скобках с "Invalid collation character", и `grep ... || continue`
# в хуке молча пропускает каждый термин (измерено 2026-09-21, CI PR 1824). Перечисление
# валидно и в POSIX, и в C.UTF-8, и в en_US.UTF-8.
...
# P-7: согласованный с оператором заголовок (plain-words recap v2, D-G) — называет
# содержание блока, а не жанр; story-guard ищет этот литерал точным совпадением.
AIF_STORY_MARKER='## 🎬 Что изменилось за сессию'

# Story-ветка Stop-хука / скил /story: сессионный пересказ простыми словами, когда
# работа сдана (запушен PR). Тело — те же пять секций, что и у ходового пересказа,
# сессионный контур (aif_msg_eot_recap_sections session), чтобы формы не разъезжались.
# Hook-style: вся инструкция локализована, поэтому язык вывода следует активному паку.
# ${anchor:-} безопасен при unset (у /story нет transcript-anchor — цель называет сам
# из контекста).
aif_msg_eot_branch_story() {
  cat <<EOF
Работа сдана (только что запушен PR) — расскажи человеку, что изменила вся эта сессия, в первую очередь для него.
...
Тот же пересказ из пяти секций, что и каждый ход, — в масштабе сессии, один блок, в этом порядке:
$(aif_msg_eot_recap_sections session)
Вне секций — как писать:
...
• Одна мысль — одна фраза. Плохо: «Поправил X, но CI красный, потому что Y». Хорошо: «Поправил X. CI красный: Y.»
• Термин из CONTEXT.md пиши голым, как в словаре, и не заменяй его пересказом. Жаргон, которого в словаре нет, говори простыми словами. Плохо: «сделал merge-forward». Хорошо: «влил свежий staging в ветку».
Тон: просто и конкретно; без воды и само-поздравления; правда важнее гладкости. Если пункт не выходит конкретным — скажи прямо.
EOF
}
