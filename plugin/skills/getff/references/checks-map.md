...
> **Это обобщённая модель уровней, а не опись того, что ставит getff.** Уровни 5–8 называют
> Stryker, Pact Broker, Datadog и Argo Rollouts — getff не устанавливает ничего из этого; вы
> заводите их сами, если они вам нужны. То же и с уровнем 3: `tsc --noEmit`, `vitest related` и
> `dependency-cruiser` — это то, что на pre-push уместно держать, а не то, что getff туда
> прописывает. **Установленный getff хук `.husky/pre-push` запускает собственные проверки
> правил getff** — живость rule-глобов, разрешение бинарей lint-staged, срабатывание
> сгенерированных правил и проверку ссылок в изменённом Markdown; ваши typecheck, тесты и
> архитектурную проверку вы подключаете сами. Реестр секций — `packages/core/hooks/pre-push.ts`.
